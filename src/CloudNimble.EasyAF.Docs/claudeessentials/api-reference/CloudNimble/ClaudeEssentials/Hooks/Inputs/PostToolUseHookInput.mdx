---
title: PostToolUseHookInput
description: "Represents the input received by a PostToolUse hook.             This hook runs after tool calls complete and includes the tool's response."
icon: code-branch
keywords: ['PostToolUseHookInput', 'CloudNimble.ClaudeEssentials.Hooks.Inputs.PostToolUseHookInput<TToolInput, TToolResponse>', 'CloudNimble.ClaudeEssentials.Hooks.Inputs', 'class', 'CloudNimble.ClaudeEssentials.Hooks.Inputs.ToolHookInputBase<TToolInput>']
---
...
```csharp
CloudNimble.ClaudeEssentials.Hooks.Inputs.PostToolUseHookInput<TToolInput, TToolResponse>
```
...
- `TToolResponse` - The type representing the tool's response data.
            Use a specific tool response class or [Object](https://learn.microsoft.com/dotnet/api/system.object) for dynamic responses.
...
### <Icon icon="tag" iconType="duotone" color="#419AC5" size={24} className="mr-2" /> ToolResponse
