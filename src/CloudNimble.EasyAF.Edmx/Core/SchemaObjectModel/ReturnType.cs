...
namespace System.Data.Entity.Core.SchemaObjectModel
{
    internal class ReturnType : ModelFunctionTypeElement
    {
        ...
        internal override TypeUsage TypeUsage
        {
            get
            {
                if (_typeSubElement is not null)
                {
                    return _typeSubElement.GetTypeUsage();
                }
                else if (_typeUsage is not null)
                {
                    return _typeUsage;
                }
                else if (base.TypeUsage is null)
                {
                    return null;
                }
        ...
        {
            Debug.Assert(_typeSubElement is not null, "Nested type expected.");
        ...
        private void HandleTypeAttribute(XmlReader reader)
        {
            ...
            Debug.Assert(UnresolvedType is null);
            ...
            Function.RemoveTypeModifier(ref type, out var typeModifier, out _isRefType);
        ...
        internal override void ResolveTopLevelNames()
        {
            ...
            if (_unresolvedType is not null)
            {
            ...
            if (ParentElement.IsFunctionImport
                && _unresolvedEntitySet is not null)
            {
        ...
        internal override void Validate()
        {
                ...
                != SchemaDataModelOption.EntityDataModel)
            {
                    ...
                    == SchemaDataModelOption.ProviderManifestModel)
                {
                    ...
                    if (_type is not null && (_type is ScalarType == false || _collectionKind != CollectionKind.None)
                        ||
                        _typeSubElement is not null && _typeSubElement.Type is ScalarType == false)
                    {
                        var typeName = "";
                        ...
                        else if (_typeSubElement is not null)
                        {
                    ...
                    else if (_typeSubElement is not null)
                    {
                        if (_typeSubElement.Type is ScalarType == false)
                        {
                            ...
                            else
                            {
                                ...
                                Debug.Assert(collection is not null, "Can't find <CollectionType> inside TVF <ReturnType> element");
                                if (collection is not null)
                                {
                                    var row = collection.SubElement as RowTypeElement;
                                    Debug.Assert(row is not null, "Can't find <RowType> inside TVF <ReturnType><CollectionType> element");
                                    if (row is not null)
                                    {
