    ...
    internal sealed class EntityContainer : SchemaType
    {
        ...
        internal override void ResolveTopLevelNames()
        {
            if (!_isAlreadyResolved)
            {
                ...
                if (!String.IsNullOrEmpty(_unresolvedExtendedEntityContainerName))
                {
                    ...
                    else if (!Schema.SchemaManager.TryResolveType(null, _unresolvedExtendedEntityContainerName, out var extendingEntityContainer))
                    {
                        AddError(
        ...
        internal override void Validate()
        {
            ...
            if (!_isAlreadyValidated)
            {
                ...
                if (ExtendingEntityContainer is not null)
                {
            ...
            AddErrorKind error)
        {
            ...
            Debug.Assert(ExtendingEntityContainer is not null, "ExtendingEntityContainer must not be null");
        ...
        private void ValidateOnlyBaseEntitySetTypeDefinesConcurrency()
        {
            ...
            foreach (var type in Schema.SchemaTypes)
            {
                ...
                {
                    if (TypeIsSubTypeOf(itemType, baseEntitySetTypes, out var set)
                        &&
            ...
            out EntityContainerEntitySet set)
        {
            ...
            for (var baseType = itemType.BaseType as SchemaEntityType; baseType is not null; baseType = baseType.BaseType as SchemaEntityType)
            {
        ...
        private void CheckForDuplicateTableMapping(HashSet<string> tableKeys, EntityContainerEntitySet entitySet)
        {
            ...
            if (entitySet.DefiningQuery is not null)
            {
