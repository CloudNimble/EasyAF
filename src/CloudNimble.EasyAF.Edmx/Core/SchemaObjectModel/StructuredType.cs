...
namespace System.Data.Entity.Core.SchemaObjectModel
{
    ...
    internal abstract class StructuredType : SchemaType
    {
        ...
        public virtual bool IsTypeHierarchyRoot
        {
            ...
            {
                Debug.Assert(
                    (BaseType is null && _unresolvedBaseType is null) ||
                    (BaseType is not null && _unresolvedBaseType is not null),
            ...
            foreach (var member in NamedMembers)
            {
                if (BaseType is not null)
                {
                    ...
                    if (HowDefined.AsMember
                        == BaseType.DefinesMemberName(member.Name, out var definingType, out var definingMember))
                    {
        ...
        private HowDefined DefinesMemberName(string name, out StructuredType definingType, out SchemaElement definingMember)
        {
            ...
            Debug.Assert(definingMember is null, "we allow the scenario that members can have same name but different cases");
        ...
        private bool TryResolveBaseType()
        {
            ...
            if (UnresolvedBaseType is null)
            {
                _baseTypeResolveResult = true;
                return _baseTypeResolveResult.Value;
            }

            if (!Schema.ResolveTypeName(this, UnresolvedBaseType, out var element))
            {
        ...
        {
            Debug.Assert(UnresolvedBaseType is null, string.Format(CultureInfo.CurrentCulture, "{0} is already defined", reader.Name));

            if (!Utils.GetDottedName(Schema, reader, out var baseType))
            {
                return;
            }
