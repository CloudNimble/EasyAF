...
namespace System.Data.Entity.Core.SchemaObjectModel
{
    ...
    internal class StructuredProperty : Property
    {
        ...
        {
            if (!Schema.ResolveTypeName(this, typeName, out var element))
            {
                return null;
            }

            if (element is not SchemaComplexType
                && element is not ScalarType
                && element is not SchemaEnumType)
            {
                AddError(
        ...
        private void HandleTypeAttribute(XmlReader reader)
        {
            ...
            if (!Utils.GetDottedName(Schema, reader, out var type))
            {
                return;
            }
