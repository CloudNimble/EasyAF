...
namespace System.Data.Entity.Core.SchemaObjectModel
{
    ...
    internal class TypeUsageBuilder
    {
        ...
        private bool TryGetFacets(EdmType edmType, bool complainOnMissingFacet, out Dictionary<string, Facet> calculatedFacets)
        {
            ...
            {
                if (_facetValues.TryGetValue(defaultFacet.Name, out var value))
                {
        ...
        {
            Trace.Assert(_element is not null);
            Trace.Assert(scalar is not null);
            ...
            var noErrors = TryGetFacets(scalar.Type, complainOnMissingFacet, out var calculatedFacets);
            ...
            if (noErrors)
            {
                ...
                switch (scalar.TypeKind)
                {
                    ...
                    case PrimitiveTypeKind.DateOnly:
                        // DateOnly has no facets to validate
                        break;
                    case PrimitiveTypeKind.TimeOnly:
                        // TimeOnly supports precision facet like Time
                        ValidatePrecisionFacetsForDateTimeFamily(scalar.Type, calculatedFacets);
                        break;
                    ...
                    case PrimitiveTypeKind.GeometryCollection:
                        ...
                        break;
                    default:
                        Console.WriteLine($"WARNING: Unhandled PrimitiveTypeKind in TypeUsageBuilder: {scalar.Type.PrimitiveTypeKind}");
                        // Debug.Fail($"Did you miss a value: {scalar.Type.PrimitiveTypeKind}");
                        break;
                }
            }
        ...
        private void ValidateAndSetDecimalFacets(EdmType type, Dictionary<string, Facet> facets)
        {
            ...
            if (facets.TryGetValue(DbProviderManifest.PrecisionFacetName, out var precisionFacet)
                && precisionFacet.Value is not null)
            {
            ...
            if (facets.TryGetValue(DbProviderManifest.ScaleFacetName, out var scaleFacet)
                && scaleFacet.Value is not null)
            {
        ...
        private void ValidatePrecisionFacetsForDateTimeFamily(EdmType type, Dictionary<string, Facet> facets)
        {
            ...
            Debug.Assert(
                ...
                || (primitiveType.PrimitiveTypeKind == PrimitiveTypeKind.TimeOnly));
        ...
        private void ValidateLengthFacets(EdmType type, Dictionary<string, Facet> facets)
        {
            ...
            if (!facets.TryGetValue(DbProviderManifest.MaxLengthFacetName, out var maxLenFacet)
                || maxLenFacet.Value is null)
            {
                return;
            }
        ...
        private void ValidateSpatialFacets(EdmType type, Dictionary<string, Facet> facets)
        {
            ...
            if (_element.Schema.DataModel == SchemaDataModelOption.EntityDataModel
                && (!facets.TryGetValue(DbProviderManifest.IsStrictFacetName, out var isStrictFacet) || (bool)isStrictFacet.Value))
            {
                _element.AddError(
            ...
            //here we are assuming if the facet should be here or not is already get checked before
            if (!facets.TryGetValue(DbProviderManifest.SridFacetName, out var sridFacet)
                || sridFacet.Value is null)
            {
                return;
            }
