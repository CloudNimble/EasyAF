    ...
    internal class ViewSimplifier
    {
            ...
            ReferentialConstraint fkConstraint)
        {
            ...
            if (fkConstraint.FromProperties.Count == 1)
            {
                ...
                principalKeyValues = [singleKeyNameAndValue.Item2];
            }
            else
            {
        ...
        private static DbExpression SimplifyNestedTphDiscriminator(DbExpression expression)
        {
                ...
                if (predicateColumnNames.Contains(propName))
                {
                    ...
                    if (
                        !TryMatchPropertyEqualsValue(
                            casePredicate.When[0], rowProjection.Input.VariableName, out var comparedProp, out var constValue)
                        ||
            ...
            for (var idx = 0; idx < entitySelector.When.Count; idx++)
            {
                ...
                if (!selectorPredicateMap.TryGetValue(propWhen.Property.Name, out var discriminatorWhen))
                {
                    return null;
                }
        ...
        private class ValueSubstituter : DefaultExpressionVisitor
        {
            ...
            public override DbExpression Visit(DbPropertyExpression expression)
            {
                ...
                if (expression.Instance.ExpressionKind == DbExpressionKind.VariableReference
                    ...
                    &&
                    replacements.TryGetValue(expression.Property.Name, out var replacementValue))
                {
        ...
        private static DbExpression SimplifyCaseStatement(DbExpression expression)
        {
            ...
            {
                if (TrySimplifyPredicate(when, out var simplifiedPredicate))
                {
            ...
            DbFilterExpression filter, Action<DbComparisonExpression, object> onMatchedComparison)
        {
            ...
            {
                if (!TryMatchPropertyEqualsValue(term, filter.Input.VariableName, out var currentDiscriminator, out var discriminatorValue))
                {
                    return false;
                }
