...
namespace System.Data.Entity.Core.Common.CommandTrees.ExpressionBuilder
{
    ...
    public static class DbExpressionBuilder
    {
        ...
        private static readonly AliasGenerator _bindingAliases = new("Var_", 0);
        ...
        public static DbExpressionBinding BindAs(this DbExpression input, string varName)
        {
            ...
            if (!TypeHelpers.TryGetCollectionElementType(input.ResultType, out var elementType))
            {
        ...
        {
            var funcArgs = ArgumentValidation.ValidateFunctionAggregate(function, [argument]);
        ...
        public static DbGroupAggregate GroupAggregate(DbExpression argument)
        {
            ...
            var arguments = new DbExpressionList([argument]);
            ;
        ...
        public static DbCrossJoinExpression CrossJoin(IEnumerable<DbExpressionBinding> inputs)
        {
            ...
            var validInputs = ArgumentValidation.ValidateCrossJoin(inputs, out var resultType);
            ...
            IEnumerable<KeyValuePair<string, DbAggregate>> aggregates)
        {
            ...
            var resultType = ArgumentValidation.ValidateGroupBy(keys, aggregates, out var validKeys, out var validAggregates);
    ...
    // Binary 
        ...
        private static DbArithmeticExpression CreateArithmetic(DbExpressionKind kind, DbExpression left, DbExpression right)
        {
            ...
            var arguments = new DbExpressionList([left, right]);
        ...
        public static DbArithmeticExpression UnaryMinus(this DbExpression argument)
        {
            ...
            return new DbArithmeticExpression(DbExpressionKind.UnaryMinus, resultType, new DbExpressionList([argument]));
        }
        ...
        private static void ValidateOfType(DbExpression argument, TypeUsage type)
        {
            ...
            if (!TypeHelpers.TryGetCollectionElementType(argument.ResultType, out var elementType)
        ...
        public static DbDerefExpression Deref(this DbExpression argument)
        {
            ...
            if (!TypeHelpers.TryGetRefEntityType(argument.ResultType, out var entityType))
            {
        ...
        {
            var refResultType = ArgumentValidation.ValidateCreateRef(entitySet, entitySet.ElementType, keyValues, out var keyConstructor);
        ...
        private static DbRefExpression CreateRefExpression(EntitySet entitySet, EntityType entityType, IEnumerable<DbExpression> keyValues)
        {
            ...
            var refResultType = ArgumentValidation.ValidateCreateRef(entitySet, entityType, keyValues, out var keyConstructor);
            return new DbRefExpression(refResultType, entitySet, keyConstructor);
        }
        ...
        public static DbRefKeyExpression GetRefKey(this DbExpression argument)
        {
            ...
            Debug.Assert(refType.ElementType is not null, "RefType constructor allowed null ElementType?");
            ...
            this DbExpression navigateFrom, RelationshipEndMember fromEnd, RelationshipEndMember toEnd)
        {
            ...
            var resultType = ArgumentValidation.ValidateNavigate(
                navigateFrom, fromEnd, toEnd, out var relType, allowAllRelationshipsInSameTypeHierarchy: false);
            ...
            this RelationshipType type, string fromEndName, string toEndName, DbExpression navigateFrom)
        {
            ...
            var resultType = ArgumentValidation.ValidateNavigate(navigateFrom, type, fromEndName, toEndName, out var fromEnd, out var toEnd);
            ...
            IEnumerable<DbExpression> whenExpressions, IEnumerable<DbExpression> thenExpressions, DbExpression elseExpression)
        {
            ...
            var resultType = ArgumentValidation.ValidateCase(
                whenExpressions, thenExpressions, elseExpression, out var validWhens, out var validThens);
        ...
        {
            var resultType = ArgumentValidation.ValidateFunction(function, arguments, out var validArguments);
        ...
        {
            var resultType = ArgumentValidation.ValidateInvoke(lambda, arguments, out var validArguments);
        ...
        {
            var resultType = ArgumentValidation.ValidateNew(instanceType, arguments, out var validArguments);
        ...
        public static DbNewInstanceExpression NewEmptyCollection(this TypeUsage collectionType)
        {
            ...
            var validResultType = ArgumentValidation.ValidateNewEmptyCollection(collectionType, out var validElements);
        ...
        public static DbNewInstanceExpression NewRow(IEnumerable<KeyValuePair<string, DbExpression>> columnValues)
        {
            ...
            var resultType = ArgumentValidation.ValidateNewRow(columnValues, out var validElements);
        ...
        private static DbPropertyExpression PropertyByName(DbExpression instance, string propertyName, bool ignoreCase)
        {
            ...
            var resultType = ArgumentValidation.ValidateProperty(instance, propertyName, ignoreCase, out var property);
        ...
        internal static string[] ExtractAliases(MethodInfo method)
        {
            ...
            var generateNames = methodParams.Skip(start).Any(p => p.Name is null);
        ...
        private static bool TryResolveToConstant(Type type, object value, out DbExpression constantOrNullExpression)
        {
            ...
            if (ClrProviderManifest.TryGetPrimitiveTypeKind(valueType, out var primitiveTypeKind))
            {
            ...
            return (constantOrNullExpression is not null);
        }
        ...
        private static DbExpression ResolveToExpression<TArgument>(TArgument argument)
        {
            ...
            if (TryResolveToConstant(typeof(TArgument), untypedArgument, out var constantResult))
            {
                return constantResult;
            }

            if (untypedArgument is null)
            {
                return null;
            }
        ...
        {
            var sourceBinding = ConvertToBinding(source, apply, out var applyTemplate);
        ...
        public static DbQuantifierExpression All(this DbExpression source, Func<DbExpression, DbExpression> predicate)
        {
            ...
            var input = ConvertToBinding(source, predicate, out var predicateExp);
            ...
            this DbExpression left, DbExpression right, Func<DbExpression, DbExpression, DbExpression> joinCondition)
        {
            ...
            var inputs = ConvertToBinding(left, right, joinCondition, out var condExp);
            ...
            Func<DbExpression, DbExpression> innerKey)
        {
            ...
            var leftBinding = ConvertToBinding(outer, outerKey, out var leftOperand);

            var rightBinding = ConvertToBinding(inner, innerKey, out var rightOperand);
        ...
        public static DbSortExpression OrderBy(this DbExpression source, Func<DbExpression, DbExpression> sortKey)
        {
            ...
            var input = ConvertToBinding(source, sortKey, out var keyExpression);
            var sortClause = keyExpression.ToSortClause();
            return input.Sort([sortClause]);
        }
        ...
        public static DbProjectExpression Select<TProjection>(this DbExpression source, Func<DbExpression, TProjection> projection)
        {
            ...
            var input = ConvertToBinding(source, projection, out var intermediateProjection);
        ...
        public static DbProjectExpression SelectMany(this DbExpression source, Func<DbExpression, DbExpression> apply)
        {
            ...
            var inputBinding = ConvertToBinding(source, apply, out var functorResult);
...
#region Internal Helper API - ideally these methods should be removed
            ...
            DbExpressionKind joinKind, DbExpression joinCondition, DbExpressionBinding input1, DbExpressionBinding input2)
        {
            ...
            {
                Debug.Assert(joinCondition is null, "Condition should not be specified for CrossJoin");
                return CrossJoin([input1, input2]);
            }
            else
            {
                Debug.Assert(joinCondition is not null, "Condition must be specified for non-CrossJoin");
            ...
            EntityType entityType, IList<DbExpression> attributeValues, IList<DbRelatedEntityRef> relationships)
        {
            var resultType = ArgumentValidation.ValidateNewEntityWithRelationships(
                entityType, attributeValues, relationships, out var validAttributes, out var validRelatedRefs);
        ...
        {
            var resultType = ArgumentValidation.ValidateNavigate(
                navigateFrom, fromEnd, toEnd, out var relType, allowAllRelationshipsInSameTypeHierarchy: true);
            return new DbRelationshipNavigationExpression(resultType, relType, fromEnd, toEnd, navigateFrom);
        }
