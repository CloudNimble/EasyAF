...
namespace System.Data.Entity.Core.Common.CommandTrees
{
    ...
    public class DbExpressionRebinder : DefaultExpressionVisitor
    {
        ...
        {
            if (_metadata.TryGetEntityContainer(entitySet.EntityContainer.Name, entitySet.EntityContainer.DataSpace, out var container))
            {
                if (container.BaseEntitySets.TryGetValue(entitySet.Name, false, out var extent)
                    &&
                    extent is not null
                    &&
        ...
        protected override EdmFunction VisitFunction(EdmFunction functionMetadata)
        {
            ...
            else
            {
                ...
                if (_perspective.TryGetFunctionByName(
                    functionMetadata.NamespaceName, functionMetadata.Name, /*ignoreCase:*/ false, out var candidateFunctions))
                {
                    Debug.Assert(
                    ...
                    var retFunc = FunctionOverloadResolver.ResolveFunctionOverloads(
                        candidateFunctions, paramTypes, /*isGroupAggregateFunction:*/ false, out var isAmbiguous);
                    if (!isAmbiguous
                        &&
                        retFunc is not null)
                    {
        ...
        private static bool TryGetMember<TMember>(DbExpression instance, string memberName, out TMember member) where TMember : EdmMember
        {
            ...
            {
                if (declType.Members.TryGetValue(memberName, false, out var foundMember))
                {
        ...
        public override DbExpression Visit(DbPropertyExpression expression)
        {
            ...
            if (!ReferenceEquals(expression.Instance, newInstance))
            {
                ...
                {
                    if (!TryGetMember(newInstance, expression.Property.Name, out RelationshipEndMember endMember))
                    {
                ...
                {
                    if (!TryGetMember(newInstance, expression.Property.Name, out NavigationProperty navProp))
                    {
                        var type = newInstance.ResultType.EdmType;
                        throw new ArgumentException(
                ...
                {
                    if (!TryGetMember(newInstance, expression.Property.Name, out EdmProperty prop))
                    {
                        var type = newInstance.ResultType.EdmType;
                        throw new ArgumentException(
