...
namespace System.Data.Entity.Core.Common.EntitySql
{
    ...
    internal sealed class TypeResolver
    {
        ...
        internal void DeclareInlineFunction(string name, InlineFunctionInfo functionInfo)
        {
            ...
            if (!_functionDefinitions.TryGetValue(name, out var overloads))
            {
                overloads = [];
        ...
        {
            private static readonly TypeUsageStructuralComparer _instance = new();
        ...
        internal MetadataMember ResolveMetadataMemberAccess(MetadataMember qualifier, string name, ErrorContext errCtx)
        {
                ...
                == MetadataMemberClass.Namespace)
            {
                ...
                //
                if (TryGetFunctionFromMetadata(qualifier.Name, name, out var function))
                {
                    return function;
                }
                     ...
                     == MetadataMemberClass.Type)
            {
                ...
                if (TypeSemantics.IsEnumerationType(type.TypeUsage))
                {
                    if (_perspective.TryGetEnumMember(
                        (EnumType)type.TypeUsage.EdmType, name, _parserOptions.NameComparisonCaseInsensitive /*ignoreCase*/, out var member))
                    {
        ...
        internal MetadataMember ResolveUnqualifiedName(string name, bool partOfQualifiedName, ErrorContext errCtx)
        {
            ...
            if (!resolveAsNamespaceOnly
                &&
                includeFunctions
                && TryGetInlineFunction(name, out var inlineFunctionGroup))
            {
            ...
            //
            if (_aliasedNamespaces.TryGetValue(name, out var aliasedNamespaceImport))
            {
            ...
            if (!resolveAsNamespaceOnly)
            {
                ...
                if (!TryGetTypeFromMetadata(name, out var type))
                {
                    if (includeFunctions)
                    {
                        //
                ...
                foreach (var namespaceImport in _namespaces)
                {
                    ...
                    if (TryGetTypeFromMetadata(fullName, out var importedType))
                    {
                        if (type is null
                            && functionGroup is null)
                        {
                            type = importedType;
                    ...
                    if (includeFunctions && TryGetFunctionFromMetadata(namespaceImport.Name, name, out var importedFunctionGroup))
                    {
                        if (type is null
                            && functionGroup is null)
                        {
        ...
        {
            Debug.Assert(name is not null && length > 1 && length <= name.Length, "name must not be empty");
        ...
        {
            var message = Strings.AmbiguousMetadataMemberName(name, ns1.Name, ns2 is not null ? ns2.Name : null);
            throw EntitySqlException.Create(errCtx, message, null);
        }
        ...
        {
            if (_perspective.TryGetTypeByName(typeFullName, _parserOptions.NameComparisonCaseInsensitive /* ignore case */, out var typeUsage))
            {
        ...
        internal bool TryGetFunctionFromMetadata(string namespaceName, string functionName, out MetadataFunctionGroup functionGroup)
        {
            if (_perspective.TryGetFunctionByName(
                namespaceName, functionName, _parserOptions.NameComparisonCaseInsensitive /* ignore case */, out var functionMetadata))
            {
        ...
        {
            if (_includeInlineFunctions && _functionDefinitions.TryGetValue(functionName, out var inlineFunctionMetadata))
            {
