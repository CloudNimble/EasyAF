...
namespace System.Data.Entity.Core.Common.EntitySql.AST
{
    ...
    internal sealed class DotExpr : Node
    {
        ...
        internal bool IsMultipartIdentifier(out string[] names)
        {
            ...
            if (leftIdenitifier is not null)
            {
                _names = [leftIdenitifier.Name, _identifier.Name];
            }

            var leftDotExpr = _leftExpr as DotExpr;
            if (leftDotExpr is not null
                && leftDotExpr.IsMultipartIdentifier(out var leftNames))
            {
            ...
            Debug.Assert(_names is null || _names.Length > 0, "_names must be null or non-empty");

            _isMultipartIdentifierComputed = _names is not null;
            names = _names;
            return _isMultipartIdentifierComputed.Value;
        }
    }
