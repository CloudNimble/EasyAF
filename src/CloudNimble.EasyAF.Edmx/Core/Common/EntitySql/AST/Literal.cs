...
namespace System.Data.Entity.Core.Common.EntitySql.AST
{
    ...
    internal sealed class Literal : Node
    {
        ...
        internal void PrefixSign(string sign)
        {
            ...
            Debug.Assert(_computedValue is null);
        ...
        private static readonly char[] _numberSuffixes = ['U', 'u', 'L', 'l', 'F', 'f', 'M', 'm', 'D', 'd'];
        private static readonly char[] _floatTokens = ['.', 'E', 'e'];
        ...
        private static object ConvertNumericLiteral(ErrorContext errCtx, string numericString)
        {
            ...
            if (-1 != k)
            {
                ...
                switch (suffix)
                {
                    case "U":
                        {
                            if (!UInt32.TryParse(numberPart, NumberStyles.Integer, CultureInfo.InvariantCulture, out var value))
                            {
                    ...
                    case "L":
                        {
                            if (!Int64.TryParse(numberPart, NumberStyles.Integer, CultureInfo.InvariantCulture, out var value))
                            {
                    ...
                    case "LU":
                        {
                            if (!UInt64.TryParse(numberPart, NumberStyles.Integer, CultureInfo.InvariantCulture, out var value))
                            {
                    ...
                    case "F":
                        {
                            if (!Single.TryParse(numberPart, NumberStyles.Float, CultureInfo.InvariantCulture, out var value))
                            {
        ...
        private static object DefaultNumericConversion(string numericString, ErrorContext errCtx)
        {
            ...
            {
                if (!Double.TryParse(numericString, NumberStyles.Float, CultureInfo.InvariantCulture, out var value))
                {
                    var message = Strings.CannotConvertNumericLiteral(numericString, "double");
                    throw EntitySqlException.Create(errCtx, message, null);
                }
            ...
            {
                if (Int32.TryParse(numericString, NumberStyles.Integer, CultureInfo.InvariantCulture, out var int32Value))
                {
                    return int32Value;
                }

                if (!Int64.TryParse(numericString, NumberStyles.Integer, CultureInfo.InvariantCulture, out var int64Value))
                {
                    var message = Strings.CannotConvertNumericLiteral(numericString, "long");
                    throw EntitySqlException.Create(errCtx, message, null);
                }
        ...
        {
            if (!Boolean.TryParse(booleanLiteralValue, out var result))
            {
        ...
        private static string GetStringLiteralValue(string stringLiteralValue, bool isUnicode)
        {
            ...
            // paranoid assumption that all input is evil. we should not see this exception under normal 
            // conditions.
            var before = stringLiteralValue.Split([delimiter]).Length - 1;
            ...
            // paranoid assumption that all input is evil. we should not see this exception under normal 
            // conditions.
            var after = result.Split([delimiter]).Length - 1;
        ...
        private static readonly char[] _datetimeOffsetSeparators = [' ', ':', '-', '.', '+', '-'];
        ...
        private static DateTimeOffset ConvertDateTimeOffsetLiteralValue(ErrorContext errCtx, string datetimeLiteralValue)
        {
            ...
            GetTimeParts(datetimeLiteralValue, timeParts, 3, out var hour, out var minute, out var second, out var ticks);
