...
namespace System.Data.Entity.Core.Common.EntitySql
{
    ...
    internal sealed class SemanticAnalyzer
    {
        ...
        internal ParseResult AnalyzeCommand(Node astExpr)
        {
            ...
            Debug.Assert(parseResult is not null, "ConvertStatement produced null parse result");
            Debug.Assert(parseResult.CommandTree is not null, "ConvertStatement returned null command tree");
        ...
        internal DbLambda AnalyzeQueryCommand(Node astExpr)
        {
            ...
            //
            var expression = ConvertQueryStatementToDbExpression(astQueryCommandExpr.Statement, _sr, out var functionDefs);
            ...
            Debug.Assert(lambda is not null, "AnalyzeQueryCommand returned null");
            ...
            NodeList<NamespaceImport> nsImportList, ErrorContext cmdErrCtx, SemanticResolver sr)
        {
            ...
            if (nsImportList is not null)
            {
                foreach (var namespaceImport in nsImportList)
                {
                    ...
                    if (dotExpr is not null
                        && dotExpr.IsMultipartIdentifier(out name))
                    {
                        Debug.Assert(name is not null, "name is not null");
                    }
                    ...
                    var alias = namespaceImport.Alias is not null ? namespaceImport.Alias.Name : null;

                    var metadataMember = sr.ResolveMetadataMemberName(name, namespaceImport.NamespaceName.ErrCtx);
                    Debug.Assert(metadataMember is not null, "metadata member name resolution must not return null");
            ...
            sr.TypeResolver.AddNamespaceImport(
                new MetadataNamespace(EdmConstants.EdmNamespace), nsImportList is not null ? nsImportList.ErrCtx : cmdErrCtx);
        ...
        private static ParseResult ConvertStatement(Statement astStatement, SemanticResolver sr)
        {
            ...
            Debug.Assert(converted is not null, "statementConverter returned null");
            Debug.Assert(converted.CommandTree is not null, "statementConverter produced null command tree");
        ...
        private static ParseResult ConvertQueryStatementToDbCommandTree(Statement astStatement, SemanticResolver sr)
        {
            ...
            var converted = ConvertQueryStatementToDbExpression(astStatement, sr, out var functionDefs);

            Debug.Assert(converted is not null, "ConvertQueryStatementToDbExpression returned null");
            Debug.Assert(functionDefs is not null, "ConvertQueryStatementToDbExpression produced null functionDefs");
            ...
            NodeList<AST.FunctionDefinition> functionDefList, SemanticResolver sr)
        {
            ...
            if (functionDefList is not null)
            {
                //
                ...
                foreach (var functionDefAst in functionDefList)
                {
                    ...
                    Debug.Assert(parameters is not null, "parameters must not be null"); // should be empty collection if no parameters
            ...
            NodeList<PropDefinition> parameterDefs, SemanticResolver sr)
        {
            ...
            if (parameterDefs is not null)
            {
                foreach (var paramDef in parameterDefs)
                {
                    ...
                    Debug.Assert(typeUsage is not null, "typeUsage must not be null");
        ...
        private sealed class InlineFunctionInfoImpl : InlineFunctionInfo
        {
            ...
            {
                if (_convertedDefinition is null)
                {
                    //
        ...
        private static DbExpression ConvertValueExpressionAllowUntypedNulls(Node astExpr, SemanticResolver sr)
        {
            ...
            if (dotExpr is not null
                && dotExpr.IsMultipartIdentifier(out var names))
            {
        ...
        private static Pair<DbExpression, DbExpression> ConvertValueExpressionsWithUntypedNulls(
            ...
            SemanticResolver sr)
        {
            var leftExpr = leftAst is not null ? ConvertValueExpressionAllowUntypedNulls(leftAst, sr) : null;
            var rightExpr = rightAst is not null ? ConvertValueExpressionAllowUntypedNulls(rightAst, sr) : null;
            ...
            else rightExpr ??= leftExpr.ResultType.Null();
        ...
        private static TypeUsage GetLiteralTypeUsage(Literal literal)
        {
            ...
            if (!ClrProviderManifest.Instance.TryGetPrimitiveType(literal.Type, out var primitiveType))
            {
        ...
        private static ExpressionResolution ConvertGroupPartitionExpr(Node astExpr, SemanticResolver sr)
        {
            ...
            //
            if (!TryConvertAsResolvedGroupAggregate(groupAggregateExpr, sr, out var converted))
            {
                //
        ...
        private static ExpressionResolution ConvertMethodExpr(MethodExpr methodExpr, bool includeInlineFunctions, SemanticResolver sr)
        {
            ...
            using (sr.TypeResolver.EnterFunctionNameResolution(includeInlineFunctions))
            {
                ...
                if (simpleFunctionName is not null)
                {
        ...
        {
            return leftExpr is not null && leftExpr.Left is Identifier ? sr.EnterIgnoreEntityContainerNameResolution() : null;
        }
        ...
        private static IDisposable ConvertMethodExpr_TryEnterV1ViewGenBackwardCompatibilityResolution(DotExpr leftExpr, SemanticResolver sr)
        {
                 ...
                 sr.ParserOptions.ParserCompilationMode == ParserOptions.CompilationMode.UserViewGenerationMode))
            {
                ...
                Debug.Assert(mappingCollection is not null, "mappingCollection is not null");
            ...
            out ValueExpression inlineFunctionCall)
        {
            ...
            //
            var args = ConvertFunctionArguments(methodExpr.Args, sr, out var argTypes);
        ...
        private static ValueExpression ConvertTypeConstructorCall(MetadataType metadataType, MethodExpr methodExpr, SemanticResolver sr)
        {
            ...
            if (methodExpr.HasRelationships)
            {
                ...
                for (var i = 0; i < methodExpr.Relationships.Count; i++)
                {
                    ...
                    var targetEndId = String.Join(
                        ":", [relshipTarget.TargetEnd.DeclaringType.Identity, relshipTarget.TargetEnd.Identity]);
            ...
            return new ValueExpression(
                CreateConstructorCallExpression(
                    ...
                    ConvertFunctionArguments(methodExpr.Args, sr, out var argTypes),
                    relshipExprList,
                    sr));
        }
            ...
            MethodExpr methodExpr, MetadataFunctionGroup metadataFunctionGroup, SemanticResolver sr)
        {
            ...
            //
            if (TryConvertAsResolvedGroupAggregate(methodExpr, sr, out var converted))
            {
                return converted;
            }
            ...
            //
            var savedInnermostReferencedScopeRegion = sr.CurrentGroupAggregateInfo is not null
            ...
            else if (sr.CurrentGroupAggregateInfo is not null)
            {
                sr.CurrentGroupAggregateInfo.InnermostReferencedScopeRegion = savedInnermostReferencedScopeRegion;
            }
            Debug.Assert(argTypes is not null, "argTypes is not null");
            ...
            GroupAggregateExpr groupAggregateExpr, SemanticResolver sr, out DbExpression converted)
        {
            ...
            // otherwise exit.
            //
            if (groupAggregateExpr.AggregateInfo is null)
            {
                return false;
            }
            ...
            {
                Debug.Assert(
                    groupAggregateExpr.AggregateInfo.AggregateStubExpression is not null, "Resolved aggregate stub expression must not be null.");
                converted = groupAggregateExpr.AggregateInfo.AggregateStubExpression;
            }

            Debug.Assert(converted is not null, "converted is not null");
            ...
            out DbExpression converted)
        {
            ...
            {
                args = ConvertFunctionArguments(methodExpr.Args, sr, out var aggArgTypes);
                // Sanity check - argument types must agree.
                Debug.Assert(
                    argTypes.Count == aggArgTypes.Count &&
                    argTypes.Zip(aggArgTypes).All(
                        types => types.Key is null && types.Value is null || TypeSemantics.IsStructurallyEqual(types.Key, types.Value)),
            ...
            SemanticResolver sr)
        {
            ...
            //
            if (relshipExprList is not null
            ...
            NodeList<Node> astExprList, SemanticResolver sr, out List<TypeUsage> argTypes)
        {
            ...
            argTypes = convertedArgs.Select(a => a is not null ? a.ResultType : null).ToList();
        ...
        private static ExpressionResolution ConvertParameter(Node expr, SemanticResolver sr)
        {
            ...
            if (null == sr.Parameters
                || !sr.Parameters.TryGetValue(parameter.Name, out var paramRef))
            {
            ...
            RelshipNavigationExpr relshipExpr, EntityType driverEntityType, SemanticResolver sr)
        {
            ...
            if (relshipExpr.ToEndIdentifier is not null)
            {
                toEnd =
            ...
            Debug.Assert(toEnd is not null, "toEnd must be resolved.");

            //
            // Convert From end if explicitly defined, derive if implicit.
            //
            RelationshipEndMember fromEnd;
            if (relshipExpr.FromEndIdentifier is not null)
            {
                fromEnd =
                    (RelationshipEndMember)
                    relationshipType.Members.FirstOrDefault(
            ...
            Debug.Assert(fromEnd is not null, "fromEnd must be resolved.");
            ...
            else
            {
                var fromEndCandidates = relationshipType.Members.Select(m => (RelationshipEndMember)m)
                                                        .Where(
                                                            ...
                                                            (toEnd is null || !e.EdmEquals(toEnd))).ToArray();
                switch (fromEndCandidates.Length)
                {
                    case 1:
                        fromEnd = fromEndCandidates[0];
                    ...
                    case 0:
                        ...
                        throw EntitySqlException.Create(errCtx, message, null);
                    default:
                        Debug.Assert(toEnd is null, "N-ary relationship? N > 2");
                        var errCtx1 = relshipExpr.ErrCtx;
                        var message1 = Strings.RelationshipFromEndIsAmbiguos;
                        throw EntitySqlException.Create(errCtx1, message1, null);
                }
        ...
        private static Pair<DbExpression, DbExpression> ConvertPlusOperands(BuiltInExpr astBuiltInExpr, SemanticResolver sr)
        {
            ...
            if (TypeHelpers.GetCommonTypeUsage(operands.Left.ResultType, operands.Right.ResultType) is null)
            {
                var errCtx = astBuiltInExpr.ErrCtx;
                var message = Strings.ArgumentTypesAreIncompatible(
                    operands.Left.ResultType.EdmType.FullName, operands.Right.ResultType.EdmType.FullName);
        ...
        private static Pair<DbExpression, DbExpression> ConvertLogicalArgs(BuiltInExpr astBuiltInExpr, SemanticResolver sr)
        {
            ...
            if (astBuiltInExpr.Arg2 is not null)
            {
        ...
        private static Pair<DbExpression, DbExpression> ConvertSetArgs(BuiltInExpr astBuiltInExpr, SemanticResolver sr)
        {
            ...
            if (null != astBuiltInExpr.Arg2)
            {
                ...
                if (!TypeSemantics.TryGetCommonType(leftElemType, rightElemType, out var commonType))
                {
        ...
        private static TypeUsage ConvertTypeName(Node typeName, SemanticResolver sr)
        {
            ...
            if (dotExpr is not null
                && dotExpr.IsMultipartIdentifier(out name))
            {
                Debug.Assert(name is not null, "name is not null for a multipart identifier");
            }
        ...
        private static TypeUsage ConvertTypeSpecArgs(TypeUsage parameterizedType, NodeList<Node> typeSpecArgs, ErrorContext errCtx)
        {
            ...
            //
            ConvertTypeFacetValue(primitiveType, (Literal)typeSpecArgs[0], DbProviderManifest.PrecisionFacetName, out var precision);
        ...
        private static TypeUsage ConvertTypeDefinition(Node typeDefinitionExpr, SemanticResolver sr)
        {
            ...
            if (collTypeDefExpr is not null)
            {
            ...
            else if (refTypeDefExpr is not null)
            {
            ...
            else if (rowTypeDefExpr is not null)
            {
                Debug.Assert(
                    rowTypeDefExpr.Properties is not null && rowTypeDefExpr.Properties.Count > 0,
            ...
            Debug.Assert(converted is not null, "Type definition conversion yielded null");
        ...
        private static ExpressionResolution ConvertRowConstructor(Node expr, SemanticResolver sr)
        {
            ...
            for (var i = 0; i < rowExpr.AliasedExprList.Count; i++)
            {
                ...
                {
                    if (aliasExpr.Alias is not null)
                    {
        ...
        private static ExpressionResolution ConvertMultisetConstructor(Node expr, SemanticResolver sr)
        {
            ...
            var multisetTypes = mSetExprs.Where(e => e is not null).Select(e => e.ResultType).ToArray();
        ...
        private static ExpressionResolution ConvertCaseExpr(Node expr, SemanticResolver sr)
        {
            ...
            //
            var elseExpr = caseExpr.ElseExpr is not null ? ConvertValueExpressionAllowUntypedNulls(caseExpr.ElseExpr, sr) : null;

            //
            // Collect result types from THENs and the ELSE.
            //
            var resultTypes = thenExprList.Where(e => e is not null).Select(e => e.ResultType).ToList();
        ...
        private static ExpressionResolution ConvertQueryExpr(Node expr, SemanticResolver sr)
        {
            ...
            if (queryExpr.SelectClause.TopExpr is not null)
            {
                if (queryExpr.OrderByClause is not null
                    && queryExpr.OrderByClause.LimitSubClause is not null)
                {
                ...
                if (queryExpr.OrderByClause is not null
                    && queryExpr.OrderByClause.SkipSubClause is not null)
                {
                    var errCtx = queryExpr.SelectClause.TopExpr.ErrCtx;
            ...
            DbExpressionBinding source, QueryExpr queryExpr, bool queryProjectionProcessed, SemanticResolver sr)
        {
            ...
            //
            if (selectClause.TopExpr is not null
                || (queryExpr.OrderByClause is not null && queryExpr.OrderByClause.LimitSubClause is not null))
            {
                Node limitExpr;
                string exprName;
                if (selectClause.TopExpr is not null)
                {
                    Debug.Assert(
                        queryExpr.OrderByClause is null || queryExpr.OrderByClause.LimitSubClause is null,
        ...
        private static List<KeyValuePair<string, DbExpression>> ConvertSelectClauseItems(QueryExpr queryExpr, SemanticResolver sr)
        {
                ...
                == SelectKind.Value)
            {
                ...
                //
                if (selectClause.Items[0].Alias is not null
                    && queryExpr.OrderByClause is null)
                {
            ...
            for (var i = 0; i < selectClause.Items.Count; i++)
            {
                ...
                {
                    if (projectionItem.Alias is not null)
                    {
            ...
            TypeUsage projectExpressionResultType, ErrorContext defaultErrCtx, List<ErrorContext> projectionItemErrCtxs)
        {
            ...
            if (!TypeHelpers.IsValidDistinctOpType(projectionType))
            {
                ...
                if (projectionItemErrCtxs is not null
        ...
        private static DbExpressionBinding ProcessFromClause(FromClause fromClause, SemanticResolver sr)
        {
            ...
            for (var i = 0; i < fromClause.FromClauseItems.Count; i++)
            {
                ...
                //
                var currentItemBinding = ProcessFromClauseItem(fromClause.FromClauseItems[i], sr, out var fromClauseItemEntries);
                fromClauseEntries.AddRange(fromClauseItemEntries);

                if (fromBinding is null)
                {
            ...
            Debug.Assert(fromBinding is not null, "fromBinding is not null");
            ...
            FromClauseItem fromClauseItem, SemanticResolver sr, out List<SourceScopeEntry> scopeEntries)
        {
            ...
            Debug.Assert(fromItemBinding is not null, "fromItemBinding is not null");
            ...
            AliasedExpr aliasedExpr, SemanticResolver sr, out List<SourceScopeEntry> scopeEntries)
        {
            ...
            {
                if (aliasedExpr.Alias is not null)
                {
            ...
            scopeEntries = [sourceScopeEntry];

            Debug.Assert(aliasedBinding is not null, "aliasedBinding is not null");
            ...
            JoinClauseItem joinClause, SemanticResolver sr, out List<SourceScopeEntry> scopeEntries)
        {
            ...
            //
            var leftBindingExpr = ProcessFromClauseItem(joinClause.LeftExpr, sr, out var leftExprScopeEntries);
            ...
            //
            var rightBindingExpr = ProcessFromClauseItem(joinClause.RightExpr, sr, out var rightExprScopeEntries);
            ...
            Debug.Assert(joinBinding is not null, "joinBinding is not null");
            ...
            ApplyClauseItem applyClause, SemanticResolver sr, out List<SourceScopeEntry> scopeEntries)
        {
            ...
            //
            var leftBindingExpr = ProcessFromClauseItem(applyClause.LeftExpr, sr, out var leftExprScopeEntries);

            //
            // Resolve right expression.
            //
            var rightBindingExpr = ProcessFromClauseItem(applyClause.RightExpr, sr, out var rightExprScopeEntries);
            ...
            Debug.Assert(applyBinding is not null, "applyBinding is not null");
            ...
            DbExpressionBinding source, Node predicate, ErrorContext errCtx, string bindingNameTemplate, SemanticResolver sr)
        {
            ...
            Debug.Assert(whereBinding is not null, "whereBinding is not null");
        ...
        private static DbExpressionBinding ProcessGroupByClause(DbExpressionBinding source, QueryExpr queryExpr, SemanticResolver sr)
        {
            ...
            //
            var groupKeysCount = groupByClause is not null ? groupByClause.GroupItems.Count : 0;
            ...
            if (!isImplicitGroup)
            {
                ...
                for (var i = 0; i < groupKeysCount; i++)
                {
                    ...
                    //
                    if (aliasedExpr.Alias is null)
                    {
                        var dotExpr = aliasedExpr.Expr as DotExpr;
                        if (null != dotExpr
                            && dotExpr.IsMultipartIdentifier(out var alternativeName))
                        {
            ...
            foreach (var groupKeyInfo in groupKeys)
            {
                ...
                if (groupKeyInfo.AlternativeName is not null)
                {
            ...
            if (groupPartitionRefFound)
            {
                ...
                if (projectionItems is not null)
                {
                    //
        ...
        private sealed class GroupKeyInfo
        {
            ...
            internal string[] AlternativeName
            {
                ...
                set
                {
                    Debug.Assert(_alternativeName is null, "GroupKeyInfo.AlternativeName can not be reset");
            ...
            DbExpressionBinding source, QueryExpr queryExpr, out bool queryProjectionProcessed, SemanticResolver sr)
        {
            ...
            if (queryExpr.OrderByClause is null)
            {
                return source;
            }
            ...
            if (orderByClause.SkipSubClause is not null)
            {
                //
            ...
            for (var i = 0; i < orderByClause.OrderByClauseItem.Count; i++)
            {
                ...
                if (orderClauseItem.Collation is not null)
                {
        ...
        private static Dictionary<Type, AstExprConverter> CreateAstExprConverters()
        {
            ...
            {
                { typeof(Literal), ConvertLiteral },
                { typeof(QueryParameter), ConvertParameter },
                { typeof(Identifier), ConvertIdentifier },
                { typeof(DotExpr), ConvertDotExpr },
                { typeof(BuiltInExpr), ConvertBuiltIn },
                { typeof(QueryExpr), ConvertQueryExpr },
                { typeof(ParenExpr), ConvertParenExpr },
                { typeof(RowConstructorExpr), ConvertRowConstructor },
                { typeof(MultisetConstructorExpr), ConvertMultisetConstructor },
                { typeof(CaseExpr), ConvertCaseExpr },
                { typeof(RelshipNavigationExpr), ConvertRelshipNavigationExpr },
                { typeof(RefExpr), ConvertRefExpr },
                { typeof(DerefExpr), ConvertDeRefExpr },
                { typeof(MethodExpr), ConvertMethodExpr },
                { typeof(CreateRefExpr), ConvertCreateRefExpr },
                { typeof(KeyExpr), ConvertKeyExpr },
                { typeof(GroupPartitionExpr), ConvertGroupPartitionExpr }
            };
                ...
                BuiltInKind.Plus, delegate(BuiltInExpr bltInExpr, SemanticResolver sr)
                    {
                        ...
                        else
                        {
                            ...
                            //
                            if (!sr.TypeResolver.TryGetFunctionFromMetadata("Edm", "Concat", out var function))
                            {
                            ...
                            {
                                args.Left.ResultType,
                                args.Right.ResultType
                            };
                            ...
                            return concatFunction.Invoke([args.Left, args.Right]);
                        }
                    });
                        ...
                        if (TypeSemantics.IsUnsignedNumericType(argument.ResultType))
                        {
                            if (
                                !TypeHelpers.TryGetClosestPromotableType(
                                    argument.ResultType, out var closestPromotableType))
                            {
                ...
                BuiltInKind.IsNull, delegate(BuiltInExpr bltInExpr, SemanticResolver sr)
                    {
                        ...
                        return isNullExpr is not null ? (DbExpression)isNullExpr.IsNull() : DbExpressionBuilder.True;
                    });
