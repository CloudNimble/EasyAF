...
namespace System.Data.Entity.Core.Common.EntitySql
{
    ...
    internal static class FunctionOverloadResolver
    {
            ...
            out bool isAmbiguous) where TFunctionMetadata : class
        {
            ...
            for (int i = 0, maxTotalRank = int.MinValue; i < functionsMetadata.Count; i++)
            {
                if (TryRankFunctionParameters(
                    ...
                    isStructurallyEqual,
                    isGroupAggregateFunction,
                    out var totalRank, out var rank))
                {
            ...
            if (bestCandidate is not null
                ...
                ranks.Count > 1)
            {
                Debug.Assert(bestCandidateRank is not null);
            ...
            Func<TypeUsage, TypeUsage, bool> isStructurallyEqual)
        {
            ...
            //
            Debug.Assert(fromType is not null, "fromType is not null");
            Debug.Assert(toType is not null, "toType is not null");
            ...
            if (primitiveFromType is not null
                && primitiveToType is not null)
            {
            ...
            if (entityBaseFromType is not null
                && entityBaseToType is not null)
            {
                var promotionIndex = 0;
                EdmType t;
                for (t = entityBaseFromType; t != entityBaseToType && t is not null; t = t.BaseType, ++promotionIndex)
                {
                    ;
                }
