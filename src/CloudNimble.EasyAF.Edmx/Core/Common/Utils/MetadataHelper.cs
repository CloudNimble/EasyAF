...
namespace System.Data.Entity.Core.Common.Utils
{
    ...
    internal static class MetadataHelper
    {
        ...
        internal static bool TryGetFunctionImportReturnType<T>(EdmFunction functionImport, int resultSetIndex, out T returnType)
            where T : EdmType
        {
            if (TryGetWrappedReturnEdmTypeFromFunctionImport(functionImport, resultSetIndex, out T resultType))
            {
        ...
        {
            if (!TryGetFunctionImportReturnType(functionImport, resultSetIndex, out EdmType expectedEdmType))
            {
        ...
        internal static void CheckFunctionImportReturnType<TElement>(EdmType expectedEdmType, MetadataWorkspace workspace)
        {
            ...
            if (!workspace.TryDetermineCSpaceModelType<TElement>(out var modelEdmType)
        ...
        internal static bool HasDiscreteDomain(EdmType edmType)
        {
            ...
            return primitiveType is not null && primitiveType.PrimitiveTypeKind == PrimitiveTypeKind.Boolean;
        }
        ...
        internal static bool IsComposable(EdmFunction function)
        {
            ...
            if (function.MetadataProperties.TryGetValue("IsComposableAttribute", false, out var isComposableProperty))
            {
        ...
        internal static IEnumerable<EntitySet> GetInfluencingEntitySetsForTable(EntitySet table, MetadataWorkspace workspace)
        {
            ...
            workspace.TryGetItemCollection(DataSpace.CSSpace, out var itemCollection);
        ...
        internal static IEnumerable<EdmType> GetTypeAndParentTypesOf(EdmType type, bool includeAbstractTypes)
        {
            ...
            while (specificType is not null)
            {
        ...
        internal static Dictionary<EntityType, Set<EntityType>> BuildUndirectedGraphOfTypes(EdmItemCollection edmItemCollection)
        {
            ...
            {
                if (childType.BaseType is null) //root type
                {
                    continue;
                }

                var parentType = childType.BaseType as EntityType;
                Debug.Assert(parentType is not null, "Parent type not Entity Type ??");
        ...
        internal static bool IsExtentAtSomeRelationshipEnd(AssociationSet relationshipSet, EntitySetBase extent)
        {
            ...
            {
                return GetSomeEndForEntitySet(relationshipSet, extent) is not null;
            }
            return false;
        }
        ...
        {
            if (typeUsage.Facets.TryGetValue(EdmProviderManifest.ConcurrencyModeFacetName, false, out var concurrencyFacet)
                && concurrencyFacet.Value is not null)
            {
        ...
        {
            if (member.TypeUsage.Facets.TryGetValue(EdmProviderManifest.StoreGeneratedPatternFacetName, false, out var storeGeneratedFacet)
                && storeGeneratedFacet.Value is not null)
            {
