...
namespace System.Data.Entity.Core.Common.Internal.Materialization
{
    ...
    internal abstract class Shaper
    {
        ...
        {
            Debug.Assert(context is null || workspace == context.MetadataWorkspace, "workspace must match context's workspace");
            ...
            IEntityWrapper wrappedEntity, EntityKey targetKey, AssociationEndMember targetMember)
        {
            ...
            if (null != (object)targetKey)
            {
                ...
                var associationSet = Context.MetadataWorkspace.MetadataOptimization.FindCSpaceAssociationSet(
                    ...
                    targetKey.EntitySetName, targetKey.EntityContainerName, out var targetEntitySet);
                Debug.Assert(associationSet is not null, "associationSet should not be null");
                ...
                if (
                    !ObjectStateManager.TryUpdateExistingRelationships(
                        Context, MergeOption, associationSet, sourceMember, sourceRelationships, wrappedEntity, targetMember, targetKey,
                         /*setIsLoaded*/ true, out var newEntryState))
                {
                    ...
                    switch (sourceMember.RelationshipMultiplicity)
                    {
                        ...
                        case RelationshipMultiplicity.One:
                            ...
                            // devnote: targetEntry can be a key entry (targetEntry.Entity is null), 
            ...
            {
                if (TryGetRelatedEnd(
                    wrappedEntity, (AssociationType)targetMember.DeclaringType, sourceMember.Name, targetMember.Name, out var relatedEnd))
                {
            ...
            out RelatedEnd relatedEnd)
        {
            ...
            if (sourceEnd is not null
                && targetEnd is not null)
            {
                var createRelatedEnd = false;
                if (wrappedEntity.EntityKey is null)
                {
                ...
                else
                {
                    ...
                    var associationSet = Workspace.MetadataOptimization.FindCSpaceAssociationSet(associationType, sourceEndName,
                        wrappedEntity.EntityKey.EntitySetName, wrappedEntity.EntityKey.EntityContainerName, out var entitySet);
        ...
        private void SetIsLoadedForSpan(RelatedEnd relatedEnd, bool forceToTrue)
        {
            ...
            if (!forceToTrue)
            {
                ...
                {
                    forceToTrue &= reference.EntityKey is null;
                }
            }
            ...
            object targetValue, IEntityWrapper wrappedSource, EntityKey sourceKey, AssociationEndMember targetMember)
        {
                    ...
                    MergeOption == MergeOption.OverwriteChanges))
            {
                ...
                if (!Context.Perspective.TryGetType(wrappedSource.IdentityType, out var entityTypeUsage)
        ...
        private void CheckClearedEntryOnSpan(EntityKey sourceKey, AssociationEndMember targetMember)
        {
            ...
            var associationSet = Context.MetadataWorkspace.MetadataOptimization.FindCSpaceAssociationSet(
                ...
                sourceKey.EntitySetName, sourceKey.EntityContainerName, out var sourceEntitySet);
            ...
            IEntityWrapper wrappedSource, IList<TTargetEntity> spannedEntities, AssociationEndMember targetMember)
        {
            ...
            if (wrappedSource.Entity is not null)
            {
                var sourceMember = MetadataHelper.GetOtherAssociationEnd(targetMember);

                if (TryGetRelatedEnd(
                    wrappedSource, (AssociationType)targetMember.DeclaringType, sourceMember.Name, targetMember.Name, out var relatedEnd))
                {
        ...
        internal abstract class ErrorHandlingValueReader<T>
        {
            ...
            private static T GetTypedValueDefault(DbDataReader reader, int ordinal)
            {
                    ...
                    && underlyingType.IsEnum())
                {
                    ...
                    return (T)methodInfo.Invoke(null, [reader, ordinal]);
                }

                // use the specific reader.GetXXX method
                var readerMethod = CodeGenEmitter.GetReaderMethod(typeof(T), out var isNullable);
                var result = (T)readerMethod.Invoke(reader, [ordinal]);
                return result;
            }
        ...
        {
            if (_materializedEntities is not null)
            {
        ...
        public void InitializeForOnMaterialize()
        {
            ...
            else if (_materializedEntities is not null)
            {
