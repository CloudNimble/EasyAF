    ...
    internal class ColumnMapKeyBuilder : ColumnMapVisitor<int>
    {
        ...
        internal void Append(string prefix, TypeUsage type)
        {
            if (null != type)
            {
                ...
                // here.
                if (InitializerMetadata.TryGetInitializerMetadata(type, out var initializer))
                {
        ...
        internal override void Visit(RefColumnMap columnMap, int dummy)
        {
            ...
            var isRefType = TypeHelpers.TryGetRefEntityType(columnMap.Type, out var referencedEntityType);
