...
namespace System.Data.Entity.Core.Common
{
    ...
    public abstract class DbXmlEnabledProviderManifest : DbProviderManifest
    {
        ...
        private readonly Dictionary<string, PrimitiveType> _storeTypeNameToEdmPrimitiveType = [];
        private readonly Dictionary<string, PrimitiveType> _storeTypeNameToStorePrimitiveType = [];
        ...
        private void Load(XmlReader reader)
        {
            var errors = SchemaManager.LoadProviderManifest(
                reader, reader.BaseURI.Length > 0 ? reader.BaseURI : null, true /*checkForSystemNamespace*/, out var schema);
            ...
            foreach (var schemaType in schema.SchemaTypes)
            {
                ...
                if (typeElement is not null)
                {
                    ...
                    if (EnumerableToReadOnlyCollection(typeElement.FacetDescriptions, out
        ...
        {
            if (typeDictionary.TryGetValue(type, out var collection))
            {
                return collection;
            }
            else
