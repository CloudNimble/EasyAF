...
namespace System.Data.Entity.Core.Objects.Internal
{
    ...
    internal class EntityProxyFactory
    {
        ...
        // </summary>
        private static readonly Dictionary<Type, EntityProxyTypeInfo> _proxyTypeMap = [];

        private static readonly Dictionary<Assembly, ModuleBuilder> _moduleBuilders = [];
        private static readonly ReaderWriterLockSlim _typeMapLock = new();

        // <summary>
        // The runtime assembly of the proxy types.
        // This is not the same as the AssemblyBuilder used to create proxy types.
        // </summary>
        private static readonly HashSet<Assembly> _proxyRuntimeAssemblies = [];
        ...
        private static ModuleBuilder GetDynamicModule(EntityType ospaceEntityType)
        {
            ...
            if (!_moduleBuilders.TryGetValue(assembly, out var moduleBuilder))
            {
                var assemblyName =
        ...
        internal static bool TryGetProxyWrapper(object instance, out IEntityWrapper wrapper)
        {
            ...
            if (IsProxyType(instance.GetType())
                &&
                TryGetProxyType(instance.GetType(), out var proxyTypeInfo))
            {
        ...
        internal static EntityProxyTypeInfo GetProxyType(ClrEntityType ospaceEntityType, MetadataWorkspace workspace)
        {
            ...
            if (TryGetProxyType(ospaceEntityType.ClrType, ospaceEntityType.CSpaceTypeName, out var proxyTypeInfo))
            {
                if (proxyTypeInfo is not null)
                {
            ...
            IEntityWrapper wrappedEntity, string relationshipName, out AssociationType associationType)
        {
            ...
            return (TryGetProxyType(wrappedEntity.Entity.GetType(), out var proxyInfo) && proxyInfo is not null &&
        ...
        internal static IEnumerable<AssociationType> TryGetAllAssociationTypesFromProxyInfo(IEntityWrapper wrappedEntity)
        {
            ...
            return TryGetProxyType(wrappedEntity.Entity.GetType(), out var proxyInfo)
        ...
        internal static void TryCreateProxyTypes(IEnumerable<EntityType> ospaceEntityTypes, MetadataWorkspace workspace)
        {
            ...
            try
            {
                ...
                {
                    Debug.Assert(ospaceEntityType is not null, "Null EntityType element reference present in enumeration.");
        ...
        private static EntityProxyTypeInfo TryCreateProxyType(EntityType ospaceEntityType, MetadataWorkspace workspace)
        {
            ...
            if (!_proxyNameMap.TryGetValue(proxyIdentity, out var proxyTypeInfo)
        ...
        internal static bool IsProxyType(Type type)
        {
            ...
            return type is not null && _proxyRuntimeAssemblies.Contains(type.Assembly());
        }
            ...
            return (entity) =>
                {
                    ...
                    if (IsProxyType(type))
                    {
                        if (TryGetBasePropertyValue(type, propertyName, entity, out var value))
                        {
                            return value;
                        }
                    }
        ...
        {
            value = null;
            if (TryGetProxyType(proxyType, out var typeInfo)
        ...
        private static void InterceptMember(EdmMember member, Type proxyType, EntityProxyTypeInfo proxyTypeInfo)
        {
            ...
            var interceptorDelegate = GetInterceptorDelegateMethod.
                                          ...
                                          Invoke(null, [member, proxyTypeInfo.EntityWrapperDelegate]) as Delegate;
        ...
        private static void SetResetFKSetterFlagDelegate(Type proxyType, EntityProxyTypeInfo proxyTypeInfo)
        {
            ...
            Debug.Assert(resetFKSetterFlagField is not null, "Expected resetFKSetterFlagField to be defined on the proxy type.");
        ...
        private static Action<object> GetResetFKSetterFlagDelegate(Func<object, object> getEntityWrapperDelegate)
        {
            return (proxy) =>
                {
                    Debug.Assert(getEntityWrapperDelegate is not null, "entityWrapperDelegate must not be null");
        ...
        private static void SetCompareByteArraysDelegate(Type proxyType)
        {
            ...
            Debug.Assert(compareByteArraysField is not null, "Expected compareByteArraysField to be defined on the proxy type.");
        ...
        private static bool CanProxyType(EntityType ospaceEntityType)
        {
            ...
            return ctor is not null && (((ctor.Attributes & MethodAttributes.MemberAccessMask) == MethodAttributes.Public) ||
        ...
        internal class ProxyTypeBuilder
        {
            ...
            private readonly List<FieldBuilder> _serializedFields = new(3);
            ...
            private static void EmitBaseSetter(TypeBuilder typeBuilder, PropertyBuilder propertyBuilder, PropertyInfo baseProperty)
            {
                if (CanProxySetter(baseProperty))
                {
                    ...
                    var setterBuilder = typeBuilder.DefineMethod(
                        "set_" + baseProperty.Name, methodAccess | methodAttributes, null, [baseProperty.PropertyType]);
            ...
            private static readonly Lazy<ConstructorInfo> _scriptIgnoreAttributeConstructor =
                new(TryGetScriptIgnoreAttributeConstructor);
            ...
            private static ConstructorInfo TryGetScriptIgnoreAttributeConstructor()
            {
                try
                {
                    if (AspProxy.IsSystemWebLoaded())
                    {
                        ...
                        if (scriptIgnoreAttributeType is not null)
                        {
            ...
            public static void MarkAsNotSerializable(FieldBuilder field)
            {
                ...
                if (field.IsPublic)
                {
                    ...
                    if (_scriptIgnoreAttributeConstructor.Value is not null)
                    {
