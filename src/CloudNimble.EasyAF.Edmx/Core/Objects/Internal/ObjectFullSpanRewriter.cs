...
namespace System.Data.Entity.Core.Objects.Internal
{
    internal class ObjectFullSpanRewriter : ObjectSpanRewriter
    {
        ...
        // </summary>
        private readonly Stack<SpanPathInfo> _currentSpanPath = new();
            ...
            : base(tree, toRewrite, aliasGenerator)
        {
            ...
            if (!TryGetEntityType(Query.ResultType, out var entityType))
            {
        ...
        private void ConvertSpanPath(SpanPathInfo parentInfo, List<string> navPropNames, int pos)
        {
            ...
            if (!parentInfo.DeclaringType.NavigationProperties.TryGetValue(navPropNames[pos], true, out var nextNavProp))
            {
            ...
            if (!parentInfo.Children.TryGetValue(nextNavProp, out var nextChild))
            {
        ...
        {
            TryGetEntityType(navProp.TypeUsage, out var retType);
            // Currently, navigation properties may only return an Entity or Collection<Entity> result
            Debug.Assert(retType is not null, "Navigation property has non-Entity and non-Entity collection result type?");
            return retType;
        }
        ...
        internal override SpanTrackingInfo CreateEntitySpanTrackingInfo(DbExpression expression, EntityType entityType)
        {
            ...
            if (currentInfo.Children is not null)
            {
