...
namespace System.Data.Entity.Core.Objects.Internal
{
    ...
    internal class ObjectSpanRewriter
    {
        ...
        private readonly Stack<NavigationInfo> _navSources = new();
            ...
            out SpanIndex spanInfo)
        {
            ...
            if (rewriter is not null)
            {
                ...
                if (newQuery is not null)
                {
                    Debug.Assert(
                        rewriter.SpanIndex is not null || tree.Query.ResultType.EdmEquals(newQuery.ResultType),
                        "Query was rewritten for Span but no SpanIndex was created?");
                    spanInfo = rewriter.SpanIndex;
                }
            }

            return (spanInfo is not null);
        }
        ...
        private DbExpression RewriteEntity(DbExpression expression, EntityType entityType)
        {
            ...
            if (relationshipSpans is not null)
            {
                ...
                foreach (var relSpan in relationshipSpans)
                {
                        ...
                        !tracking.FullSpannedEnds.ContainsKey(relSpan.Value))
                    {
                        ...
                        if (!TryGetNavigationSource(relSpan.Value, out var columnDef))
                        {
        ...
        private DbExpression RewriteRow(DbExpression expression, RowType rowType)
        {
            ...
            else
            {
                ...
                {
                    var columnProp = rowType.Properties[idx];
                    if (!spannedColumns.TryGetValue(idx, out var columnDef))
                    {
                ...
                if (newRow is null)
                {
                    ...
                    rewritten = DbExpressionBuilder.Case(
                        new List<DbExpression>([condition]),
                        new List<DbExpression>([nullExpression]),
                        rewritten);
                }
        ...
        private class RelationshipNavigationVisitor : DefaultExpressionVisitor
        {
                ...
                DbExpression expression, AliasGenerator aliasGenerator, out NavigationInfo navInfo)
            {
                ...
                {
                    Debug.Assert(
                        visitor._original is not null && visitor._rewritten is not null, "Expression was rewritten but no navigation was found?");
