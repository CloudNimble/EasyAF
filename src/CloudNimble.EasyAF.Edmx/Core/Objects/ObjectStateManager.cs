...
namespace System.Data.Entity.Core.Objects
{
    ...
    public class ObjectStateManager : IEntityStateManager
    {
        ...
        internal virtual void OnObjectStateManagerChanged(CollectionChangeAction action, object entity)
        {
            ...
            if (onObjectStateManagerChangedDelegate is not null)
            {
        ...
        {
            Debug.Assert(!(entity is IEntityWrapper), "Object is an IEntityWrapper instance instead of the raw entity.");
            if (onEntityDeletedDelegate is not null)
            {
            ...
            IEntityWrapper wrappedObject, EntityKey passedKey, EntitySet entitySet, string argumentName, bool isAdded)
        {
...
#if DEBUG
            if ((object)dataObjectEntityKey is not null
                && (object)entityKey is not null)
            {
            ...
            if ((object)entityKey is not null
            ...
            if ((isAdded)
                &&
                ((dataObjectEntityKey is null && (null != (existingEntry = FindEntityEntry(wrappedObject.Entity)))) ||
                 (dataObjectEntityKey is not null && (null != (existingEntry = FindEntityEntry(dataObjectEntityKey))))))
            {
        ...
        {
            if (!_danglingForeignKeys.TryGetValue(foreignKey, out var danglingEntries))
            {
                danglingEntries = [];
                _danglingForeignKeys.Add(foreignKey, danglingEntries);
            }
            Debug.Assert(entry.ObjectStateManager is not null, "Attempt to add detached state entry to dangling keys");
        ...
        internal virtual void AssertEntryDoesNotExistInForeignKeyIndex(EntityEntry entry)
        {
            foreach (var dFkEntry in _danglingForeignKeys.SelectMany(kv => kv.Value))
            {
                ...
                {
                    Debug.Assert(
                        dFkEntry.Item2.EntityKey is null || entry.EntityKey is null ||
        ...
        internal virtual void AssertAllForeignKeyIndexEntriesAreValid()
        {
            ...
            {
                Debug.Assert(entry.Item2._cache is not null, "found an entry in the _danglingForeignKeys collection that has been nulled out");
                Debug.Assert(
                    validEntries.Contains(entry.Item2),
                    "The entry in the dangling foreign key store is no longer in the ObjectStateManager. Key="
                    +
                    (entry.Item2.State == EntityState.Detached ? "detached" : entry.Item2.EntityKey is not null ? "null" : entry.Item2.EntityKey.ConcatKeyValue()));
                Debug.Assert(
        ...
        {
            if (_danglingForeignKeys.TryGetValue(foreignKey, out var foundEntries))
            {
        ...
        {
            return _entriesWithConceptualNulls is not null && _entriesWithConceptualNulls.Count != 0;
        }
        ...
        {
            return _entriesWithConceptualNulls is not null && _entriesWithConceptualNulls.Contains(entry);
        }
        ...
        {
            if (!entry.IsKeyEntry
                && _entriesWithConceptualNulls is not null
            ...
            bool replacingEntry)
        {
            ...
            if (contextToAttach is not null)
            {
        ...
        internal virtual void TrackPromotedRelationship(RelatedEnd relatedEnd, IEntityWrapper wrappedEntity)
        {
            ...
            DebugCheck.NotNull(wrappedEntity);
            Debug.Assert(wrappedEntity.Entity is not null);
            Debug.Assert(
            ...
            if (!TransactionManager.PromotedRelationships.TryGetValue(relatedEnd, out var entities))
            {
                entities = [];
            ...
            bool inKeyEntryPromotion)
        {
                ...
                != RelationshipMultiplicity.Many)
            {
                ...
                switch (mergeOption)
                {
                    ...
                    case MergeOption.OverwriteChanges:
                        ...
                        if (currentWrappedTarget is not null
                            && currentWrappedTarget.Entity is not null
                            && currentWrappedTarget != wrappedTarget)
                        {
                            ...
                            Debug.Assert(
                                relationshipEntry is not null || relatedEnd.IsForeignKey,
                                "Could not find relationship entry for LAT relationship.");

                            relatedEnd.RemoveAll();

                            if (relationshipEntry is not null)
                            {
                                Debug.Assert(relationshipEntry is not null, "Could not find relationship entry.");
            ...
            Debug.Assert(
                !(inKeyEntryPromotion && wrappedSource.Context is null),
                        ...
                        else
                        {
                            ...
                            if (
                                !TryUpdateExistingRelationships(
                                    ...
                                    setIsLoaded, out var newEntryState))
                            {
            ...
            out EntityState newEntryState)
        {
            ...
            if (entriesToDetach is not null)
            {
            ...
            if (entriesToUpdate is not null)
            {
            ...
            ObjectStateManager cache, RelationshipEntry relationshipEntry, EntityKey sourceEntityKey, string targetRoleName)
        {
            ...
            if (entry.WrappedEntity.Entity is not null)
            {
                var reference =
        ...
        private void ValidateKeylessEntityStore()
        {
            ...
            if (_keylessEntityStore is not null
                && _keylessEntityStore.Count == stores.Sum(s => s is null ? 0 : s.Count))
            {
                return;
            }
                ...
                if (null != store)
                {
                    foreach (var entry in store.Values)
                    {
                            ...
                            !(entry.Entity is IEntityWithKey))
                        {
                            ...
                            if (_keylessEntityStore.TryGetValue(entry.Entity, out var keylessEntry))
                            {
        ...
        {
            return (_addedRelationshipStore is not null && _addedRelationshipStore.Count > 0)
                || (_addedEntityStore is not null && _addedEntityStore.Count > 0)
                || (_modifiedEntityStore is not null && _modifiedEntityStore.Count > 0)
                || (_deletedRelationshipStore is not null && _deletedRelationshipStore.Count > 0)
                || (_deletedEntityStore is not null && _deletedEntityStore.Count > 0);
        }
        ...
        {
            var size = 0;
            if (_addedEntityStore is not null)
            {
                size += _addedEntityStore.Count;
            }
            if (_modifiedEntityStore is not null)
            {
                size += _modifiedEntityStore.Count;
            }
            if (_deletedEntityStore is not null)
            {
                size += _deletedEntityStore.Count;
            }
            if (_unchangedEntityStore is not null)
            {
        ...
        internal virtual void FixupKey(EntityEntry entry)
        {
            ...
            Debug.Assert((object)oldKey is not null, "Cannot fixup a cache entry with a null key.");
        ...
        internal virtual void ReplaceKeyWithTemporaryKey(EntityEntry entry)
        {
            ...
            Debug.Assert(FindEntityEntry(newKey) is null, "no entry should exist with the new temporary key");
            ...
            EntityState relationshipState) where TEntity : class
        {
            ...
            var navigationProperty = ObjectContext.ParsePropertySelectorExpression(navigationPropertySelector, out var removedConvert);
            ...
            EntityEntry targetEntry, RelatedEnd relatedEnd, RelationshipWrapper relationshipWrapper, EntityState requestedState)
        {
            ...
            switch (requestedState)
            {
                case EntityState.Added:
                    ...
                    Debug.Assert(relationshipEntry is not null, "null relationshipEntry");
                    break;
                case EntityState.Unchanged:
                    relatedEnd.Add(
                        targetEntry.WrappedEntity,
        ...
        bool IEntityStateManager.TryGetEntityStateEntry(EntityKey key, out IEntityStateEntry entry)
        {
            ...
            var result = TryGetObjectStateEntry(key, out var objectStateEntry);
        ...
        {
            if (!TryGetEntityEntry(dependentKey, out var dependentEntry))
            {
                principalKey = null;
                return false;
            }
        ...
        {
            bool result;
            result = TryGetEntityEntry(key, out var entityEntry);
            entry = entityEntry;
            return result;
        }
        ...
        {
            if (!TryGetRelationshipManager(entity, out var rm))
            {
        ...
        public virtual bool TryGetRelationshipManager(object entity, out RelationshipManager relationshipManager)
        {
            ...
            if (withRelationships is not null)
            {
            ...
            else
            {
                ...
                if (wrappedEntity.Context is null)
                {
        ...
        internal virtual void ChangeState(EntityEntry entry, EntityState oldState, EntityState newState)
        {
            ...
            if (newState == EntityState.Detached)
            {
                ...
                Debug.Assert((object)entry.EntityKey is not null, "attached entry must have a key");
                ...
                if (fireEvent
                    && wrappedEntity.Entity is not null
        ...
        private void AddRelationshipEntryToDictionary(RelationshipEntry entry, EntityState state)
        {
            ...
            Debug.Assert(dictionaryToAdd is not null, "Couldn't find the correct relationship dictionary based on entity state.");
        ...
        private void AddEntityEntryToDictionary(EntityEntry entry, EntityState state)
        {
            ...
            Debug.Assert(dictionaryToAdd is not null, "Couldn't find the correct entity dictionary based on entity state.");
        ...
        private void RemoveObjectStateEntryFromDictionary(RelationshipEntry entry, EntityState state)
        {
            ...
            Debug.Assert(dictionaryContainingEntry is not null, "Couldn't find the correct relationship dictionary based on entity state.");
        ...
        private void RemoveObjectStateEntryFromDictionary(EntityEntry entry, EntityState state)
        {
            ...
            Debug.Assert(dictionaryContainingEntry is not null, "Couldn't find the correct entity dictionary based on entity state.");
        ...
        internal virtual StateManagerTypeMetadata GetOrAddStateManagerTypeMetadata(Type entityType, EntitySet entitySet)
        {
            ...
            if (!_metadataMapping.TryGetValue(new EntitySetQualifiedType(entityType, entitySet), out var typeMetadata))
            {
        ...
        internal virtual StateManagerTypeMetadata GetOrAddStateManagerTypeMetadata(EdmType edmType)
        {
            ...
            if (!_metadataStore.TryGetValue(edmType, out var typeMetadata))
            {
                typeMetadata = AddStateManagerTypeMetadata(
        ...
        private void DetectConflicts(IList<EntityEntry> entries)
        {
            ...
            foreach (var entry in entries)
            {
                ...
                tm.AddedRelationshipsByGraph.TryGetValue(entry.WrappedEntity, out var addedRelationshipsByGraph);
                tm.AddedRelationshipsByForeignKey.TryGetValue(entry.WrappedEntity, out var addedRelationshipsByForeignKey);

                //Ensure new graph relationships do not involve a Deleted Entity
                if (addedRelationshipsByGraph is not null
                ...
                if (addedRelationshipsByForeignKey is not null)
                {
                            ...
                            else if (pair.Value.Count == 1)
                            {
                                ...
                                else
                                {
                                    ...
                                    if (tm.AddedRelationshipsByPrincipalKey.TryGetValue(
                                        entry.WrappedEntity, out var addedRelationshipsByPrincipalKey))
                                    {
                                        addedRelationshipsByPrincipalKey.TryGetValue(pair.Key, out newFks);
                                    }
                                }

                                if (newFks is not null
                                    && newFks.Count > 0)
                                {
                                ...
                                else
                                {
                                    ...
                                    if (tm.DeletedRelationshipsByForeignKey.TryGetValue(
                                        entry.WrappedEntity, out var deletedRelationshipsByForeignKey))
                                    {
                                        if (deletedRelationshipsByForeignKey.TryGetValue(pair.Key, out var removedKeys))
                                        {
        ...
        internal virtual EntityKey GetPermanentKey(IEntityWrapper entityFrom, RelatedEnd relatedEndFrom, IEntityWrapper entityTo)
        {
            ...
            if (entityTo.ObjectStateEntry is not null)
            {
            ...
            foreach (var entry in entries)
            {
                    ...
                    !entry.IsKeyEntry) // Still need to check this here because entries may have been demoted
                {
                    foreach (var relatedEnd in entry.WrappedEntity.RelationshipManager.Relationships)
                    {
                        ...
                        if (tm.AddedRelationshipsByGraph.TryGetValue(entry.WrappedEntity, out var addedRelationshipsByGraph))
                        {
                        ...
                        if (entityKeysOfAddedObjects is not null)
                        {

                            foreach (var entityKeyOfAddedObjects in entityKeysOfAddedObjects)
                            {
                                // we are interested only in tracked non-Added entities
                                if (TryGetEntityEntry(entityKeyOfAddedObjects, out var relatedEntry)
                                    &&
                                    relatedEntry.WrappedEntity.Entity is not null)
                                {
                                    entitiesToAdd = entitiesToAdd is not null ? entitiesToAdd : [];
            ...
            foreach (var entry in entries)
            {
                    ...
                    !entry.IsKeyEntry) // Still need to check this here because entries may have been demoted
                {
                    foreach (var relatedEnd in entry.WrappedEntity.RelationshipManager.Relationships)
                    {
                        ...
                        if (reference is not null
                            &&
                            tm.DeletedRelationshipsByForeignKey.TryGetValue(entry.WrappedEntity, out var deletedRelationshipsByForeignKey))
                        {
                        ...
                        if (tm.DeletedRelationshipsByGraph.TryGetValue(entry.WrappedEntity, out var deletedRelationshipsByGraph))
                        {
                        ...
                        if (entityKeysOfDeletedObjects is not null)
                        {
                            foreach (var key in entityKeysOfDeletedObjects)
                            {
                                IEntityWrapper relatedEntity = null;
                                if (TryGetEntityEntry(key, out var relatedEntry)
                                    &&
                                    relatedEntry.WrappedEntity.Entity is not null)
                                {
                                ...
                                {
                                    entitiesToDelete = entitiesToDelete is not null ? entitiesToDelete : [];
                        ...
                        if (entitiesToDelete is not null)
                        {
            ...
            HashSet<IEntityWrapper> entitiesToDelete)
        {
            ...
            if (relatedEnd.IsForeignKey)
            {
                ...
                if (otherEnd.IsDependentEndOfReferentialConstraint(false))
                {
                    ...
                    if (TransactionManager.DeletedRelationshipsByForeignKey.TryGetValue(relatedEntity, out var deletedRelationshipsByForeignKey)
                        &&
                        deletedRelationshipsByForeignKey.TryGetValue(otherEnd, out var entityKeysOfDeletedObjects)
                        &&
                        entityKeysOfDeletedObjects.Count > 0
                        &&
                        TransactionManager.DeletedRelationshipsByGraph.TryGetValue(relatedEntity, out var deletedRelationshipsByGraph)
                        &&
        ...
        private bool HasAddedReference(IEntityWrapper wrappedOwner, EntityReference reference)
        {
            if (reference is not null
                &&
                TransactionManager.AddedRelationshipsByGraph.TryGetValue(wrappedOwner, out var addedRelationshipsByGraph)
                &&
                addedRelationshipsByGraph.TryGetValue(reference, out var entitiesToAdd)
                &&
        ...
        private bool IsReparentingReference(IEntityWrapper wrappedEntity, EntityReference reference)
        {
            ...
            if (reference.IsPrincipalEndOfReferentialConstraint())
            {
                ...
                reference = wrappedEntity.Entity is null
                                ? null
                                : reference.GetOtherEndOfRelationship(wrappedEntity) as EntityReference;
            }

            if (wrappedEntity.Entity is not null
                && reference is not null)
            {
                if (tm.AddedRelationshipsByForeignKey.TryGetValue(wrappedEntity, out var addedRelationshipsByForeignKey)
                    &&
                    addedRelationshipsByForeignKey.TryGetValue(reference, out var entityKeysOfAddedObjects)
                    &&
                ...
                if (tm.AddedRelationshipsByGraph.TryGetValue(wrappedEntity, out var addedRelationshipsByGraph)
                    &&
                    addedRelationshipsByGraph.TryGetValue(reference, out var entitiesToAdd)
                    &&
                    entitiesToAdd.Count > 0)
