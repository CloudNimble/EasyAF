    ...
    internal sealed partial class ExpressionConverter
    {
            ...
            : TypedTranslator<ConstantExpression>
        {
            ...
            protected override DbExpression TypedTranslate(ExpressionConverter parent, ConstantExpression linq)
            {
                ...
                {
                    Debug.Assert(linq.Value is not null, "null enum constants should have alredy been taken care of");

                    linqType = linq.Value.GetType();
                }

                if (parent.TryGetValueLayerType(linqType, out var type))
                {
            ...
            : TypedTranslator<MemberExpression>
        {
            ...
            {
                var memberInfo = TypeSystem.PropertyOrField(linq.Member, out var memberName, out var memberType);

                // note: we check for "regular" properties last, since the other two flavors derive
                // from this one
                if (linq.Expression is not null)
                {
                    ...
                    if (TryResolveAsProperty(
                        parent, memberInfo,
                        instance.ResultType, instance, out var propertyExpression))
                    {
                ...
                if (memberInfo.MemberType == MemberTypes.Property)
                {
                    ...
                    if (TryGetTranslator((PropertyInfo)memberInfo, out var propertyTranslator))
                    {
            ...
            private static bool TryGetTranslator(PropertyInfo propertyInfo, out PropertyTranslator propertyTranslator)
            {
                ...
                if (_propertyTranslators.TryGetValue(propertyInfo, out var translatorInstance))
                {
                ...
                MemberInfo clrMember, TypeUsage definingType, DbExpression instance, out DbExpression propertyExpression)
            {
                ...
                if (null != rowType)
                {
                    if (rowType.Members.TryGetValue(name, false, out var member))
                    {
                ...
                {
                    if (parent._perspective.TryGetMember(structuralType, name, false, out var member))
                    {
                        if (null != member)
                        {
                ...
                if (name == KeyColumnName)
                {
                        ...
                        == instance.ExpressionKind)
                    {
                        ...
                        if (property.Property.Name == GroupColumnName
                            ...
                            InitializerMetadata.TryGetInitializerMetadata(property.Instance.ResultType, out var initializerMetadata)
                            &&
            ...
            private static DbExpression TranslateCount(ExpressionConverter parent, Type sequenceElementType, Expression sequence)
            {
                ...
                ReflectionUtil.TryLookupMethod(SequenceMethod.Count, out var countMethod);
            ...
            {
                return [new VBDateAndTimeNowTranslator(vbAssembly)];
            }
            ...
            internal static bool CanFuncletizePropertyInfo(PropertyInfo propertyInfo)
            {
                ...
                return GenericICollectionTranslator.TryGetPropertyTranslator(propertyInfo, out var propertyTranslator) ||
            ...
            private sealed class GenericICollectionTranslator : PropertyTranslator
            {
                ...
                {
                    if (IsICollection(type, out var collectionElementType))
                    {
            ...
            {
                private static readonly Dictionary<PropertyInfo, string> _propertyRenameMap = new(2);
            ...
            : TypedTranslator<NewExpression>
        {
            ...
            protected override DbExpression TypedTranslate(ExpressionConverter parent, NewExpression linq)
            {
                ...
                {
                    TypeSystem.PropertyOrField(linq.Members[i], out var memberName, out var memberType);
            ...
            : TypedTranslator<ListInitExpression>
        {
            ...
            protected override DbExpression TypedTranslate(ExpressionConverter parent, ListInitExpression linq)
            {
                ...
                if ((linq.NewExpression.Constructor is not null)
            ...
            : TypedTranslator<MemberInitExpression>
        {
            ...
            protected override DbExpression TypedTranslate(ExpressionConverter parent, MemberInitExpression linq)
            {
                ...
                for (var i = 0; i < linq.Bindings.Count; i++)
                {
                    ...
                    var memberInfo = TypeSystem.PropertyOrField(binding.Member, out var memberName, out var memberType);
