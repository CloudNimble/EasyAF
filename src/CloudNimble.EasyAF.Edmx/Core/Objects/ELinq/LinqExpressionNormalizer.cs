...
namespace System.Data.Entity.Core.Objects.ELinq
{
    ...
    internal class LinqExpressionNormalizer : EntityExpressionVisitor
    {
        ...
        internal override Expression VisitBinary(BinaryExpression b)
        {
            ...
            // NORMALIZED: Equal(x, y)
            if (_patterns.TryGetValue(b.Left, out var pattern)
                && pattern.Kind == PatternKind.Compare
                && IsConstantZero(b.Right))
            {
                var comparePattern = (ComparePattern)pattern;
                // handle relational operators
                if (TryCreateRelationalOperator(b.NodeType, comparePattern.Left, comparePattern.Right, out var relationalExpression))
                {
            ...
            else
            {
                    ...
                    && m.Arguments.Count == 1)
                {
                        ...
                        && declaringType.GetGenericTypeDefinition() == typeof(List<>))
                    {
                        ...
                        if (ReflectionUtil.TryLookupMethod(SequenceMethod.Contains, out var containsMethod))
                        {
        ...
        private static MethodCallExpression NormalizePredicateArgument(MethodCallExpression callExpression)
        {
            ...
            if (HasPredicateArgument(callExpression, out var argumentOrdinal)
                &&
                TryMatchCoalescePattern(callExpression.Arguments[argumentOrdinal], out var normalizedArgument))
            {
                var normalizedArguments = new List<Expression>(callExpression.Arguments)
                {
                    ...
                    [argumentOrdinal] = normalizedArgument
                };
        ...
        private static bool HasPredicateArgument(MethodCallExpression callExpression, out int argumentOrdinal)
        {
            ...
            if (2 <= callExpression.Arguments.Count
                &&
                ReflectionUtil.TryIdentifySequenceMethod(callExpression.Method, out var sequenceMethod))
            {
        ...
        {
            if (!TryCreateRelationalOperator(op, left, right, out var result))
            {
