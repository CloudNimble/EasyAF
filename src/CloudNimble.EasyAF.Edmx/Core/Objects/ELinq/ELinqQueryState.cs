...
namespace System.Data.Entity.Core.Objects.ELinq
{
    ...
    internal class ELinqQueryState : ObjectQueryState
    {
        ...
        {
            Debug.Assert(Span is null, "Include span specified on compiled LINQ-based ObjectQuery instead of within the expression tree?");
            ...
            if (plan is null)
            {
                ...
                if (_linqParameters is not null
                    && _linqParameters.Any())
                {
                ...
                if (PlanCachingEnabled && !_recompileRequired())
                {
                    ...
                    if (ExpressionKeyGen.TryGenerateKey(queryExpression, out var expressionKey))
                    {
                        ...
                        if (cacheManager.TryCacheLookup(cacheKey, out ObjectQueryExecutionPlan executionPlan))
                        {
                            plan = executionPlan;
                        }
                    }
                ...
                if (plan is null)
                {
                    ...
                    if (cacheKey is not null)
                    {
                        ...
                        if (cacheManager.TryLookupAndAdd(newEntry, out var foundEntry))
                        {
        ...
        internal override ObjectQueryState Include<TElementType>(ObjectQuery<TElementType> sourceQuery, string includePath)
        {
            ...
            Debug.Assert(includeMethod is not null, "Unable to find ObjectQuery.Include method?");

            Expression includeCall = Expression.Call(
                Expression.Constant(sourceQuery), includeMethod, [Expression.Constant(includePath, typeof(string))]);
