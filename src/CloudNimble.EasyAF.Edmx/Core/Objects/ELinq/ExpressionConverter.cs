...
namespace System.Data.Entity.Core.Objects.ELinq
{
    ...
    internal sealed partial class ExpressionConverter
    {
        ...
        private readonly AliasGenerator _aliasGenerator = new("LQ", 0);
        ...
        internal void ValidateInitializerMetadata(InitializerMetadata metadata)
        {
            ...
            if (_initializers is not null
                && _initializers.TryGetValue(metadata.ClrType, out var existingMetadata))
            {
        ...
        private DbExpression AddSpanMapping(DbExpression expression, Span span)
        {
                ...
                && CanIncludeSpanInfo())
            {
                ...
                if (_spanMappings.TryGetValue(expression, out var storedSpan))
                {
        ...
        {
            if (TryGetSpan(from, out var argumentSpan))
            {
        ...
        private void UnifySpanMappings(DbExpression left, DbExpression right, DbExpression to)
        {
            ...
            var hasLeftSpan = TryGetSpan(left, out var leftSpan);
            var hasRightSpan = TryGetSpan(right, out var rightSpan);
            if (!hasLeftSpan
                && !hasRightSpan)
            {
                return;
            ...
            Debug.Assert(leftSpan is not null || rightSpan is not null, "Span mappings contain null?");
        ...
        private TypeUsage GetCastTargetType(TypeUsage fromType, Type toClrType, Type fromClrType, bool preserveCastForDateTime)
        {
            ...
            if (TryGetValueLayerType(toClrType, out var toType)
        ...
        private DbExpression TranslateExpression(Expression linq)
        {
            ...
            if (!_bindingContext.TryGetBoundExpression(linq, out var result))
            {
                // translate to a CQT expression
                if (_translators.TryGetValue(linq.NodeType, out var translator))
                {
        ...
        private void CheckInitializerType(Type type)
        {
            ...
            if (_funcletizer.RootContext.Perspective.TryGetType(type, out var typeUsage))
            {
        ...
        private bool TryGetValueLayerType(Type linqType, out TypeUsage type)
        {
            ...
            if (ClrProviderManifest.TryGetPrimitiveTypeKind(nonNullableType, out var primitiveTypeKind))
            {
            ...
            {
                if (TryGetValueLayerType(elementType, out var elementTypeUsage))
                {
        ...
        private static void VerifyTypeSupportedForComparison(Type clrType, TypeUsage edmType, Stack<EdmMember> memberPath, bool isNullComparison)
        {
            ...
            switch (edmType.EdmType.BuiltInTypeKind)
            {
                ...
                case BuiltInTypeKind.RowType:
                    {
                        if (!InitializerMetadata.TryGetInitializerMetadata(edmType, out var initializerMetadata)
                            ||
            ...
            DbExpression left, DbExpression right, EqualsPattern pattern, Type leftClrType, Type rightClrType)
        {
            ...
            {
                if (!TypeSemantics.TryGetCommonType(leftType, rightType, out var commonType))
                {
        ...
        {
            var providerSupportsEscapingLikeArgument = ProviderManifest.SupportsEscapingLikeArgument(out var dummyEscapeChar);
            ...
            if (!providerSupportsEscapingLikeArgument && (escapeExpression is not null))
            {
            ...
            var translatedEscapeExpression = (escapeExpression is not null ? TranslateExpression(escapeExpression) : null);
            var translatedInputExpression = TranslateExpression(inputExpression);

            return escapeExpression is not null ?
        ...
        {
            var providerSupportsEscapingLikeArgument = ProviderManifest.SupportsEscapingLikeArgument(out var escapeChar);
            ...
            if (providerSupportsEscapingLikeArgument && (queryParameterExpression is not null))
            {
            ...
            if (providerSupportsEscapingLikeArgument && translatedPatternExpression.ExpressionKind == DbExpressionKind.Constant)
            {
                ...
                Debug.Assert(preparedPattern.Item1 is not null, "The prepared value should not be null when the input is non-null");
        ...
        private static Tuple<string, bool> PreparePattern(string patternValue, bool insertPercentAtStart, bool insertPercentAtEnd, DbProviderManifest providerManifest)
        {
            ...
            if (escapedPatternValue is null)
            {
            ...
            string namespaceName, string functionName, IList<TypeUsage> argumentTypes, bool isGroupAggregateFunction, Expression Expression)
        {
            ...
            var function = FunctionOverloadResolver.ResolveFunctionOverloads(
                candidateFunctions, argumentTypes, isGroupAggregateFunction, out var isAmbiguous);
        ...
        private static void ThrowUnresolvableFunction(Expression Expression)
        {
            ...
            {
                var memberInfo = TypeSystem.PropertyOrField(((MemberExpression)Expression).Member, out var memberName, out var memberType);
