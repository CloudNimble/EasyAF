    ...
    internal sealed class QueryParameterExpression : Expression
    {
        ...
        private static bool TryEvaluatePath(Expression expression, out ConstantExpression constantExpression)
        {
            ...
            if (me is not null)
            {
                ...
                stack.Push(me);
                while ((me = me.Expression as MemberExpression) is not null)
                {
                    stack.Push(me);
                }
                me = stack.Pop();
                ...
                if (ce is not null)
                {
                    if (!TryGetFieldOrPropertyValue(me, ((ConstantExpression)me.Expression).Value, out var memberVal))
                    {
                        return false;
                    }
                    if (stack.Count > 0)
