    ...
    internal sealed partial class ExpressionConverter
    {
        ...
        internal sealed partial class MethodCallTranslator : TypedTranslator<MethodCallExpression>
        {
            ...
            protected override CqtExpression TypedTranslate(ExpressionConverter parent, MethodCallExpression linq)
            {
                ...
                if (ReflectionUtil.TryIdentifySequenceMethod(linq.Method, out var sequenceMethod)
                    &&
                    _sequenceTranslators.TryGetValue(sequenceMethod, out var sequenceTranslator))
                {
                ...
                {
                    if (_objectQueryTranslators.TryGetValue(linq.Method.Name, out var builderTranslator))
                    {
                    ...
                    case "Contains":
                        {
                            ...
                            {
                                if (linq.Method.IsImplementationOfGenericInterfaceMethod(typeof(ICollection<>), out var genericArguments))
                                {
            ...
            private static readonly FunctionCallTranslator _functionCallTranslator = new();
            ...
            private static readonly object _vbInitializerLock = new();
            ...
            private sealed class ObjectQueryIncludeTranslator : ObjectQueryCallTranslator
            {
                ...
                internal override CqtExpression Translate(ExpressionConverter parent, MethodCallExpression call)
                {
                    ...
                    Debug.Assert(
                        call.Arguments.Count == 1 && call.Arguments[0] is not null
                    ...
                    if (!parent.TryGetSpan(queryExpression, out var span))
                    {
                        span = null;
                    }
            ...
            internal sealed class CanonicalFunctionDefaultTranslator : CallTranslator
            {
                ...
                internal override CqtExpression Translate(ExpressionConverter parent, MethodCallExpression call)
                {
                    ...
                    {
                        Debug.Assert(call.Object is not null, "Instance method without this");
...
#pragma warning restore 612,618
            ...
            internal sealed class HasFlagTranslator : CallTranslator
            {
                     ...
                     LinqExpression sourceExpression, LinqExpression valueExpression)
                {
                    if (valueExpression.NodeType == ExpressionType.Constant && 
                        ((ConstantExpression)valueExpression).Value is null)
                    {
            ...
            internal sealed class StringConcatTranslator : CallTranslator
            {
                ...
                internal override CqtExpression Translate(ExpressionConverter parent, MethodCallExpression call)
                {
                    ...
                    if (call.Arguments.Count == 1 && (call.Arguments.First().Type == typeof(object[]) || call.Arguments.First().Type == typeof(string[])))
                    {
                        ...
                        if (newArrayExpression is not null)
                        {
                        ...
                        else
                        {
                            ...
                            if (valueExpression.Value is null)
                            {
            ...
            {
                private const string s_stringsTypeFullName = "Microsoft.VisualBasic.Strings";
                private static readonly Dictionary<MethodInfo, string> s_methodNameMap = new(4);
                ...
                private static IEnumerable<MethodInfo> GetMethods(Assembly vbAssembly)
                {
                    //Strings Types 
                    var stringsType = vbAssembly.GetType(s_stringsTypeFullName);
                    yield return GetMethodInfo(stringsType, "Len", Length, [typeof(string)]);
                    yield return GetMethodInfo(stringsType, "Mid", Substring, [typeof(string), typeof(int), typeof(int)]);
                    yield return GetMethodInfo(stringsType, "UCase", ToUpper, [typeof(string)]);
            ...
            internal sealed class VBDatePartTranslator : CallTranslator
            {
                ...
                private static readonly HashSet<string> _supportedIntervals =
                [
                        Year,
                        Month,
                        Day,
                ...
                internal override CqtExpression Translate(ExpressionConverter parent, MethodCallExpression call)
                {
                    ...
                    if (intervalLinqExpression is null)
                    {
                        throw new NotSupportedException(
            ...
            private sealed class TakeTranslator : PagingTranslator
            {
                    ...
                    ExpressionConverter parent, CqtExpression operand, CqtExpression count)
                {
                    ...
                    return constant is null || !constant.Value.Equals(0)
            ...
            private sealed class JoinTranslator : SequenceMethodTranslator
            {
                ...
                internal override CqtExpression Translate(ExpressionConverter parent, MethodCallExpression call)
                {
                    ...
                    var selectorLambdaIsTrivialRename = IsTrivialRename(
                        selectorLambda, parent, out var outerBindingName, out var innerBindingName, out var initializerMetadata);

                    // translator key selectors
                    var outerKeySelector = selectorLambdaIsTrivialRename
                                               ? parent.TranslateLambda(outerLambda, outer, outerBindingName, out var outerBinding)
                                               : parent.TranslateLambda(outerLambda, outer, out outerBinding);
                    var innerKeySelector = selectorLambdaIsTrivialRename
                                               ? parent.TranslateLambda(innerLambda, inner, innerBindingName, out var innerBinding)
                        ...
                        var resultType = TypeUsage.Create(
                            TypeHelpers.CreateRowType(
                                new List<KeyValuePair<string, TypeUsage>>
                                    {
                                        new(outerBinding.VariableName, outerBinding.VariableType),
                                        new(innerBinding.VariableName, innerBinding.VariableType)
                                    },
                                initializerMetadata));
            ...
            private abstract class AggregateTranslator : SequenceMethodTranslator
            {
                ...
                internal override CqtExpression Translate(ExpressionConverter parent, MethodCallExpression call)
                {
                    ...
                    if (!isUnary)
                    {
                        ...
                        var cqtLambda = parent.TranslateLambda(lambda, operand, out DbExpressionBinding sourceBinding);
            ...
            private sealed class OfTypeTranslator : UnarySequenceMethodTranslator
            {
                    ...
                    MethodCallExpression call)
                {
                    ...
                    if (!parent.TryGetValueLayerType(clrType, out var modelType)
                        ||
            ...
            private abstract class OneLambdaTranslator : SequenceMethodTranslator
            {
                ...
                internal override CqtExpression Translate(ExpressionConverter parent, MethodCallExpression call)
                {
                    return Translate(parent, call, out var source, out var sourceBinding, out var lambda);
                }
            ...
            private sealed class SelectTranslator : OneLambdaTranslator
            {
                ...
                internal override CqtExpression Translate(ExpressionConverter parent, MethodCallExpression call)
                {
                    var result = Translate(parent, call, out var source, out var sourceBinding, out var lambda);
                    return result;
                }
            ...
            private sealed class DefaultIfEmptyTranslator : SequenceMethodTranslator
            {
                ...
                internal override DbExpression Translate(ExpressionConverter parent, MethodCallExpression call)
                {
                    ...
                    DbExpression left = DbExpressionBuilder.NewCollection([1]);
                    ...
                    if (requireSentinel)
                    {
                        projection = DbExpressionBuilder.Case(
                            [projection.Property("sentinel").IsNull()], [defaultValue], projection.Property("value"));
                    }
            ...
            private abstract class FirstTranslatorBase : UnarySequenceMethodTranslator
            {
                    ...
                    ExpressionConverter parent, CqtExpression operand, MethodCallExpression call)
                {
                    ...
                    if (parent.TryGetSpan(operand, out var inputSpan))
                    {
                ...
                internal static CqtExpression AddDefaultCase(CqtExpression element, Type elementType)
                {
                    ...
                    {
                        CreateIsNullExpression(element, elementType)
                    };
            ...
            private abstract class FirstPredicateTranslatorBase : OneLambdaTranslator
            {
                ...
                internal override CqtExpression Translate(ExpressionConverter parent, MethodCallExpression call)
                {
                    ...
                    else
                    {
                        ...
                        // Span is preserved over First/FirstOrDefault with or without a predicate
                        if (parent.TryGetSpan(input, out var inputSpan))
                        {
            ...
            private sealed class SelectManyTranslator : OneLambdaTranslator
            {
                ...
                internal override CqtExpression Translate(ExpressionConverter parent, MethodCallExpression call)
                {
                    ...
                    var isLeftOuterJoin = IsLeftOuterJoin(apply, out var applyInput, out var lojRightInput);
                    if (isLeftOuterJoin)
                    {
                        // 1)
                        ...
                        if (resultSelector is not null
                            && IsTrivialRename(resultSelector, parent, out var outerBindingName, out var innerBindingName, out var initializerMetadata))
                        {
                            ...
                            var resultType = TypeUsage.Create(
                                TypeHelpers.CreateRowType(
                                    new List<KeyValuePair<string, TypeUsage>>
                                        {
                                            new(newInput.VariableName, newInput.VariableType),
                                            new(newApply.VariableName, newApply.VariableType)
                                        },
                                    initializerMetadata));
                    ...
                    CqtExpression cqtExpression, out DbExpressionBinding crossApplyInput, out EdmProperty lojRightInput)
                {
                    ...
                    if (lojRightValueSource is null)
                    {
                        return false;
                    }
            ...
            private sealed class GroupByTranslator : SequenceMethodTranslator
            {
                    ...
                    ExpressionConverter parent, MethodCallExpression call, SequenceMethod sequenceMethod)
                {
                    ...
                    var keySelector = parent.TranslateLambda(keySelectorLinq, source, out DbGroupExpressionBinding sourceGroupBinding);
                    ...
                    if (hasElementSelector)
                    {
                        ...
                        var elementSelector = parent.TranslateLambda(elementSelectorLinq, selection, out DbExpressionBinding elementSelectorSourceBinding);
                    ...
                    {
                        new EdmProperty(KeyColumnName, projectionTerms[0].ResultType),
                        new EdmProperty(GroupColumnName, projectionTerms[1].ResultType)
                    };
            ...
            private abstract class ThenByTranslatorBase : SequenceMethodTranslator
            {
                ...
                internal override CqtExpression Translate(ExpressionConverter parent, MethodCallExpression call)
                {
                    ...
                    {
                        new DbSortClause(lambda, _ascending, null)
                    };
