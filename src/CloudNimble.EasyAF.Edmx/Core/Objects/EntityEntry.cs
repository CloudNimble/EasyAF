...
namespace System.Data.Entity.Core.Objects
{
    ...
    internal sealed class EntityEntry : ObjectStateEntry
    {
            ...
            : base(cache, entitySet, state)
        {
            ...
            Debug.Assert((entityKey is null) || (entityKey.EntitySetName == entitySet.Name), "different entitySet");
        ...
        {
            _requiresScalarChangeTracking = Entity is not null && !(Entity is IEntityWithChangeTracker);

            _requiresComplexChangeTracking = Entity is not null &&
                                             (_requiresScalarChangeTracking ||
                                              (WrappedEntity.IdentityType != Entity.GetType() &&
                                               _cacheTypeMetadata.Members.Any(m => m.IsComplex)));

            _requiresAnyChangeTracking = Entity is not null &&
        ...
        public override void RejectPropertyChanges(string propertyName)
        {
            ...
            if (_modifiedFields is not null
        ...
        public override void AcceptChanges()
        {
            ...
            switch (State)
            {
                ...
                case EntityState.Added:
                    ...
                    Debug.Assert(Entity is not null, "Non-relationship entries should have a non-null entity.");
                    Debug.Assert((object)_entityKey is not null, "All entities in the state manager should have a non-null EntityKey.");
        ...
        public override RelationshipManager RelationshipManager
        {
            get
            {
                ...
                if (WrappedEntity.Entity is null)
                {
        ...
        {
            Debug.Assert(_cache is not null, "Cannot Reset an entity that is not currently attached to a context.");
        ...
        internal override void RevertDelete()
        {
            ...
            State = (_modifiedFields is null) ? EntityState.Unchanged : EntityState.Modified;
        ...
        private void EntityMemberChanged(string entityMemberName, object complexObject, string complexObjectMemberName)
        {
            ...
            try
            {
                var changingOrdinal = GetAndValidateChangeMemberInfo(
                    ...
                    out var typeMetadata, out var changingMemberName, out var changingObject);
                    ...
                    && IsPropertyAForeignKey(entityMemberName, out var relationships))
                {
                    foreach (var relationship in relationships)
                    {
                        ...
                        Debug.Assert(relatedEnd is not null, "relatedEnd should exist if property is a foreign key");
                        var reference = relatedEnd as EntityReference;
                        Debug.Assert(reference is not null, "relatedEnd should be an EntityReference");
            ...
            {
                Debug.Assert(_cache is not null, "Unexpected null state manager.");
        ...
        {
            Debug.Assert(EntityKey is not null, "entry should have a not null EntityKey");
            ...
            StateManagerMemberMetadata memberMetadata, object oldComplexObject, object newComplexObject, bool useOldComplexObject)
        {
            ...
            if (newComplexObject is null)
            {
                throw new InvalidOperationException(Strings.ComplexObject_NullableComplexTypesNotSupported(memberMetadata.CLayerName));
            }
            Debug.Assert(
                oldComplexObject is null || (oldComplexObject.GetType() == newComplexObject.GetType()),
        ...
        {
            FindRelatedEntityKeysByForeignKeys(out var keys, useOriginalValues: false);
            if (keys is not null)
            {
                foreach (var pair in keys)
                {
                    var reference = pair.Key as EntityReference;
                    Debug.Assert(reference is not null, "EntityReference expected");
        ...
        private void AddComplexObjectSnapshot(object userObject, int ordinal, object complexObject)
        {
            ...
            if (!_originalComplexObjects.TryGetValue(userObject, out var ordinal2complexObject))
            {
                ordinal2complexObject = [];
        ...
        {
            if (_originalComplexObjects is null
                || complexObject is null)
            {
                return;
            }
            ...
            bool detectOnly)
        {
            ...
            Debug.Assert(oldComplexValue is not null, "original complex type value should not be null at this point");
        ...
        private object GetComplexObjectSnapshot(object parentObject, int parentOrdinal)
        {
            ...
            if (_originalComplexObjects is not null)
            {
                if (_originalComplexObjects.TryGetValue(parentObject, out var ordinal2complexObject))
                {
        ...
        internal void UpdateComplexObjectSnapshot(StateManagerMemberMetadata member, object userObject, int ordinal, object currentValue)
        {
            ...
            if (_originalComplexObjects is not null)
            {
                if (_originalComplexObjects.TryGetValue(userObject, out var ordinal2complexObject))
                {
                    Debug.Assert(ordinal2complexObject is not null, "value should already exists");

                    ordinal2complexObject.TryGetValue(ordinal, out var oldValue);
        ...
        internal void FixupFKValuesFromNonAddedReferences()
        {
            ...
            foreach (var dependent in ForeignKeyDependents)
            {
                var reference =
                    ...
                    EntityReference;
                Debug.Assert(reference is not null, "Expected reference to exist and be an entity reference (not collection)");
                ...
                if (reference.TargetAccessor.HasProperty)
                {
                    ...
                    {
                        if (_cache.TryGetObjectStateEntry(principal, out var principalEntry)
        ...
        {
            Debug.Assert(_wrappedEntity is not null, "wrapped entity shouldn't be null");
            Debug.Assert(
        ...
        private void TakeSnapshotOfSingleRelationship(RelatedEnd relatedEnd, NavigationProperty n, object o)
        {
            ...
            if (relatedEntry is not null)
            {
                ...
                {
                    Debug.Assert(relatedWrapper.ObjectStateEntry is not null, "Expected related entity to be tracked in snapshot code.");
        ...
        internal void DetectChangesInRelationshipsOfSingleEntity()
        {
            ...
            foreach (var n in navigationProperties)
            {
                ...
                Debug.Assert(relatedEnd is not null, "relatedEnd is null");
            ...
            bool verifyForAdd)
        {
            ...
            if (verifyForAdd
                ...
                && ObjectStateManager.FindEntityEntry(relatedObject) is null)
            {
            ...
            RelatedEnd relatedEnd)
        {
            ...
            if (!relationships.TryGetValue(relatedEnd.WrappedOwner, out var alreadyDetectedRelationshipsFrom))
            {
                alreadyDetectedRelationshipsFrom = [];
                relationships.Add(relatedEnd.WrappedOwner, alreadyDetectedRelationshipsFrom);
            }

            if (!alreadyDetectedRelationshipsFrom.TryGetValue(relatedEnd, out var objectsInRelatedEnd))
            {
                objectsInRelatedEnd = [];
        ...
        internal void Detach()
        {
            ...
            Debug.Assert(relationshipManager is not null, "Entity wrapper returned a null RelationshipManager");
        ...
        private void NullAllForeignKeys()
        {
            foreach (var dependent in ForeignKeyDependents)
            {
                ...
                Debug.Assert(relatedEnd is not null, "Expected non-null EntityReference to principal.");
        ...
        private void DetachRelationshipsEntries(RelationshipManager relationshipManager)
        {
            ...
            foreach (var relationshipEntry in _cache.CopyOfRelationshipsByKey(EntityKey))
            {
                ...
                var targetKey = relationshipEntry.RelationshipWrapper.GetOtherEntityKey(EntityKey);
                Debug.Assert(targetKey is not null, "EntityKey not on either side of relationship as expected");
            ...
            if (State != EntityState.Detached)
            {
                ...
                foreach (var relationshipEntry in _cache.CopyOfRelationshipsByKey(EntityKey))
                {
                    ...
                    if ((relationshipEntry.State != EntityState.Deleted)
                        &&
                        (relationshipSet is null || relationshipSet == relationshipEntry.EntitySet))
                    {
                        var otherEnd = GetOtherEndOfRelationship(relationshipEntry);
                        if (endMember is null
        ...
        private bool RetrieveAndCheckReferentialConstraintValuesInAcceptChanges()
        {
            ...
            // names of properties which should be retrieved from Principal entities
            // true iff there are properties which should be checked in dependent entities

            // Get RI property names from metadata
            var skippedFKs = relationshipManager.FindNamesOfReferentialConstraintProperties(
                out var propertiesToRetrieve, out var propertiesToCheckExist, skipFK: true);

            // Do not try to retrieve RI properties if entity doesn't participate in any RI Constraints
            if (propertiesToRetrieve is not null)
            {
        ...
        internal void RetrieveReferentialConstraintPropertiesFromKeyEntries(Dictionary<string, KeyValuePair<object, IntBox>> properties)
        {
            ...
            foreach (var relationshipEntry in _cache.FindRelationshipsByKey(EntityKey))
            {
                ...
                if (otherEnd.IsKeyEntry)
                {
                    ...
                    Debug.Assert(association is not null, "relationship is not an association");
                    ...
                    foreach (var constraint in association.ElementType.ReferentialConstraints)
                    {
                        ...
                        if (constraint.ToRole.Name == thisRole)
                        {
                            ...
                            Debug.Assert(otherEndKeyValues is not null, "key entry must have key values");
        ...
        {
            Debug.Assert(!IsKeyEntry);
            Debug.Assert((object)_entityKey is not null);
        ...
        internal void AttachObjectStateManagerToEntity()
        {
            ...
            Debug.Assert(_wrappedEntity.Entity is not null, "Cannot attach a null entity to the state manager");
        ...
        internal void GetOtherKeyProperties(Dictionary<string, KeyValuePair<object, IntBox>> properties)
        {
            ...
            Debug.Assert(entityType is not null, "EntityType is null");
        ...
        internal void VerifyEntityValueIsEditable(StateManagerTypeMetadata typeMetadata, int ordinal, string memberName)
        {
            ...
            Debug.Assert(member is not null, "Member shouldn't be null.");
        ...
        internal void SetCurrentEntityValue(StateManagerTypeMetadata metadata, int ordinal, object userObject, object newValue)
        {
            ...
            var member = metadata.Member(ordinal);
            Debug.Assert(member is not null, "StateManagerMemberMetadata was not found for the given ordinal.");
        ...
        internal bool HasRecordValueChanged(DbDataRecord record, int propertyIndex, object newFieldValue)
        {
            ...
            return (existing != newFieldValue) &&
                   ...
                   (_cache.EntryHasConceptualNull(this) && _modifiedFields is not null && _modifiedFields[propertyIndex]);
        }
        ...
        {
            Debug.Assert(_cache is not null, "Attempt to fixup detached entity entry");
        ...
        internal void FixupEntityReferenceByForeignKey(EntityReference reference)
        {
            ...
            bool needToSetRef;
            if ((object)foreignKey is null
                || existingPrincipal.Entity is null)
            {
                needToSetRef = true;
            }
            else
            {
                ...
                if ((existingPrincipalKey is null || existingPrincipalKey.IsTemporary)
                    && existingPrincipalEntry is not null)
                {
            ...
            if (_cache.TransactionManager.RelationshipBeingUpdated != reference)
            {
                if (needToSetRef)
                {
                    ...
                    if ((object)foreignKey is not null)
                    {
            ...
            EntityReference relatedEnd, EntityKey foreignKey, bool setIsLoaded, bool replaceExistingRef)
        {
            ...
            var canModifyReference = _cache.TransactionManager.RelationshipBeingUpdated != relatedEnd &&
                                     (!_cache.TransactionManager.IsForeignKeyUpdate ||
                                      relatedEnd.ReferenceValue.ObjectStateEntry is null ||
            ...
            ObjectStateManager.ForgetEntryWithConceptualNull(this, resetAllKeys: false);
            if (foreignKey is not null) // Implies no value is null or CreateKeyFromForeignKeyValues would have returned null
            {
                ...
                if (_cache.TryGetEntityEntry(foreignKey, out var principalEntry)
                    &&
                    ...
                    && relatedEnd.CanSetEntityType(principalEntry.WrappedEntity))
                {
                    if (canModifyReference)
                    {
                        ...
                        if (_cache.TransactionManager.PopulatedEntityReferences is not null)
                        {
                            Debug.Assert(
                ...
                else
                {
                    ...
                    if (canModifyReference
                        && replaceExistingRef
                        && relatedEnd.ReferenceValue.Entity is not null)
                    {
            ...
            {
                if (replaceExistingRef && (relatedEnd.ReferenceValue.Entity is not null || relatedEnd.EntityKey is not null))
                {
                    relatedEnd.ReferenceValue = NullEntityWrapper.NullWrapper;
                }
                if (setIsLoaded)
        ...
        private static bool WillNotRefSteal(EntityReference refToPrincipal, IEntityWrapper wrappedPrincipal)
        {
            ...
            if ((refToPrincipal.ReferenceValue.Entity is null && refToPrincipal.NavigationPropertyIsNullOrMissing())
                &&
                (refToDependent is null
                 || (refToDependent.ReferenceValue.Entity is null && refToDependent.NavigationPropertyIsNullOrMissing())))
            {
                // Return true if the ref to principal is null and it's not 1:1 or it is 1:1 and the ref to dependent is also null.
                return true;
            }
            else if (refToDependent is not null
                     &&
            ...
            else if (refToDependent is null
                     ||
        ...
        internal bool TryGetReferenceKey(AssociationEndMember principalRole, out EntityKey principalKey)
        {
            ...
            Debug.Assert(relatedEnd is not null, "Expected there to be a non null EntityReference to the principal");
            if (relatedEnd.CachedValue.Entity is null
                || relatedEnd.CachedValue.ObjectStateEntry is null)
            {
                principalKey = null;
                return false;
            }
            principalKey = relatedEnd.EntityKey ?? relatedEnd.CachedValue.ObjectStateEntry.EntityKey;
            return principalKey is not null;
        }
        ...
        private void FixupForeignKeysByReference(List<EntityEntry> visited)
        {
            ...
            foreach (var dependent in ForeignKeyDependents)
            {
                ...
                Debug.Assert(reference is not null, "Expected reference to exist and be an entity reference (not collection)");
                var existingPrincipal = reference.ReferenceValue;
                if (existingPrincipal.Entity is not null)
                {
            ...
            foreach (var principal in ForeignKeyPrincipals)
            {
                ...
                foreach (var dependent in principalEnd.GetWrappedEntities())
                {
                    ...
                    Debug.Assert(dependentEntry is not null, "Should have fully tracked graph at this point.");
                        ...
                        || (dependentEntry.State == EntityState.Modified && !fkOverlapsPk))
                    {
                        ...
                        Debug.Assert(principalRef is not null, "Expected reference to exist and be an entity reference (not collection)");
                        // "forceChange" is false because we don't want to actually set the property values
                        // here if they are aready set to the same thing--we don't want the events and setting
                        // the modified flag is irrelavent during AcceptChanges.
        ...
        private bool IsPropertyAForeignKey(string propertyName, out List<Pair<string, string>> relationships)
        {
            ...
            return relationships is not null;
        }
            ...
            bool useOriginalValues)
        {
            ...
            foreach (var dependent in ForeignKeyDependents)
            {
                ...
                if (foreignKey is not null) // Implies no value is null or CreateKeyFromForeignKeyValues would have returned null
                {
                    ...
                    relatedEntities = relatedEntities is not null ? relatedEntities : [];
                    if (!relatedEntities.TryGetValue(reference, out var entityKeys))
                    {
                        entityKeys = [];
            ...
            foreach (var entityReference in FindFKRelatedEnds())
            {
                    ...
                    || currentKey is not null)
                {
                    if (originalKey is null)
                    {
                        ...
                        ObjectStateManager.TryGetEntityEntry(currentKey, out var entry);
