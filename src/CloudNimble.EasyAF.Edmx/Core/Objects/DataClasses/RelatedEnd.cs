...
namespace System.Data.Entity.Core.Objects.DataClasses
{
    ...
    public abstract class RelatedEnd : IRelatedEnd
    {
        ...
        internal bool IsForeignKey
        {
            get
            {
                Debug.Assert(ObjectContext is not null, "the IsForeignKey property shouldn't be used in detached scenarios");
                Debug.Assert(_relationMetadata is not null, "this._relationMetadata is null");
        ...
        internal ObjectQuery<TEntity> CreateSourceQuery<TEntity>(MergeOption mergeOption, out bool hasResults)
        {
            ...
            {
                Debug.Assert(stateEntry is not null, "Entity should exist in the current context");
        ...
        {
            Debug.Assert(_relationshipSet is not null, "If we are attached to a context, we should have a relationship set.");
        ...
        internal void DeferredLoad()
        {
                ...
                CanDeferredLoad)
            {
                ...
                Debug.Assert(_wrappedOwner is not null, "Wrapper owner should never be null");
                if (UsingNoTracking ||
                    (_wrappedOwner.ObjectStateEntry is not null &&
        ...
        internal virtual void Merge<TEntity>(IEnumerable<TEntity> collection, MergeOption mergeOption, bool setIsLoaded)
        {
            ...
            if (refreshedCollection is null)
            {
                refreshedCollection = [];
        ...
        internal virtual void Merge<TEntity>(List<IEntityWrapper> collection, MergeOption mergeOption, bool setIsLoaded)
        {
            ...
            if (WrappedOwner.EntityKey is null)
            {
                throw Error.EntityKey_UnexpectedNull();
            }
        ...
        void IRelatedEnd.Attach(object entity)
        {
            ...
            CheckOwnerNull();
            Attach([EntityWrapperFactory.WrapEntityUsingContext(entity, ObjectContext)], false);
        }
        ...
        internal void Attach(IEnumerable<IEntityWrapper> wrappedEntities, bool allowCollection)
        {
            ...
            try
            {
                ...
                if (constraint is not null)
                {
                    ...
                    Debug.Assert(ownerEntry is not null, "Both entities should be attached.");
                    ...
                    else
                    {
                        foreach (var wrappedTarget in collection)
                        {
                            ...
                            if (targetRelatedEnd.IsDependentEndOfReferentialConstraint(checkIdentifying: false))
                            {
                                ...
                                Debug.Assert(targetEntry is not null, "Both entities should be attached.");
                                if (!VerifyRIConstraintsWithRelatedEntry(
        ...
        internal void Add(IEntityWrapper wrappedEntity)
        {
            ...
            if (_wrappedOwner.Entity is not null)
            {
        ...
        private void ValidateContextsAreCompatible(RelatedEnd targetRelatedEnd)
        {
            ...
            else if (ObjectContext is not null
                     && targetRelatedEnd.ObjectContext is not null)
            {
            ...
            else if ((_context is null || UsingNoTracking)
                     && (targetRelatedEnd.ObjectContext is not null && !targetRelatedEnd.UsingNoTracking))
            {
                ...
                && ObjectContext is not null)
            {
                    ...
                    && !UsingNoTracking)
                {
                    ...
                    if (!ObjectContext.ObjectStateManager.TransactionManager.IsLocalPublicAPI
                        && WrappedOwner.EntityKey is not null
                ...
                if (!attachedRelatedEnd.UsingNoTracking)
                {
                    ...
                    try
                    {
                        ...
                        finally
                        {
                            if (doCleanup)
                            {
                                Debug.Assert(entityToAdd is not null, "entityToAdd should be set if attachedRelatedEnd is set");
            ...
            bool addRelationshipAsUnchanged, bool doAttach)
        {
            ...
            if (!relationshipAlreadyExists
                && ObjectContext is not null
                && wrappedEntity.Context is not null)
            {
                if (!IsForeignKey)
                {
        ...
        internal void IncludeEntity(IEntityWrapper wrappedEntity, bool addRelationshipAsUnchanged, bool doAttach)
        {
            ...
            Debug.Assert(
                cacheEntry is null || cacheEntry.Entity == wrappedEntity.Entity,
        ...
        internal void MarkForeignKeyPropertiesModified()
        {
            ...
            Debug.Assert(constraint is not null, "null constraint");

            var dependentEntry = WrappedOwner.ObjectStateEntry;
            Debug.Assert(dependentEntry is not null, "Expected tracked entity.");
        ...
        {
            Debug.Assert(RelationshipNavigation is not null, "null RelationshipNavigation");

            if (TargetAccessor.HasProperty
                && !CheckIfNavigationPropertyContainsEntity(wrapper))
            {
                Debug.Assert(wrapper.Context is not null, "Expected context to be available.");
            ...
            ReferentialConstraint constraint, Func<string, object> getDependentPropertyValue, EntityKey principalKey)
        {
            ...
            for (var i = 0; i < constraint.FromProperties.Count; ++i)
            {
                ...
                Debug.Assert(currentValue is not null, "currentValue is part of Key on an attached entity, it must not be null");
        ...
        internal virtual void OnAssociationChanged(CollectionChangeAction collectionChangeAction, object entity)
        {
            ...
            if (!_suppressEvents)
            {
                if (_onAssociationChanged is not null)
                {
        ...
        {
            DebugCheck.NotNull(wrappedEntity);
            Debug.Assert(_context is not null, "Can't add to state manager if _context is null");
        ...
        internal EntitySet GetTargetEntitySetFromRelationshipSet()
        {
            ...
            Debug.Assert(associationSet is not null, "(AssociationSet) cast failed");

            var associationEndMember = (AssociationEndMember)ToEndMember;
            Debug.Assert(associationEndMember is not null, "(AssociationEndMember) cast failed");

            entitySet = associationSet.AssociationSetEnds[associationEndMember.Name].EntitySet;
            Debug.Assert(entitySet is not null, "cannot find entitySet");
            return entitySet;
        }
            ...
            IEntityWrapper wrappedEntity, bool addRelationshipAsUnchanged, bool doAttach)
        {
            ...
            Debug.Assert(_context is not null && wrappedEntity.Context is not null, "should be called only if both entities are attached");
        ...
        internal static void RemoveEntityFromObjectStateManager(IEntityWrapper wrappedEntity)
        {
            ...
            if (wrappedEntity.Context is not null
                ...
                && wrappedEntity.Context.ObjectStateManager.TransactionManager.PromotedKeyEntries.TryGetValue(wrappedEntity.Entity, out var entry))
            {
            ...
            IEntityWrapper wrappedEntity, IEntityWrapper wrappedOwner, RelationshipSet relationshipSet, RelationshipNavigation navigation)
        {
            DebugCheck.NotNull(wrappedEntity);
            Debug.Assert(
                relationshipSet is null || !(relationshipSet.ElementType as AssociationType).IsForeignKey,
        ...
        {
            if (_navigation.AssociationType is null || _navigation.AssociationType.Index < 0)
            {
        ...
        {
            if (_wrappedOwner.Entity is null)
            {
        ...
        {
            _wrappedOwner = wrappedOwner is not null ? wrappedOwner : NullEntityWrapper.NullWrapper;
...
#pragma warning restore 612
        ...
        internal NavigationProperty NavigationProperty
        {
            get
            {
                if (navigationPropertyCache is null
                    && _wrappedOwner.Context is not null
                    && TargetAccessor.HasProperty)
                {
                    ...
                    if (!entityType.NavigationProperties.TryGetValue(navigationPropertyName, false, out var member))
                    {
        ...
        private void EnsureRelationshipNavigationAccessorsInitialized()
        {
            ...
            if (!RelationshipNavigation.IsInitialized)
            {
                ...
                if (associationType.AssociationEndMembers.TryGetValue(sourceRoleName, false, out var sourceEnd))
                {
                ...
                if (associationType.AssociationEndMembers.TryGetValue(targetRoleName, false, out var targetEnd))
                {
                ...
                if (sourceAccessor is null
                    || targetAccessor is null)
                {
