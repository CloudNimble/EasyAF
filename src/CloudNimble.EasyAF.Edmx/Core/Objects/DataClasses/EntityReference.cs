    ...
    public abstract class EntityReference : RelatedEnd
    {
        ...
        public EntityKey EntityKey
        {
            ...
            get
            {
                if (ObjectContext is not null
                    && !UsingNoTracking)
                {
                    Debug.Assert(WrappedOwner.Entity is not null, "Unexpected null Owner on EntityReference attached to a context");

                    EntityKey attachedKey = null;

                    // If this EntityReference contains an entity, look up the key on that object
                    if (CachedValue.Entity is not null)
                    {
                    ...
                    Debug.Assert(
                        attachedKey is null || IsValidEntityKeyType(attachedKey),
        ...
        internal void SetEntityKey(EntityKey value, bool forceFixup)
        {
            if (value is not null
                && value == EntityKey
                && (ReferenceValue.Entity is not null || (ReferenceValue.Entity is null && !forceFixup)))
            {
                ...
                else
                {
                    ...
                    if (addNewRelationship)
                    {
                        var ownerKey = ValidateOwnerWithRIConstraints(
                            targetEntry is null ? null : targetEntry.WrappedEntity, value, checkBothEnds: true);
        ...
        {
            if (ReferenceValue.Entity is null
                &&
                WrappedOwner.Entity is not null
                &&
                WrappedOwner.Context is not null
                &&
                !UsingNoTracking
                &&
                IsForeignKey)
        ...
        internal EntityKey AttachedEntityKey
        {
            ...
            {
                Debug.Assert(
                    ObjectContext is not null && !UsingNoTracking,
        ...
        {
            if (ObjectContext is not null
                && ObjectContext.ObjectStateManager is not null // are we attached?
                && source is not null // do we have an entry?
                && _cachedForeignKey is not null
        ...
        internal IEnumerable<EntityKey> GetAllKeyValues()
        {
            ...
            if (_cachedForeignKey is not null)
            {
                yield return _cachedForeignKey;
            }

            if (_detachedEntityKey is not null)
            {
        ...
        internal EntityKey ValidateOwnerWithRIConstraints(IEntityWrapper targetEntity, EntityKey targetEntityKey, bool checkBothEnds)
        {
            ...
            if ((object)ownerKey is not null
                &&
                !ownerKey.IsTemporary
                &&
                IsDependentEndOfReferentialConstraint(checkIdentifying: true))
            {
                Debug.Assert(CachedForeignKey is not null || EntityKey is null, "CachedForeignKey should not be null if EntityKey is not null.");
                ValidateSettingRIConstraints(
                    targetEntity,
                    targetEntityKey is null,
                    (CachedForeignKey is not null && CachedForeignKey != targetEntityKey));
            }
            else if (checkBothEnds
                     && targetEntity is not null
                     && targetEntity.Entity is not null)
            {
        ...
        {
            var isNoTracking = targetEntity is not null && targetEntity.MergeOption == MergeOption.NoTracking;
            ...
            if (settingToNull
                ...
                (targetEntity is not null &&
                 !isNoTracking &&
                 (targetEntity.ObjectStateEntry is null || // setting to a detached principle
                  (EntityKey is null && targetEntity.ObjectStateEntry.State == EntityState.Deleted || // setting to a deleted principle
                   (CachedForeignKey is null && targetEntity.ObjectStateEntry.State == EntityState.Added)))))
            ...
            IEntityWrapper dependentEntity, IEntityWrapper principalEntity, Dictionary<int, object> changedFKs, bool forceChange)
        {
            ...
            var isUnchangedDependent = (object)WrappedOwner.EntityKey is not null &&
            ...
            try
            {
                ...
                for (var i = 0; i < numValues; i++)
                {
                    ...
                    if (forceChange || valueChanging)
                    {
                        ...
                        {
                            ValidateSettingRIConstraints(
                                principalEntity, settingToNull: value is null, changingForeignKeyValue: valueChanging);
                        }
                        ...
                        if (changedFKs is not null)
                        {
                            if (changedFKs.TryGetValue(dependentOrdinal, out var previouslySetValue))
                            {
                    ...
                    {
                        SetCachedForeignKey(
                            value is null ? null : new EntityKey(principalEntitySet, value),
                            dependentEntity.ObjectStateEntry);
                    }
                }
                ...
                {
                    SetCachedForeignKey(
                        values.Any(v => v is null) ? null : new EntityKey(principalEntitySet, values),
                        dependentEntity.ObjectStateEntry);
                }

                if (WrappedOwner.ObjectStateEntry is not null)
                {
        ...
        internal bool NavigationPropertyIsNullOrMissing()
        {
            ...
            return !TargetAccessor.HasProperty || WrappedOwner.GetNavigationPropertyValue(this) is null;
        }
        ...
        internal override void AddEntityToObjectStateManager(IEntityWrapper wrappedEntity, bool doAttach)
        {
            ...
            if (DetachedEntityKey is not null)
            {
        ...
        internal override void AddToNavigationPropertyIfCompatible(RelatedEnd otherRelatedEnd)
        {
            ...
            if (NavigationPropertyIsNullOrMissing())
            {
                ...
                Debug.Assert(otherRelatedEnd.ObjectContext is not null, "Expected attached context at this point.");
        ...
        internal override bool CheckReferentialConstraintPrincipalProperty(EntityEntry ownerEntry, ReferentialConstraint constraint)
        {
            ...
            if (!IsEmpty())
            {
                ...
                if (wrappedRelatedEntity.ObjectStateEntry is not null
        ...
        {
            Debug.Assert(ObjectContext is not null, "Nulling FKs only works when attached.");
                ...
                && !transManager.IsRelatedEndAdd)
            {
                ...
                if (TargetRoleName == constraint.FromRole.Name) // Only do this on the dependent end
                {
                    if (transManager.IsDetaching)
                    {
                        ...
                        Debug.Assert(entry is not null, "State entry must exist while detaching.");
                             ...
                             && !transManager.IsForeignKeyUpdate)
                    {
                        ...
                        try
                        {
                            ...
                            var canSetModifiedProps = entry is not null
                            ...
                            for (var i = 0; i < constraint.FromProperties.Count; i++)
                            {
                                ...
                                if (member.ClrMetadata.Nullable)
                                {
                                    ...
                                    if (member.GetValue(WrappedOwner.Entity) is not null)
                                    {
                                    ...
                                    else
                                    {
                                        ...
                                        if (canSetModifiedProps
                                            && WrappedOwner.ObjectStateEntry.OriginalValues.GetValue(dependentOrdinal) is not null)
                                        {
