...
namespace System.Data.Entity.Core.Objects.DataClasses
{
        ...
        where TEntity : class
    {
        ...
        internal override IEntityWrapper ReferenceValue
        {
            ...
            set
            {
                ...
                else
                {
                    if (UsingNoTracking)
                    {
                        if (_wrappedCachedValue.Entity is not null)
                        {
        ...
        public override void Load(MergeOption mergeOption)
        {
            ...
            var sourceQuery = ValidateLoad<TEntity>(mergeOption, "EntityReference", out var hasResults);
        ...
        internal override IEnumerable GetInternalEnumerable()
        {
            ...
            if (ReferenceValue.Entity is not null)
            {
        ...
        {
            return _wrappedCachedValue.Entity is null ? new IEntityWrapper[0] : [_wrappedCachedValue];
        }
        ...
        {
            Debug.Assert(ObjectContext is not null, "Should not be trying to add entities to state manager if context is null");
            ...
            else if (DetachedEntityKey is not null)
            {
        ...
        {
            Debug.Assert(ObjectContext is not null, "Should not be trying to remove entities from state manager if context is null");
            ...
            if (null != _wrappedCachedValue.Entity)
            {
                ...
                if ((transManager.ProcessedEntities is null || !transManager.ProcessedEntities.Contains(_wrappedCachedValue))
                    &&
                    (doFullRemove || doRelatedEndRemove))
                {
                    ...
                    Debug.Assert(
                        IsForeignKey || relationshipEntry is not null,
                ...
                foreach (var constraint in ((AssociationType)RelationMetadata).ReferentialConstraints)
                {
                    if (constraint.ToRole == FromEndMember)
                    {
                        ...
                        _wrappedCachedValue.RelationshipManager.RetrieveReferentialConstraintProperties(
                            out var retrievedProperties, visited, includeOwnValues: true);

                        Debug.Assert(retrievedProperties is not null);
                        Debug.Assert(
        ...
        {
            return _wrappedCachedValue.Entity is null;
        }
        ...
        internal override bool ContainsEntity(IEntityWrapper wrappedEntity)
        {
            ...
            return _wrappedCachedValue.Entity is not null && _wrappedCachedValue.Entity == wrappedEntity.Entity;
        }
        ...
        {
            CheckOwnerNull();
            return CreateSourceQuery<TEntity>(DefaultMergeOption, out var hasResults);
        }
        ...
        {
            Debug.Assert(_wrappedCachedValue.Entity is null, "The EntityReference already has a value.");
            var reference = relatedEnd as EntityReference<TEntity>;
            if (reference is not null
                && reference._wrappedCachedValue.Entity is not null)
            {
        ...
        internal override void AddToLocalCache(IEntityWrapper wrappedEntity, bool applyConstraints)
        {
            ...
            {
                var tm = ObjectContext is not null ? ObjectContext.ObjectStateManager.TransactionManager : null;
