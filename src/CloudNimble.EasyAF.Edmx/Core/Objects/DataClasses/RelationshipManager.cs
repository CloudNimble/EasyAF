...
namespace System.Data.Entity.Core.Objects.DataClasses
{
    ...
    public class RelationshipManager
    {
        ...
        internal void SetWrappedOwner(IEntityWrapper wrappedOwner, object expectedOwner)
        {
            ...
            Debug.Assert(
                _owner is not null || !(wrappedOwner.Entity is IEntityWithRelationships),
            ...
            where TTargetEntity : class
        {
            ...
            TryGetCachedRelatedEnd(relationshipName, targetRoleName, out var relatedEnd);

            var previousCollection = relatedEnd as EntityCollection<TTargetEntity>;
            if (existingRelatedEnd is null)
            {
                if (relatedEnd is not null)
                {
                    ...
                    Debug.Assert(previousCollection is not null, "should never receive anything but an EntityCollection here");
            ...
            else
            {
                ...
                Debug.Assert(existingRelatedEnd._onAssociationChanged is null, "Disconnected RelatedEnd had events");

                if (relatedEnd is not null)
                {
                    Debug.Assert(_relationships is not null, "Expected _relationships to be non-null.");
                ...
                if (collection is not null)
                {
                    ...
                    finally
                    {
                        ...
                        if (doCleanup && relatedEnd is not null)
                        {
                            Debug.Assert(_relationships is not null, "Expected _relationships to be non-null.");
            ...
            where TTargetEntity : class
        {
            ...
            if (TryGetCachedRelatedEnd(relationshipName, targetRoleName, out var relatedEnd))
            {
                ...
                // a collection or reference and it doesn't match the target role multiplicity, it will be detected
                // in the public GetRelatedCollection<T> or GetRelatedReference<T>
                Debug.Assert(entityRef is not null, "should never receive anything but an EntityReference here");
                return entityRef;
            }
            else
            {
        ...
        internal RelatedEnd GetRelatedEnd(string navigationProperty, bool throwArgumentException = false)
        {
            ...
            Debug.Assert(wrappedOwner.Entity is not null, "Entity is null");
            Debug.Assert(wrappedOwner.Context is not null, "Context is null");
            Debug.Assert(wrappedOwner.Context.MetadataWorkspace is not null, "MetadataWorkspace is null");
            Debug.Assert(wrappedOwner.Context.Perspective is not null, "Perspective is null");
            ...
            if (!wrappedOwner.Context.Perspective.TryGetMember(entityType, navigationProperty, false, out var member)
                ||
        ...
        internal RelatedEnd GetRelatedEndInternal(string relationshipName, string targetRoleName)
        {
            ...
            var wrappedOwner = WrappedOwner;
            if (wrappedOwner.Context is null
            ...
            Debug.Assert(associationType is not null);
            ...
            string relationshipName, string targetRoleName, RelatedEnd existingRelatedEnd, AssociationType relationship)
        {
            ...
            Debug.Assert(
                sourceEntityType.DataSpace == DataSpace.OSpace && sourceEntityType.ClrType is not null,
        ...
        internal RelatedEnd GetRelatedEndInternal(AssociationType csAssociationType, AssociationEndMember csTargetEnd)
        {
            ...
            Debug.Assert(osAssociationType is not null);
            Debug.Assert(osAssociationType.DataSpace == DataSpace.OSpace);

            GetAssociationEnds(osAssociationType, csTargetEnd.Name, out var osSourceEnd, out var osTargetEnd);
            ...
            where TTargetEntity : class
        {
            ...
            if (entityReference.WrappedOwner.Entity is not null)
            {
                throw new InvalidOperationException(
            ...
            var wrappedOwner = WrappedOwner;
            if (wrappedOwner.Context is not null
            ...
            where TTargetEntity : class
        {
            ...
            if (entityCollection.WrappedOwner.Entity is not null)
            {
                throw new InvalidOperationException(
        ...
        internal string PrependNamespaceToRelationshipName(string relationshipName)
        {
            ...
            if (!relationshipName.Contains("."))
            {
                ...
                if (objectItemCollection is not null)
                {
        ...
        private static ObjectItemCollection GetObjectItemCollection(IEntityWrapper wrappedOwner)
        {
            if (wrappedOwner.Context is not null)
            {
                Debug.Assert(wrappedOwner.Context.MetadataWorkspace is not null);
        ...
        {
            if (TryGetObjectMappingItemCollection(WrappedOwner, out var mappings)
                && mappings.TryGetMap(WrappedOwner.IdentityType.FullNameWithNesting(), DataSpace.OSpace, out var map))
            {
        ...
        {
            if (wrappedOwner.Context is not null
                && wrappedOwner.Context.MetadataWorkspace is not null)
            {
        ...
        internal AssociationType GetRelationshipType(AssociationType csAssociationType)
        {
            ...
            if (metadataWorkspace is not null)
            {
        ...
        private bool VerifyRelationship(AssociationType osAssociationType, AssociationType csAssociationType, string sourceEndName)
        {
            ...
            var metadataWorkspace = wrappedOwner.Context.MetadataWorkspace;
            Debug.Assert(metadataWorkspace is not null);
            ...
            var csAssociationSet = metadataWorkspace.MetadataOptimization.FindCSpaceAssociationSet(
                ...
                out var sourceEntitySet);

            if (csAssociationSet is null)
            {
            ...
            AssociationType relationship, string sourceEndName)
        {
            ...
            if (wrappedOwner.Context.Perspective.TryGetTypeByName(relationship.FullName, false /*ignoreCase*/, out var associationTypeUsage))
            {
                var associationSet = wrappedOwner.Context.MetadataWorkspace.MetadataOptimization.FindCSpaceAssociationSet(
                    ...
                    ownerKey.EntitySetName, ownerKey.EntityContainerName, out var sourceEntitySet);
            ...
            where TTargetEntity : class
        {
            ...
            switch (targetRoleMultiplicity)
            {
                ...
                case RelationshipMultiplicity.One:
                    if (existingRelatedEnd is not null)
                    {
                        Debug.Assert(
                            wrappedOwner.Context is null || wrappedOwner.MergeOption == MergeOption.NoTracking,
        ...
        public IEnumerable<IRelatedEnd> GetAllRelatedEnds()
        {
            ...
            if (wrappedOwner.Context is not null
                && wrappedOwner.Context.MetadataWorkspace is not null
                && TryGetOwnerEntityType(out var entityType))
            {
                ...
                Debug.Assert(wrappedOwner.EntityKey is not null, "null entityKey on a attached entity");
            ...
            else
            {
                ...
                if (wrappedOwner.Entity is not null)
                {
                ...
                && wrappedOwner.MergeOption != MergeOption.NoTracking)
            {
                foreach (RelatedEnd relatedEnd in GetAllRelatedEnds())
                {
                    ...
                    if (reference is not null
                        && reference.EntityKey is not null)
                    {
        ...
        {
            get { return _relationships is not null; }
        }
            ...
            if (null != _relationships)
            {
                ...
                finally
                {
                    ...
                    if (doCleanup)
                    {
                        var wrappedOwner = WrappedOwner;
                        Debug.Assert(
                            wrappedOwner.Context is not null && wrappedOwner.Context.ObjectStateManager is not null,
                        ...
                        if (transManager.IsAttachTracking
                            &&
                            transManager.PromotedKeyEntries.TryGetValue(wrappedOwner.Entity, out var entry))
                        {
        ...
        internal static void RemoveRelatedEntitiesFromObjectStateManager(IEntityWrapper wrappedEntity)
        {
            ...
            foreach (var relatedEnd in wrappedEntity.RelationshipManager.Relationships)
            {
                ...
                if (relatedEnd.ObjectContext is not null)
                {
                    Debug.Assert(
        ...
        {
            var wrappedOwner = WrappedOwner;
            Debug.Assert(wrappedOwner.Entity is not null);
            ...
            var ownerKey = wrappedOwner.EntityKey;
            Debug.Assert((object)ownerKey is not null);
            ...
            if (ownerKey.IsTemporary)
            {
                ...
                FindNamesOfReferentialConstraintProperties(out var propertiesToRetrieve, out var propertiesToPropagateExist, skipFK: false);
                ...
                if (propertiesToRetrieve is not null)
                {
                    ...
                    if (!CheckIfAllPropertiesWereRetrieved(properties, propertiesToRetrieve))
                    {
                        ...
                        Debug.Assert(entry is not null, "Owner entry not found in the object state manager");
            ...
            out List<string> propertiesToRetrieve, out bool propertiesToPropagateExist, bool skipFK)
        {
            ...
            Debug.Assert(entitySet is not null, "Unable to find entity set");
        ...
        {
            var wrappedOwner = WrappedOwner;
            Debug.Assert(wrappedEntity is not null, "IEntityWrapper instance is null.");
        ...
        internal void AttachContextToRelatedEnds(ObjectContext context, EntitySet entitySet, MergeOption mergeOption)
        {
            ...
            if (null != _relationships)
            {
                ...
                foreach (var relatedEnd in Relationships)
                {
                    relatedEnd.FindRelationshipSet(context, entitySet, out var relationshipType, out var relationshipSet);
                    if (relationshipSet is not null
                        || !relatedEnd.IsEmpty())
                    {
