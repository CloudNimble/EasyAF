...
namespace System.Data.Entity.Core.Objects
{
    ...
    public class ObjectContext : IDisposable, IObjectContextAdapter
    {
        ...
        {
            get { return _onObjectMaterialized is not null && _onObjectMaterialized.GetInvocationList().Length != 0; }
        }
            ...
            out bool isNoOperation)
        {
            ...
            var key = existingEntry is not null ? existingEntry.EntityKey : wrappedEntity.GetEntityKeyFromEntity();
            if (null != (object)key)
            {
                entitySetFromKey = key.GetEntitySet(MetadataWorkspace);

                if (entitySetFromName is not null)
                {
            ...
            if (doAttach && existingEntry is null)
            {
        ...
        public virtual void AddObject(string entitySetName, object entity)
        {
            ...
            ObjectStateManager.AssertAllForeignKeyIndexEntriesAreValid();
            var wrappedEntity = EntityWrapperFactory.WrapEntityUsingContextGettingEntry(entity, this, out var existingEntry);
            ...
            VerifyRootForAdd(false, entitySetName, wrappedEntity, existingEntry, out var entitySet, out var isNoOperation);
            if (isNoOperation)
            {
                return;
            }
        ...
        internal void AddSingleObject(EntitySet entitySet, IEntityWrapper wrappedEntity, string argumentName)
        {
            ...
            Debug.Assert(ObjectStateManager.TransactionManager.ProcessedEntities is not null, "Expected non-null collection when flag set.");
        ...
        public virtual void LoadProperty<TEntity>(TEntity entity, Expression<Func<TEntity, object>> selector)
        {
            ...
            var navProp = ParsePropertySelectorExpression(selector, out var removedConvert);
            var wrappedEntity = WrapEntityAndCheckContext(entity, "property");
        ...
        public virtual void AttachTo(string entitySetName, object entity)
        {
            ...
            VerifyRootForAdd(true, entitySetName, wrappedEntity, existingEntry, out var entitySet, out var isNoOperation);
            if (isNoOperation)
            {
                return;
            }
        ...
        internal void AttachSingleObject(IEntityWrapper wrappedEntity, EntitySet entitySet)
        {
            ...
            Debug.Assert(key is not null, "GetEntityKey should have returned a non-null key");
            ...
            if (null != entry)
            {
                if (entry.IsKeyEntry)
                {
                    ...
                    Debug.Assert(
                        ObjectStateManager.TransactionManager.ProcessedEntities is not null, "Expected non-null collection when flag set.");
        ...
        {
            if (wrappedEntity.Context is not null
                && wrappedEntity.Context != this
        ...
        internal EntitySet GetEntitySetFromName(string entitySetName)
        {
            ...
            GetEntitySetName(entitySetName, "entitySetName", this, out var setName, out var containerName);
            ...
            object entityLike, Dictionary<EntityKey, EntityEntry> entities, Dictionary<EntitySet, List<EntityKey>> currentKeys)
        {
            ...
            if (!currentKeys.TryGetValue(entitySet, out var setKeys))
            {
                setKeys = [];
        ...
        private void EnsureTransactionHandlerRegistered()
        {
                ...
                && !InterceptionContext.DbContexts.Any(dbc => dbc is TransactionContext))
            {
                ...
                if (transactionHandlerFactory is not null)
                {
        ...
        private T EnsureContextIsEnlistedInCurrentTransaction<T>(Transaction currentTransaction, Func<T> openConnection, T defaultValue)
        {
            ...
            if (transactionHasChanged)
            {
                ...
                else if (_connectionRequestCount > 1)
                {
                    ...
                    {
                        Debug.Assert(currentTransaction is not null, "transaction has changed and the lastTransaction was null");
        ...
        internal void DeleteObject(object entity, EntitySet expectedEntitySet)
        {
            ...
            if (expectedEntitySet is not null)
            {
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (!_disposed)
            {
                ...
                if (_transactionHandler is not null)
                {
                ...
                if (disposing)
                {
                    ...
                    if (_objectStateManager is not null)
                    {
        ...
        internal EntitySet GetEntitySet(string entitySetName, string entityContainerName)
        {
            ...
            if (String.IsNullOrEmpty(entityContainerName))
            {
                ...
                Debug.Assert(container is not null, "Problem with metadata - default container not found");
            }
            else
            {
            ...
            if (!container.TryGetEntitySetByName(entitySetName, false, out var entitySet))
            {
                throw new InvalidOperationException(
            ...
            string qualifiedName, string parameterName, ObjectContext context, out string entityset, out string container)
        {
            ...
            if (context is not null
                ...
                && context.Perspective.GetDefaultContainer() is null)
            {
        ...
        internal TypeUsage GetTypeUsage(Type entityCLRType)
        {
            ...
            if (!Perspective.TryGetType(entityCLRType, out var entityTypeUsage)
                || !TypeSemantics.IsEntityType(entityTypeUsage))
            {
                Debug.Assert(entityCLRType is not null, "The type cannot be null.");
                throw new InvalidOperationException(Strings.ObjectContext_NoMappingForEntityType(entityCLRType.FullName));
            }

            Debug.Assert(entityTypeUsage is not null, "entityTypeUsage is null");
        ...
        public virtual object GetObjectByKey(EntityKey key)
        {
            ...
            var entitySet = key.GetEntitySet(MetadataWorkspace);
            Debug.Assert(entitySet is not null, "Key's EntitySet should not be null in the MetadataWorkspace");
            ...
            if (!TryGetObjectByKey(key, out var entity))
            {
            ...
            Func<T> func, IDbExecutionStrategy executionStrategy, bool startLocalTransaction, bool releaseConnectionOnSuccess)
        {
            ...
            if (connection.CurrentTransaction is null
                ...
                && _lastTransaction is null)
            {
                needLocalTransaction = startLocalTransaction;
            }
            else if (executionStrategy is not null
        ...
        {
            // try the cache first
            ObjectStateManager.TryGetEntityEntry(key, out var entry); // this will check key argument
            if (entry is not null)
            {
                // can't find keys
                if (!entry.IsKeyEntry)
            ...
            for (var i = 0; i < members.Length; i++)
            {
                ...
                if (keyMembers.TryGetValue(members[i].Key, true, out var keyMember))
                {
            ...
            {
                Debug.Assert(entity is null, "Query for a key returned more than one entity!");
            ...
            string functionName, ExecutionOptions executionOptions, params ObjectParameter[] parameters)
        {
            ...
            var entityCommand = CreateEntityCommandForFunctionImport(functionName, out var functionImport, parameters);
            ...
            string functionName, out EdmFunction functionImport, params ObjectParameter[] parameters)
        {
            ...
            functionImport =
                MetadataHelper.GetFunctionImport(
                    ...
                    out var containerName, out var functionImportName);
            ...
            ObjectParameter[] parameters, EdmFunction functionImport, EntityCommand command)
        {
            ...
            for (var i = 0; i < parameters.Length; i++)
            {
                ...
                {
                    TypeUsage typeUsage;
                    if (functionParameter is not null)
                    {
                    ...
                    {
                        Debug.Assert(objectParameter.MappableType is not null, "MappableType must not be null");
                        Debug.Assert(Nullable.GetUnderlyingType(objectParameter.MappableType) is null, "Nullable types not expected here.");
        ...
        private static FunctionParameter FindParameterMetadata(EdmFunction functionImport, ObjectParameter[] parameters, int ordinal)
        {
            ...
            if (!functionImport.Parameters.TryGetValue(parameterName, false, out var functionParameter))
            {
            ...
            EntityProxyFactory.TryCreateProxyTypes(
                types.Select(
                    type =>
                    {
                        ...
                        ospaceItems.TryGetItem(type.FullNameWithNesting(), out
            ...
            where T : class
        {
            ...
            if (ContextOptions.ProxyCreationEnabled
                && ((proxyTypeInfo = EntityProxyFactory.GetProxyType(entityType, MetadataWorkspace)) is not null))
            {
                ...
                try
                {
                    ...
                    if (proxyTypeInfo.InitializeEntityCollections is not null)
                    {
                        proxyTypeInfo.InitializeEntityCollections.Invoke(null, [wrappedEntity]);
                    }
                }
                finally
                {
        ...
        public virtual ObjectResult<TElement> Translate<TElement>(DbDataReader reader)
        {
            ...
            var shaperFactory = InternalTranslate<TElement>(
                reader, /*entitySetName:*/ null, MergeOption.AppendOnly, /*streaming:*/ false, out var entitySet, out var edmType);
            return ShapeResult(
        ...
        public virtual ObjectResult<TEntity> Translate<TEntity>(DbDataReader reader, string entitySetName, MergeOption mergeOption)
        {
            ...
            var shaperFactory = InternalTranslate<TEntity>(
                reader, entitySetName, mergeOption, /*streaming:*/ false, out var entitySet, out var edmType);
            return ShapeResult(
            ...
            DbDataReader reader, string entitySetName, MergeOption mergeOption, bool streaming, out EntitySet entitySet, out TypeUsage edmType)
        {
            ...
            if (MetadataWorkspace.TryDetermineCSpaceModelType<TElement>(out var modelEdmType)
        ...
        internal void InitializeMappingViewCacheFactory(DbContext owner = null)
        {
            ...
            var contextType = owner is not null ? owner.GetType() : GetType();
