...
namespace System.Data.Entity.Core.Mapping
{
    ...
    public sealed class FunctionImportMappingComposable : FunctionImportMapping
    {
                ...
                Check.NotNull(targetFunction, "targetFunction"))
        {
            ...
            if (!MetadataHelper.TryGetFunctionImportReturnType(functionImport, 0, out
            EdmType resultType))
            {
            ...
            var cTypeTargetFunction =
                containerMapping.StorageMappingItemCollection is not null 
            ...
            {
                Debug.Assert(sTypeTvfElementType is null);
            ...
            : base(functionImport, targetFunction)
        {
            ...
            if (!TypeSemantics.IsScalarType(resultType)
                && (structuralTypeMappings is null || structuralTypeMappings.Count == 0))
            {
            ...
            : base(functionImport, targetFunction)
        {
            ...
            Debug.Assert(
                functionImport.EntitySet is null || structuralTypeMappings is not null,
                "Function import returning entities must have structuralTypeMappings.");
            Debug.Assert(
                structuralTypeMappings is null || structuralTypeMappings.Count > 0, "Non-null structuralTypeMappings must not be empty.");
            Debug.Assert(
                structuralTypeMappings is not null ||
                MetadataHelper.TryGetFunctionImportReturnType(functionImport, 0, out EdmType resultType) && TypeSemantics.IsScalarType(resultType),
                "Either type mappings should be specified or the function import should be Collection(Scalar).");
            Debug.Assert(
                functionImport.EntitySet is null || targetFunctionKeys is not null,
                "Keys must be inferred for a function import returning entities.");
            Debug.Assert(targetFunctionKeys is null || targetFunctionKeys.Length > 0, "Keys must be null or non-empty.");
            ...
            StructuralTypeMappings
        {
            get
            {
                return m_structuralTypeMappings is null
                           ? null
        ...
        {
            if (m_internalTreeNode is null)
            {
                var tree = GenerateFunctionView(out var discriminatorMap);
                Debug.Assert(tree is not null, "tree is not null");
            ...
            Debug.Assert(m_internalTreeNode is not null, "m_internalTreeNode is not null");
        ...
        private sealed class FunctionViewOpCopier : OpCopier
        {
            ...
            public override Node Visit(VarRefOp op, Node n)
            {
                ...
                if (op.Var.VarType == VarType.Parameter
                    && m_viewArguments.TryGetValue(((ParameterVar)op.Var).ParameterName, out var argNode))
                {
        ...
        internal DbQueryCommandTree GenerateFunctionView(out DiscriminatorMap discriminatorMap)
        {
            ...
            Debug.Assert(TargetFunction is not null, "this.TargetFunction is not null");
        ...
        {
            Debug.Assert(
                m_structuralTypeMappings is not null && m_structuralTypeMappings.Count > 0,
                "m_structuralTypeMappings is not null && m_structuralTypeMappings.Count > 0");
            ...
            else
            {
                ...
                {
                    Debug.Assert(discriminatorMap is not null, "discriminatorMap is null after it has been created");
                }
            }
        ...
        {
            Debug.Assert(condition.Property is null, "C-side conditions are not supported in function mappings.");
        ...
        private DbExpression GenerateScalarResultMappingView(DbExpression storeFunctionInvoke)
        {
            ...
            if (!MetadataHelper.TryGetFunctionImportReturnCollectionType(FunctionImport, 0, out var functionImportReturnType))
            {
