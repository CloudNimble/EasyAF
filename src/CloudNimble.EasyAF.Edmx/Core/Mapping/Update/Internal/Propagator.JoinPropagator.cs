...
using System.Linq;
using JoinDictionary = System.Collections.Generic.Dictionary<System.Data.Entity.Core.Mapping.Update.Internal.CompositeKey, System.Tuple<System.Data.Entity.Core.Mapping.Update.Internal.CompositeKey, System.Data.Entity.Core.Mapping.Update.Internal.PropagatorResult>>;
...
* */
                ...
                JoinDictionary rightDeletes, JoinDictionary rightInserts)
            {
                ...
                if (leftInserts.TryGetValue(key, out var leftInsert))
                {
                    input |= Ops.LeftInsert;
                }
                if (leftDeletes.TryGetValue(key, out var leftDelete))
                {
                    input |= Ops.LeftDelete;
                }
                if (rightInserts.TryGetValue(key, out var rightInsert))
                {
                    input |= Ops.RightInsert;
                }
                if (rightDeletes.TryGetValue(key, out var rightDelete))
                {
                ...
                Tuple<CompositeKey, PropagatorResult> left, Tuple<CompositeKey, PropagatorResult> right, ChangeNode result)
            {
                ...
                if (null != map)
                {
                    join = join.Replace(original => map.TryGetValue(original, out var replacement) ? replacement : original);
                }
