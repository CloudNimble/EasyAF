...
namespace System.Data.Entity.Core.Mapping.Update.Internal
{
    ...
    internal class Graph<TVertex>
    {
        ...
        internal void AddEdge(TVertex from, TVertex to)
        {
            ...
            {
                if (!m_successorMap.TryGetValue(from, out var successors))
                {
                ...
                if (successors.Add(to))
                {
                    ...
                    if (!m_predecessorCounts.TryGetValue(to, out var predecessorCount))
                    {
                        predecessorCount = 1;
                    }
                    else
        ...
        internal bool TryTopologicalSort(out IEnumerable<TVertex> orderedVertices, out IEnumerable<TVertex> remainder)
        {
            ...
            {
                if (!m_predecessorCounts.TryGetValue(vertex, out var predecessorCount)
                    || 0 == predecessorCount)
                {
            ...
            while (0 < rootsPriorityQueue.Count)
            {
                ...
                if (m_successorMap.TryGetValue(from, out var toSet))
                {
