    ...
    internal class UpdateCommandOrderer : Graph<UpdateCommand>
    {
        ...
        private void AddServerGenDependencies()
        {
            ...
            foreach (var command in Vertices)
            {
                ...
                {
                    if (predecessors.TryGetValue(input, out var from))
                    {
        ...
        private void AddForeignKeyEdges(KeyToListMap<ForeignKeyValue, UpdateCommand> predecessors)
        {
            foreach (var command in Vertices.OfType<DynamicUpdateCommand>())
            {
                    ...
                    ModificationOperator.Insert == command.Operator)
                {
                    ...
                    {
                        if (ForeignKeyValue.TryCreateSourceKey(fkConstraint, command.CurrentValues, true, out var fk))
                        {
                            ...
                            if (ModificationOperator.Update != command.Operator
                                ||
                                !ForeignKeyValue.TryCreateSourceKey(fkConstraint, command.OriginalValues, true, out var originalFK)
                                ||
                    ...
                    ModificationOperator.Delete == command.Operator)
                {
                    ...
                    {
                        if (ForeignKeyValue.TryCreateTargetKey(fkConstraint, command.OriginalValues, false, out var fk))
                        {
                            ...
                            if (ModificationOperator.Update != command.Operator
                                ||
                                !ForeignKeyValue.TryCreateTargetKey(fkConstraint, command.CurrentValues, false, out var currentFK)
                                ||
        ...
        private KeyToListMap<ForeignKeyValue, UpdateCommand> DetermineForeignKeyPredecessors()
        {
            ...
            foreach (var command in Vertices.OfType<DynamicUpdateCommand>())
            {
                    ...
                    ModificationOperator.Insert == command.Operator)
                {
                    foreach (var fkConstraint in _targetMap.EnumerateValues(command.Table))
                    {
                        if (ForeignKeyValue.TryCreateTargetKey(fkConstraint, command.CurrentValues, true, out var fk))
                        {
                            // if this is an update and the target key is unchanged, there is no 
                            // need to add a dependency (from the perspective of the target, the update
                            // is a no-op)
                            if (ModificationOperator.Update != command.Operator
                                ||
                                !ForeignKeyValue.TryCreateTargetKey(fkConstraint, command.OriginalValues, true, out var originalFK)
                                ||
                                !_keyComparer.Equals(originalFK, fk))
                            {
                    ...
                    ModificationOperator.Delete == command.Operator)
                {
                    foreach (var fkConstraint in _sourceMap.EnumerateValues(command.Table))
                    {
                        if (ForeignKeyValue.TryCreateSourceKey(fkConstraint, command.OriginalValues, false, out var fk))
                        {
                            // if this is an update and the source key is unchanged, there is no 
                            // need to add a dependency (from the perspective of the source, the update
                            // is a no-op)
                            if (ModificationOperator.Update != command.Operator
                                ||
                                !ForeignKeyValue.TryCreateSourceKey(fkConstraint, command.CurrentValues, false, out var currentFK)
                                ||
                                !_keyComparer.Equals(currentFK, fk))
                            {
                                predecessors.Add(fk, command);
