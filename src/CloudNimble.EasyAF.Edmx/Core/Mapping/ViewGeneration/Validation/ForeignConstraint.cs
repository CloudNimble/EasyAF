...
namespace System.Data.Entity.Core.Mapping.ViewGeneration.Validation
{
    ...
    internal class ForeignConstraint : InternalBase
    {
            ...
            ErrorLog errorLog, ConfigViewGenerator config)
        {
            ...
            if (childRewriter is null
                && parentRewriter is null)
            {
            ...
            ErrorLog errorLog, ConfigViewGenerator config)
        {
            ...
            foreach (var cell in cells)
            {
                ...
                Debug.Assert(
                    childEnd is null || parentEnd != childEnd,
                ...
                if (childEnd is not null
                    && parentEnd is not null
            ...
            {
                Debug.Assert(
                    errorListForInvalidParentColumnsForForeignKey is not null && errorListForInvalidParentColumnsForForeignKey.Count > 0);
            ...
            QueryRewriter childRewriter, QueryRewriter parentRewriter)
        {
            ...
            var pNode = parentRewriter.BasicView;
            Debug.Assert(pNode is not null);
            ...
            foreach (var childCell in childCells)
            {
                ...
                foreach (var childSlotNums in allChildSlotNums)
                {
                    ...
                    foreach (var childSlotNum in childSlotNums)
                    {
                        ...
                        Debug.Assert(childSlot is not null);
                    ...
                    foreach (var parentCell in parentCells)
                    {
                        ...
                        foreach (var parentSlotNums in allParentSlotNums)
                        {
                            ...
                            foreach (var parentSlotNum in parentSlotNums)
                            {
                                ...
                                Debug.Assert(parentSlot is not null);
                ...
                Debug.Assert(childPaths is not null, "child paths should be set");
                Debug.Assert(parentPaths is not null, "parent paths should be set");
                Debug.Assert(errorParentCell is not null, "errorParentCell should be set");
                ...
                var record = new ErrorLog.Record(
                    ViewGenErrorCode.ForeignKeyColumnOrderIncorrect, message, [errorParentCell, childCell], String.Empty);
                errorLog.AddEntry(record);
                return false;
            }
