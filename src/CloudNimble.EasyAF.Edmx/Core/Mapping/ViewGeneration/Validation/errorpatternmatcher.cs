...
using System.Linq;
using System.Text;
using CompositeCondition = System.Collections.Generic.Dictionary<System.Data.Entity.Core.Mapping.ViewGeneration.Structures.MemberPath, System.Data.Entity.Core.Common.Utils.Set<System.Data.Entity.Core.Mapping.ViewGeneration.Structures.Constant>>;
                ...
                foreach (var condition in cellQuery.GetConjunctsFromWhereClause())
                {
                        ...
                        == ViewTarget.UpdateView)
                    {
                        if (scalarCond is not null
                            ...
                            && IsMemberPartOfNotNullCondition([leftCellWrapper], memberPath, m_viewgenContext.ViewTarget))
                        {
                            var rightMemberPath = GetRightMemberPath(memberPath, leftCellWrapper);
                            if (rightMemberPath is not null
                                && rightMemberPath.IsNullable
                                &&
                                !IsMemberPartOfNotNullCondition([leftCellWrapper], rightMemberPath, m_viewgenContext.ViewTarget))
                            {
                                m_errorLog.AddEntry(
                                    new ErrorLog.Record(
                    ...
                    foreach (var element in condition.Domain.Values)
                    {
                        ...
                        if (!condMembersValues.TryGetValue(memberPath, out var values))
                        {
                                ...
                                else
                                {
                                    ...
                                    if (fragment1.LeftExtent.Equals(fragment2.LeftExtent))
                                    {
                                        GetTypesAndConditionForWrapper(
                                            fragment1, out var firstCellWrapperHasCondition, out var edmTypesForFirstCellWrapper);
                                        GetTypesAndConditionForWrapper(
                                            fragment2, out var secondCellWrapperHasCondition, out var edmTypesForSecondCellWrapper);
