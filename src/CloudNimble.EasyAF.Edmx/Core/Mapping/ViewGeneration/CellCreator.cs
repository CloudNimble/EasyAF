...
namespace System.Data.Entity.Core.Mapping.ViewGeneration
{
    ...
    internal class CellCreator : InternalBase
    {
        ...
        private void ExpandCells(List<Cell> cells)
        {
            ...
            foreach (var cell in cells)
            {
                foreach (var sSideMemberToExpand in sSideMembersToBeExpanded)
                {
                    ...
                    if (!cSideMembersForSSideExpansionCandidates.TryGetValue(sSideMemberToExpand, out var cSidePaths))
                    {
                        cSidePaths = [];
                ...
                foreach (var memberToExpand in sSideMembersToBeExpanded)
                {
                    ...
                    if (cell.SQuery.GetProjectedMembers().Contains(memberToExpand))
                    {
                        ...
                        if (TryCreateAdditionalCellWithCondition(
                            cell, memberToExpand, true /*condition value*/, ViewTarget.UpdateView /*s-side member*/, out var resultCell))
                        {
                    ...
                    else
                    {
                        ...
                        {
                            if (TryCreateAdditionalCellWithCondition(
                                cell, cMemberToExpand, true /*condition value*/, ViewTarget.QueryView /*c-side member*/, out var resultCell))
                            {
                                cells.Add(resultCell);
                            }
        ...
        private void ExtractCells(List<Cell> cells)
        {
            ...
            foreach (var extentMap in m_containerMapping.AllSetMaps)
            {
                ...
                foreach (var typeMap in extentMap.TypeMappings)
                {
                    ...
                    Debug.Assert(
                        extent is not null, "Extent map for a null extent or type of extentMap.Exent " +
                                        "is not Extent");
            ...
            Set<EdmType> allTypes, List<Cell> cells)
        {
            ...
            Debug.Assert((fragmentInfo is not null), "CSMappingFragment should support Line Info");
            ...
            ref BoolExpression sQueryWhereClause)
        {
            ...
            foreach (var propMap in properties)
            {
                ...
                Debug.Assert(
                    scalarPropMap is not null ||
                    complexPropMap is not null ||
                    associationEndPropertypMap is not null ||
                    conditionMap is not null, "Unimplemented property mapping");

                if (scalarPropMap is not null)
                {
                    Debug.Assert(scalarPropMap.Column is not null, "ColumnMember for a Scalar Property can not be null");
                ...
                if (associationEndPropertypMap is not null)
                {
                ...
                {
                    if (conditionMap.Column is not null)
                    {
                    ...
                    {
                        Debug.Assert(conditionMap.Property is not null);
                        //Produce a Condition Expression for the Condition Map.
        ...
        private static BoolExpression GetConditionExpression(MemberPath member, ConditionPropertyMapping conditionMap)
        {
            ...
            EdmMember conditionMember = (conditionMap.Column is not null) ? conditionMap.Column : conditionMap.Property;
            ...
            Debug.Assert(conditionExpression is not null);
        ...
        private static bool IsBooleanMember(MemberPath path)
        {
            ...
            return (primitive is not null && primitive.PrimitiveTypeKind == PrimitiveTypeKind.Boolean);
        }
