    ...
    internal class DiscriminatorMap
    {
        ...
        internal static bool TryCreateDiscriminatorMap(EntitySet entitySet, DbExpression queryView, out DiscriminatorMap discriminatorMap)
        {
            ...
            for (var i = 0; i < caseExpression.When.Count; i++)
            {
                ...
                if (
                    !ViewSimplifier.TryMatchPropertyEqualsValue(
                        when, projectionVariableName, out var currentDiscriminator, out var discriminatorValue))
                {
                    return false;
                }
                ...
                if (!TryMatchEntityTypeConstructor(then, propertyMap, relPropertyMap, typeToRelPropertyMap, out var currentType))
                {
                    return false;
                }
            ...
            if (null == caseExpression.Else
                ||
                !TryMatchEntityTypeConstructor(caseExpression.Else, propertyMap, relPropertyMap, typeToRelPropertyMap, out var elseType))
            {
                return false;
            }
            ...
            out EntityType entityType)
        {
            ...
            for (var j = 0; j < entityType.Properties.Count; j++)
            {
                ...
                if (propertyMap.TryGetValue(property, out var existingAssignment))
                {
            ...
            {
                if (!typeToRelPropertyMap.TryGetValue(entityType, out var relPropertyList))
                {
                    relPropertyList = [];
                ...
                foreach (var relatedRef in constructor.RelatedEntityReferences)
                {
                    ...
                    if (relPropertyMap.TryGetValue(relProperty, out var existingAssignment))
                    {
                        if (!ExpressionsCompatible(assignment, existingAssignment))
                        {
                            return false;
