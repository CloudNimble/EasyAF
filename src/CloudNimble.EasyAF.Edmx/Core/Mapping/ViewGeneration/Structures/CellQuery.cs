...
using System.Linq;
using System.Text;
using AttributeSet = System.Data.Entity.Core.Common.Utils.Set<System.Data.Entity.Core.Mapping.ViewGeneration.Structures.MemberPath>;
...
namespace System.Data.Entity.Core.Mapping.ViewGeneration.Structures
{
    ...
    internal class CellQuery : InternalBase
    {
        ...
        internal CellQuery(List<ProjectedSlot> slots, BoolExpression whereClause, MemberPath rootMember, SelectDistinct eliminateDuplicates)
            : this(slots.ToArray(), whereClause, [], eliminateDuplicates, rootMember)
        {
        }
        ...
        internal EntitySetBase Extent
        {
            get
            {
                ...
                Debug.Assert(extent is not null, "JoinTreeRoot in cellquery must be an extent");
                return extent;
            }
        }
        ...
        internal BasicCellRelation BasicCellRelation
        {
            get
            {
                Debug.Assert(m_basicCellRelation is not null, "BasicCellRelation must be created first");
        ...
        internal ErrorLog.Record CheckForDuplicateFields(CellQuery cQuery, Cell sourceCell)
        {
            ...
            for (var i = 0; i < m_projectedSlots.Length; i++)
            {
                ...
                Debug.Assert(slot is not null, "All slots for this method must be JoinTreeSlots");
                slotMap.Add(slot, i);
            }
        ...
        internal ErrorLog.Record CheckForProjectedNotNullSlots(Cell sourceCell, IEnumerable<Cell> associationSets)
        {
            ...
            foreach (var restriction in Conditions)
            {
                if (restriction.Domain.ContainsNotNull())
                {
                    ...
                    if (slot is null) //member with not null condition is not mapped in this extent
                    {
        ...
        {
            Debug.Assert(m_projectedSlots[slotNumber] is null, "Another attempt to plug in a default value");
            ...
            out CellQuery newMainQuery, out CellQuery newOtherQuery)
        {
            ...
            for (var i = 0; i < m_projectedSlots.Length; i++)
            {
                ...
                Debug.Assert(slot is not null, "All slots during cell normalization must field slots");
        ...
        internal AttributeSet GetNonNullSlots()
        {
            ...
            foreach (var projectedSlot in m_projectedSlots)
            {
                ...
                if (projectedSlot is not null)
                {
                    var projectedVar = projectedSlot as MemberProjectedSlot;
                    Debug.Assert(projectedVar is not null, "Projected slot must not be a constant");
        ...
        internal List<MemberProjectedSlot> GetAllQuerySlots()
        {
            ...
            {
                new MemberProjectedSlot(SourceExtentMemberPath)
            };
            foreach (var restriction in Conditions)
            {
        ...
        internal List<int> GetProjectedPositions(IEnumerable<MemberPath> paths)
        {
            ...
            foreach (var member in paths)
            {
                ...
                Debug.Assert(slotIndexes is not null);
        ...
        internal void UpdateWhereClause(MemberDomainMap domainMap)
        {
            ...
            foreach (var atom in WhereClause.Atoms)
            {
                ...
                Debug.Assert(restriction is not null, "All bool literals must be OneOfConst at this point");
        ...
        private static IEnumerable<MemberRestriction> GetConjunctsFromWhereClause(BoolExpression whereClause)
        {
            foreach (var boolExpr in whereClause.Atoms)
            {
                ...
                Debug.Assert(result is not null, "Atom must be restriction");
                yield return result;
            }
        }
