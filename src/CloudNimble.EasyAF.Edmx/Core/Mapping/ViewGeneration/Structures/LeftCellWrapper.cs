    ...
    internal class LeftCellWrapper : InternalBase
    {
        ...
        internal EntitySetBase RightExtent
        {
            get
            {
                ...
                Debug.Assert(result is not null, "Bad root value in join tree");
                return result;
            }
        }
        ...
        private Set<AssociationEndMember> GetEndsForTablePrimaryKey()
        {
            ...
            foreach (var keySlot in m_memberMaps.ProjectedSlotMap.KeySlots)
            {
                ...
                Debug.Assert(endMember is not null, "Element in path before scalar path is not end property?");
                result.Add(endMember);
            }
            Debug.Assert(result is not null, "No end found for keyslots of table?");
            return result;
        }
