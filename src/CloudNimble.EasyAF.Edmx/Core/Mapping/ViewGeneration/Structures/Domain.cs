...
using System.Linq;
using System.Text;
using CellConstantSet = System.Data.Entity.Core.Common.Utils.Set<System.Data.Entity.Core.Mapping.ViewGeneration.Structures.Constant>;
...
namespace System.Data.Entity.Core.Mapping.ViewGeneration.Structures
{
    ...
    internal class Domain : InternalBase
    {
        ...
        internal Domain(Constant value, IEnumerable<Constant> possibleDiscreteValues)
            :
                this([value], possibleDiscreteValues)
        {
        }
        ...
        private Set<Constant> AllPossibleValuesInternal
        {
            get
            {
                ...
                return m_possibleValues.Union([negatedPossibleValue]);
            }
        }
        ...
        private static CellConstantSet DeriveDomainFromType(EdmType type, EdmItemCollection edmItemCollection, bool leaveDomainUnbounded)
        {
            ...
            Debug.Assert(domain is not null, "Domain not set up for some type");
            return domain;
        }
        ...
        {
            if (!TryGetDefaultValueForMemberPath(memberPath, out var defaultValue))
            {
        ...
        internal bool ContainsNotNull()
        {
            ...
            return negated is not null && negated.Contains(Constant.Null);
        }
        ...
        private static CellConstantSet DeterminePossibleValues(IEnumerable<Constant> domain)
        {
            ...
            foreach (var constant in domain)
            {
                ...
                if (negated is not null)
                {
                    ...
                    {
                        Debug.Assert(constElement as NegatedConstant is null, "Negated cell constant inside NegatedCellConstant");
            ...
            IEnumerable<Cell> cells, EdmItemCollection edmItemCollection, bool isValidationEnabled)
        {
            ...
            foreach (var cell in cells)
            {
                ...
                foreach (var restriction in cQuery.GetConjunctsFromWhereClause())
                {
                    ...
                    var found = cDomainMap.TryGetValue(slot.MemberPath, out var values);
                    if (!found)
                    {
            ...
            ComputeConstantDomainSetsForSlotsInUpdateViews(IEnumerable<Cell> cells, EdmItemCollection edmItemCollection)
        {
            ...
            foreach (var cell in cells)
            {
                ...
                foreach (var sSlot in sQuery.GetConjunctsFromWhereClause().Select(oneOfConst => oneOfConst.RestrictedMemberSlot))
                {
                    ...
                    var wasDomainRestricted = GetRestrictedOrUnrestrictedDomain(sSlot, sQuery, edmItemCollection, out var restrictedDomain);
                    ...
                    if (!wasDomainRestricted)
                    {
                        ...
                        if (projectedPosition >= 0)
                        {
                            ...
                            Debug.Assert(cSlot is not null, "Assuming constants are not projected");
                    ...
                    if (TryGetDefaultValueForMemberPath(sSlotMemberPath, out var defaultValue))
                    {
                        restrictedDomain.Add(defaultValue);
                    }

                    // add all constants appearing in the domain to sDomainMap
                    if (!updateDomainMap.TryGetValue(sSlotMemberPath, out var sSlotDomain))
                    {
        ...
        private static NegatedConstant GetNegatedConstant(IEnumerable<Constant> constants)
        {
            ...
            foreach (var constant in constants)
            {
                ...
                if (negated is not null)
                {
                    Debug.Assert(result is null, "Multiple negated cell constants?");
                    result = negated;
                }
            }
            return result;
        ...
        internal void AssertInvariant()
        {
            ...
            Debug.Assert(negated is null, "m_possibleValues cannot contain negated constant");
