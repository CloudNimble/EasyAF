...
namespace System.Data.Entity.Core.Mapping.ViewGeneration.Structures
{
    ...
    internal class MemberDomainMap : InternalBase
    {
        ...
        private readonly Set<MemberPath> m_projectedConditionMembers = [];
            ...
            ConfigViewGenerator config, Dictionary<EntityType, Set<EntityType>> inheritanceGraph)
        {
            ...
            foreach (var cell in extentCells)
            {
                ...
                foreach (var condition in cellQuery.GetConjunctsFromWhereClause())
                {
                    ...
                    if (!domainMap.TryGetValue(memberPath, out var domainValues))
                    {
        ...
        {
            var found = m_conditionDomainMap.TryGetValue(path, out var result);
            if (!found)
            {
        ...
        {
            if (false == m_conditionDomainMap.TryGetValue(member, out var possibleValues))
            {
