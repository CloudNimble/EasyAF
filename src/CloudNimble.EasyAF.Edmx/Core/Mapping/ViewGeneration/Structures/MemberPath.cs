    ...
    internal sealed class MemberPath : InternalBase, IEquatable<MemberPath>
    {
        ...
        internal object DefaultValue
        {
            get
            {
                ...
                if (LeafEdmMember.TypeUsage.Facets.TryGetValue(DbProviderManifest.DefaultValueFacetName, false, out var facet))
                {
                    return facet.Value;
                }
                return null;
        ...
        internal bool IsAlwaysDefined(Dictionary<EntityType, Set<EntityType>> inheritanceGraph)
        {
            ...
            if (entitySetType is null) //association type
            {
                return true;
            }
            ...
            Dictionary<EntityType, Set<EntityType>> inheritanceGraph)
        {
            ...
            foreach (var edge in edges.Where(type => !type.EdmEquals(avoidEdge)))
            {
                ...
                if (entitySetType.BaseType is not null
        ...
        internal void AsCql(Action<string> accessMember, Action getKey, Action<StructuralType> treatAs)
        {
            ...
            foreach (var member in m_path)
            {
                ...
                else if (false == found)
                {
                    ...
                    Debug.Assert(prevRefType is null, "We do not allow subtyping in key extraction from Refs");
        ...
        internal bool IsEquivalentViaRefConstraint(MemberPath path1)
        {
            ...
            if (assocSet0 is not null
                && assocSet1 is not null)
            {
            ...
            else if (entitySet0 is not null
                     && entitySet1 is not null)
            {
            ...
            else
            {
                ...
                var assocSet = assocSet0 is not null ? assocSet0 : assocSet1;
                var entitySet = entitySet0 is not null ? entitySet0 : entitySet1;
                Debug.Assert(
                    assocSet is not null && entitySet is not null,
