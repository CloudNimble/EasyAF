    ...
    internal class RewritingValidator
    {
        ...
        internal void Validate()
        {
            ...
            foreach (var wrapper in _viewgenContext.AllWrappersForExtent)
            {
                ...
                Debug.Assert(complementSQueryTreeForCondition is not null, "Rewriting for S-side query is unsatisfiable");
                if (complementSQueryTreeForCondition is null)
                {
                ...
                Debug.Assert(plainSQueryTreeForCondition is not null, "Rewriting for S-side query is unsatisfiable");
                if (plainSQueryTreeForCondition is not null)
                {
        ...
        private Dictionary<MemberValueBinding, CellTreeNode> CreateMemberValueTrees(bool complementElse)
        {
            ...
            foreach (var column in _domainMap.ConditionMembers(_viewgenContext.Extent))
            {
                ...
                for (var i = 0; i < domain.Count; i++)
                {
                    ...
                    if (_viewgenContext.TryGetCachedRewriting(memberConditionQuery, out var rewriting))
                    {
            ...
            BoolExpression inExtentCondition)
        {
            ...
            foreach (var column in _domainMap.ConditionMembers(_viewgenContext.Extent))
            {
                ...
                if (slot is not null)
                {
                    ...
                    {
                        if (memberValueTrees.TryGetValue(new MemberValueBinding(column, domainValue), out var sQueryTreeForDomainValue))
                        {
                                ...
                                out var unsatisfiedConstraint))
                            {
                                ...
                                ReportConstraintViolation(
                                    ...
                                    sCombinedTree.GetLeaves().Concat([wrapper]));
                            }
                        }
                    }
                }
            ...
            MemberMaps memberMaps)
        {
            ...
            Debug.Assert(constant is ScalarConstant || constant.IsNull() || negatedConstant is not null, "Invalid type of constant");
            ...
            if (negatedConstant is not null)
            {
        ...
        private void CheckConstraintsOnNonNullableMembers(LeftCellWrapper wrapper)
        {
            ...
            {
                var isColumnSimpleType = (column.EdmType as SimpleType) is not null;
