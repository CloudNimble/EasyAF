...
namespace System.Data.Entity.Core.Mapping.ViewGeneration.QueryRewriting
{
    ...
    internal class QueryRewriter
    {
        ...
        private readonly List<FragmentQuery> _fragmentQueries = [];
        private readonly List<Tile<FragmentQuery>> _views = [];

        private readonly FragmentQuery _domainQuery;
        private readonly EdmType _generatedType;
        private readonly HashSet<FragmentQuery> _usedViews = [];
        private List<LeftCellWrapper> _usedCells = [];
        private BoolExpression _topLevelWhereClause;
        private CellTreeNode _basicView;
        private Dictionary<MemberPath, CaseStatement> _caseStatements = [];
        ...
        private void RemoveUnusedValueFromStoreDomain(Constant domainValue, MemberPath currentPath)
        {
            ...
            if (FindRewritingAndUsedViews(_keyAttributes, domainWhereClause, outputUsedViews, out var caseRewriting))
            {
            ...
            ErrorLog errorLog)
        {
            foreach (var domainValue in GetDomain(currentPath))
            {
                ...
                if (false == FindRewritingAndUsedViews(_keyAttributes, domainWhereClause, outputUsedViews, out var rewriting))
                {
                    ...
                    if (typeConstant is not null)
                    {
                        ...
                        if (nonConditionalAttributes.Count > 0
                            ...
                            !FindRewritingAndUsedViews(
                                nonConditionalAttributes, domainWhereClause, outputUsedViews, out rewriting, out var notCoverdAttributes))
                        {
                      ...
                      domain.Contains(Constant.NotNull, Constant.EqualityComparer));
                {
                    ...
                    foreach (var domainValue in domain)
                    {
                        ...
                        if (FindRewritingAndUsedViews(
                            memberConditionQuery.Attributes, memberConditionQuery.Condition, outputUsedViews, out var caseRewriting))
                        {
                            if (_context.ViewTarget
                                == ViewTarget.UpdateView)
                            {
                                unionCaseRewriting = (unionCaseRewriting is not null)
            ...
            CellTreeNode rightDomainQuery, Tile<FragmentQuery> unionCaseRewriting)
        {
            ...
            var hasDefaultValue = Domain.TryGetDefaultValueForMemberPath(currentPath, out var defaultValue);

            if (false == hasDefaultValue
                || false == domain.Contains(defaultValue))
            {
                Debug.Assert(unionCaseRewriting is not null, "No union of rewritings for case statements");
        ...
        private BoolExpression GetTopLevelWhereClause(HashSet<FragmentQuery> outputUsedViews)
        {
                ...
                == ViewTarget.QueryView)
            {
                ...
                {
                    if (FindRewritingAndUsedViews(_keyAttributes, _domainQuery.Condition, outputUsedViews, out var topLevelRewriting))
                    {
        ...
        internal void EnsureExtentIsFullyMapped(HashSet<FragmentQuery> outputUsedViews)
        {
            ...
            else
            {
                if (_config.IsValidationEnabled)
                {
                    ...
                    foreach (var memberPath in _context.MemberMaps.ProjectedSlotMap.Members)
                    {
                        if (memberPath.IsScalarType()
                            ...
                            &&
                            !Domain.TryGetDefaultValueForMemberPath(memberPath, out var defaultConstant))
                        {
                            ...
                            foreach (var leftCellWrapper in _context.AllWrappersForExtent)
                            {
                                ...
                                if (
                                    !RewriteQuery(
                                        CreateTile(tileQuery), noNullToAvoid, /*_views,*/ out var noNullRewriting, out var notCoveredAttributes,
                                        false /* isRelaxed */))
                                {
                                    // force error
                                    Domain.GetDefaultValueForMemberPath(memberPath, [leftCellWrapper], _config);
                                }
                            }
                        }
                    }
                ...
                foreach (var toFill in _views)
                {
                    ...
                    var found = RewriteQuery(toFill, toAvoid, out var rewriting, out var notCoveredAttributes, true /* isRelaxed */);
        ...
        private List<LeftCellWrapper> RemapFromVariables()
        {
            ...
            if (literalRemap.Count > 0)
            {
                ...
                foreach (var entry in _caseStatements)
                {
                    ...
                    Debug.Assert(entry.Value.ElseValue is null);
            ...
            bool isRelaxed)
        {
            ...
            else if (isRelaxed)
            {
                ...
                foreach (var attribute in NonKeys(toFillQuery.Attributes))
                {
                    if (attributeConditions.TryGetValue(attribute, out var remainingCondition))
                    {
            ...
            Dictionary<MemberPath, FragmentQuery> attributeConditions)
        {
            ...
            foreach (var view in _fragmentQueries)
            {
                foreach (var projectedAttribute in NonKeys(view.Attributes))
                {
                    ...
                    {
                        attributeTile = (attributeTile is null) ? CreateTile(view) : _qp.Union(attributeTile, CreateTile(view));
                    }
                }
                if (attributeConditions.Count == 0)
                {
            ...
            if (attributeConditions.Count == 0)
            {
                ...
                Debug.Assert(attributeTile is not null);
        ...
        {
            if (attributeConditions.TryGetValue(projectedAttribute, out var currentAttributeCondition))
            {
        ...
        private IEnumerable<Tile<FragmentQuery>> GetRelevantViews(FragmentQuery query)
        {
            ...
            if (firstTrueView is not null) // the collected views don't give us True, but 
            {
            ...
            IEnumerable<MemberPath> keyAttributes, MemberDomainMap domainMap)
        {
            ...
            if (domainValue is NegatedConstant)
            {
                ...
                attributes = keyAttributes.Concat([currentPath]);
            }
        ...
        private static BoolExpression TileToBoolExpr(Tile<FragmentQuery> tile)
        {
            switch (tile.OpKind)
            {
                case TileOpKind.Named:
                    ...
                    {
                        Debug.Assert(view.FromVariable is not null);
        ...
        private static bool IsDefaultValue(Constant domainValue, MemberPath path)
        {
            ...
            if (path.DefaultValue is not null)
            {
        ...
        {
            qp.GetStatistics(out var numSATChecks, out var numIntersection, out var numUnion, out var numDifference, out var numErrors);
            TraceVerbose(
