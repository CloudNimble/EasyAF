...
namespace System.Data.Entity.Core.Mapping.ViewGeneration
{
    internal abstract class ViewgenGatekeeper : InternalBase
    {
            ...
            out bool success)
        {
            ...
            if (containerMapping.GetEntitySetMapping(entity.Name).QueryView is not null)
            {
            ...
            foreach (var cellGroup in cellGroups)
            {
                ...
                Debug.Assert(viewGenerator is not null); //make sure there is no exception thrown that does not add error to log
            ...
            EntityContainerMapping containerMapping)
        {
            ...
            foreach (var cellGroup in cellGroups)
            {
                ...
                if (groupErrorLog.Count == 0)
                {
                    Debug.Assert(viewGenerator is not null);
            ...
            EntityContainerMapping containerMapping)
        {
            ...
            if (missingExtents.Count > 0)
            {
                ...
                Debug.Assert(smallestCell is not null && lowestLineNum >= 0);
