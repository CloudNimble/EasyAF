...
using Triple =
    System.Data.Entity.Core.Common.Utils.Pair<System.Data.Entity.Core.Metadata.Edm.EntitySetBase, System.Data.Entity.Core.Common.Utils.Pair<System.Data.Entity.Core.Metadata.Edm.EntityTypeBase, bool>>;
...
namespace System.Data.Entity.Core.Mapping
{
    ...
    internal class MappingItemLoader
    {
            ...
            Dictionary<EdmMember, KeyValuePair<TypeUsage, TypeUsage>> scalarMemberMappings)
        {
            ...
            if (m_currentNamespaceUri is not null)
            {
        ...
        private EntityContainerMapping LoadEntityContainerMapping(XPathNavigator nav)
        {
            ...
            else
            {
                ...
                if (entityContainerType is null)
                {
                    AddToSchemaErrorsWithMemberInfo(
                ...
                if (storageEntityContainerType is null)
                {
                    AddToSchemaErrorsWithMemberInfo(
                ...
                if ((entityContainerType is null)
                    || (storageEntityContainerType is null))
                {
                    return null;
                }
        ...
        private void ValidateQueryViewsClosure(XPathNavigator nav, EntityContainerMapping entityContainerMapping)
        {
            ...
            foreach (var setMapping in entityContainerMapping.AllSetMaps)
            {
                if (setMapping.QueryView is not null)
                {
            ...
            out Set<EntityType> entityTypes)
        {
            ...
            foreach (var name in entityTypeNames)
            {
                ...
                if (!EdmItemCollection.TryGetItem(entityTypeName, out
            ...
            EntityTypeMapping entityTypeMapping)
        {
            ...
            foreach (var parameterBinding in parameterList)
            {
                if (null != parameterBinding.MemberPath.AssociationSetEnd)
                {
                    ...
                    if (associationEnds.TryGetValue(associationSet, out var existingEnd)
                        &&
            ...
            XPathNavigator nav, EntityContainerMapping entityContainerMapping, EntityContainer storageEntityContainerType)
        {
            ...
            entityContainerMapping.EdmEntityContainer.TryGetRelationshipSetByName(
                associationSetName, false /*ignoreCase*/, out var relationshipSet);
        ...
        private void LoadFunctionImportMapping(XPathNavigator nav, EntityContainerMapping entityContainerMapping)
        {
            ...
            if (!TryGetFunctionImportStoreFunction(nav, out var targetFunction))
            {
                return;
            }

            // Get source (model) function
            if (!TryGetFunctionImportModelFunction(nav, entityContainerMapping, out var functionImport))
            {
                return;
            }
            ...
            if (functionImport.IsComposableAttribute)
            {
                ...
                if (cTypeTvfElementType is null)
                {
                    Debug.Assert(sTypeTvfElementType is null, "sTypeTvfElementType is null");
                    AddToSchemaErrors(
                ...
                var typeMappings = typeMappingsList.Count > 0 ? typeMappingsList[0] : [];

                FunctionImportMappingComposable mapping = null;
                if (MetadataHelper.TryGetFunctionImportReturnType(functionImport, 0, out EdmType resultType))
                {
                ...
                Debug.Assert(mapping is not null, "mapping is not null");
            ...
            else
            {
                ...
                {
                    if (MetadataHelper.TryGetFunctionImportReturnType(functionImport, i, out EntityType returnEntityType)
                        &&
            ...
            out EdmFunction functionImport)
        {
            ...
            if (entityContainerMapping.TryGetFunctionImportMapping(functionImport, out var targetFunctionCollision))
            {
                AddToSchemaErrorWithMessage(
        ...
        private void ValidateFunctionImportMappingParameters(XPathNavigator nav, EdmFunction targetFunction, EdmFunction functionImport)
        {
            ...
            foreach (var targetParameter in targetFunction.Parameters)
            {
                ...
                else
                {
                    ...
                    Debug.Assert(importType is not null, "Function import parameters must be primitive.");
            ...
            foreach (var importParameter in functionImport.Parameters)
            {
                ...
                if (!targetFunction.Parameters.TryGetValue(importParameter.Name, false, out var targetParameter))
                {
                    AddToSchemaErrorWithMessage(
            ...
            int resultSetIndex)
        {
            ...
            if (nav.MoveToChild(XPathNodeType.Element))
            {
                do
                {
                    ...
                    {
                        if (MetadataHelper.TryGetFunctionImportReturnType(functionImport, resultSetIndex, out EntityType resultEntityType))
                        {
                    ...
                    {
                        if (MetadataHelper.TryGetFunctionImportReturnType(functionImport, resultSetIndex, out ComplexType resultComplexType))
                        {
                            Debug.Assert(entitySet is null, "entitySet is null for complex type mapping in function imports.");

                            if (TryLoadFunctionImportComplexTypeMapping(nav.Clone(), resultComplexType, functionImport, out var typeMapping))
                            {
                                typeMappings.Add(typeMapping);
                            }
                        }
            ...
            EntityContainer storageEntityContainerType)
        {
            ...
            EdmItemCollection.TryGetItem(associationTypeName, out
            //Get the association type for association type name specified in MSL
            //If no AssociationType with the given name exists, add a schema error and return
            AssociationType associationType);
            if (associationType is null)
            {
            ...
            if (String.IsNullOrEmpty(tableName)
                && (associationSetMapping.QueryView is null))
            {
                AddToSchemaErrors(
            ...
            bool distinctFlag)
        {
            ...
            if (typeMapping.SetMapping.QueryView is not null)
            {
                AddToSchemaErrorsWithMemberInfo(
            ...
            storageEntityContainerType.TryGetEntitySetByName(tableName, false /*ignoreCase*/, out var tableMember);
            if (tableMember is null)
            {
            ...
            XPathNavigator nav, EdmType containerType, ReadOnlyMetadataCollection<EdmProperty> tableProperties)
        {
            ...
            if (!String.IsNullOrEmpty(propertyName))
            {
                    ...
                    || !(Helper.IsCollectionType(containerType)))
                {
                    ...
                    if (containerType is not null)
                    {
                        ...
                        {
                            (containerType as StructuralType).Members.TryGetValue(propertyName, false, out var tempMember);
            ...
            var columnName = GetAliasResolvedAttributeValue(nav.Clone(), MslConstructs.ScalarPropertyColumnNameAttribute);
            Debug.Assert(columnName is not null, "XSD validation should have caught this");
            tableProperties.TryGetValue(columnName, false, out var columnMember);
            if (columnMember is null)
            {
                AddToSchemaErrorsWithMemberInfo(
            ...
            XPathNavigator nav, EdmType containerType, ReadOnlyMetadataCollection<EdmProperty> tableProperties)
        {
            ...
            if (String.IsNullOrEmpty(memberTypeName))
            {
                ...
                {
                    if (containerStructuralType is not null)
                    {
                        containerStructuralType.Members.TryGetValue(propertyName, false /*ignoreCase*/, out var tempMember);
                        member = tempMember as EdmProperty;
                        if (member is null)
                        {
                            AddToSchemaErrorsWithMemberInfo(
            ...
            XPathNavigator nav, EdmType type, ReadOnlyMetadataCollection<EdmProperty> tableType)
        {
            ...
            else
            {
                ...
                do
                {
                    ...
                    EdmItemCollection.TryGetItem(currentTypeName, out ComplexType complexType);
            ...
            do
            {
                switch (nav.LocalName)
                {
                    case MslConstructs.ScalarPropertyElement:
                        ...
                        Debug.Assert(endRef is not null);
            ...
            XPathNavigator nav, EdmType containerType, ReadOnlyMetadataCollection<EdmProperty> tableProperties)
        {
            ...
            if (propertyName is not null)
            {
                ...
                if (containerType is not null)
                {
                    ((StructuralType)containerType).Members.TryGetValue(propertyName, false /*ignoreCase*/, out var tempMember);
                    member = tempMember as EdmProperty;
                }
            }
            ...
            var conditionMember = (columnMember is not null) ? columnMember : member;
            if (conditionMember is null)
            {
                AddToSchemaErrorsWithMemberInfo(
                    Strings.Mapping_InvalidContent_ConditionMapping_InvalidMember, ((columnName is not null) ? columnName : propertyName),
                    MappingErrorCode.ConditionError, m_sourceLocation, navLineInfo, m_parsingErrors);
                return null;
            }
            ...
            if (Helper.IsPrimitiveType(edmType))
            {
                ...
                Debug.Assert(clrMemberType is not null, "Scalar Types should have associated clr type");
            ...
            //Either Value or NotNull need to be specifid on the condition mapping but not both
            if ((isNullAttribute is not null)
                && (value is not null))
            {
                AddToSchemaErrors(
                    Strings.Mapping_InvalidContent_ConditionMapping_Both_Values,
            ...
            XPathNavigator nav, string attributeName, EnumType enumType, string sourceLocation, IList<EdmSchemaError> parsingErrors)
        {
            ...
            var found = enumType.Members.TryGetValue(value, false, out var result);
            if (!found)
            {
                AddToSchemaErrorsWithMemberInfo(
        ...
        private string GetAliasResolvedValue(string aliasedString)
        {
            ...
            m_alias.TryGetValue(aliasKey, out var aliasValue);
        ...
        private void ValidateAndUpdateScalarMemberMapping(EdmProperty member, EdmProperty columnMember, IXmlLineInfo lineInfo)
        {
            ...
            if (!m_scalarMemberMappings.TryGetValue(member, out var memberMappingInfo))
            {
                ...
                if (storeEquivalentTypeUsage is null)
                {
        ...
        private class ModificationFunctionMappingLoader
        {
                    ...
                    do
                    {
                            ...
                            == MslConstructs.ResultBindingElement)
                        {
                            ...
                            if (null == propertyName
                                ||
                                !entityType.Properties.TryGetValue(propertyName, false, out var property))
                            {
                            ...
                            case MslConstructs.ComplexPropertyElement:
                                {
                                    var property = LoadComplexTypeProperty(
                                        nav.Clone(), type, out var complexType);
            ...
            private AssociationSetEnd LoadAssociationEnd(XPathNavigator nav)
            {
                ...
                if (null == associationSetName
                    ||
                    !m_modelContainer.TryGetRelationshipSetByName(associationSetName, false, out var relationshipSet)
                    ||
                ...
                if (null == fromRole
                    ||
                    !associationSet.AssociationSetEnds.TryGetValue(fromRole, false, out var fromEnd))
                {
                    AddToSchemaErrorsWithMemberInfo(
                ...
                if (null == toRole
                    ||
                    !associationSet.AssociationSetEnds.TryGetValue(toRole, false, out var toEnd))
                {
                    AddToSchemaErrorsWithMemberInfo(
                        Strings.Mapping_ModificationFunction_AssociationSetRoleDoesNotExist,
            ...
            private AssociationSetEnd LoadEndProperty(XPathNavigator nav)
            {
                ...
                if (null == role
                    ||
                    !m_associationSet.AssociationSetEnds.TryGetValue(role, false, out var end))
                {
                    AddToSchemaErrorsWithMemberInfo(
                        Strings.Mapping_ModificationFunction_AssociationSetRoleDoesNotExist,
            ...
            private EdmMember LoadComplexTypeProperty(XPathNavigator nav, StructuralType type, out ComplexType complexType)
            {
                ...
                if (null == propertyName
                    ||
                    !type.Members.TryGetValue(propertyName, false, out var property))
                {
                ...
                XPathNavigator nav, StructuralType type, bool restrictToKeyMembers)
            {
                ...
                if (null == parameterName
                    ||
                    !m_function.Parameters.TryGetValue(parameterName, false, out var parameter))
                {
                    ...
                    == BuiltInTypeKind.AssociationEndMember)
                {
                    ...
                    if (associationType.IsForeignKey)
                    {
                        ...
                        if (constraint.FromRole == targetEnd)
                        {
                            ...
                            members = [constraint.ToProperties[ordinal],];
