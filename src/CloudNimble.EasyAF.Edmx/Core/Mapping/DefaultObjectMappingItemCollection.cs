    ...
    internal class DefaultObjectMappingItemCollection : MappingItemCollection
    {
            ...
            : base(DataSpace.OCSpace)
        {
            ...
            foreach (var type in cspaceTypes)
            {
                ...
                Debug.Assert(ospaceType is not null, "all primitive type must have been loaded");
        ...
        private Dictionary<string, int> _clrTypeIndexes = new(StringComparer.Ordinal);

        //Indexes into the type mappings collection based on clr type name
        private Dictionary<string, int> _edmTypeIndexes = new(StringComparer.Ordinal);
        ...
        internal override bool TryGetMap(string identity, DataSpace typeSpace, bool ignoreCase, out MappingBase map)
        {
            ...
            if (typeSpace == DataSpace.CSpace)
            {
                ...
                if (_edmTypeIndexes.TryGetValue(identity, out var index))
                {
                    map = (MappingBase)this[index];
                    return true;
                }

                if (cdmType is not null
                    ||
            ...
            else if (typeSpace == DataSpace.OSpace)
            {
                ...
                if (_clrTypeIndexes.TryGetValue(identity, out var index))
                {
                    map = (MappingBase)this[index];
                    return true;
                }
            ...
            EdmType cdmType, EdmType objectType, DefaultObjectMappingItemCollection ocItemCollection)
        {
            ...
            if (ocItemCollection is not null)
            {
        ...
        private static EdmMember GetObjectMember(EdmMember edmMember, StructuralType objectType)
        {
            ...
            if (!objectType.Members.TryGetValue(edmMember.Name, false /*ignoreCase*/, out var objectMember))
            {
            ...
            DefaultObjectMappingItemCollection ocItemCollection, Dictionary<string, ObjectTypeMapping> typeMappings)
        {
            ...
            if (typeMappings.TryGetValue(edmType.FullName, out var objectTypeMapping))
            {
                return objectTypeMapping;
            }
            ...
            if (ocItemCollection is not null)
            {
                ...
                if (ocItemCollection.ContainsMap(edmType, out var typeMapping))
                {
                    return typeMapping;
                }
            }
        ...
        private bool ContainsMap(GlobalItem cspaceItem, out ObjectTypeMapping map)
        {
            ...
            if (_edmTypeIndexes.TryGetValue(cspaceItem.Identity, out var index))
            {
