...
namespace System.Data.Entity.Core.Mapping
{
    internal class FunctionImportMappingComposableHelper
    {
            ...
            out FunctionImportMappingComposable mapping)
        {
            ...
            {
                if (MetadataHelper.TryGetFunctionImportReturnType(functionImport, 0, out StructuralType resultType))
                {
            ...
            var edmItemCollection =
                _entityContainerMapping.StorageMappingItemCollection is not null
            ...
            if (functionImportKB.MappedEntityTypes.Count > 0)
            {
                ...
                for (var i = 0; i < functionImportKB.MappedEntityTypes.Count; ++i)
                {
                    if (TryConvertToEntityTypeConditionsAndPropertyMappings(
                        ...
                        cTypeTvfElementType,
                        sTypeTvfElementType,
                        lineInfo, out var typeConditions, out var propertyMappings))
                    {
            ...
            {
                if (MetadataHelper.TryGetFunctionImportReturnType(functionImport, 0, out ComplexType resultComplexType))
                {
            ...
            out List<PropertyMapping> propertyMappings)
        {
            ...
            {
                foreach (var condition in entityTypeMapping.ColumnConditions.Where(c => c is not null))
                {
                    if (sTypeTvfElementType.Properties.TryGetValue(condition.ColumnName, false, out var column))
                    {
                        object value;
                        bool? isNull;
                        ...
                        else
                        {
                            ...
                            Debug.Assert(cTypeColumn is not null, "cTypeColumn is not null");
                            Debug.Assert(
                            ...
                            Debug.Assert(cPrimitiveType.ClrEquivalentType is not null, "Scalar Types should have associated clr type");
                            Debug.Assert(
            ...
            out List<PropertyMapping> propertyMappings)
        {
            ...
            foreach (EdmProperty property in TypeHelpers.GetAllStructuralMembers(structuralType))
            {
                ...
                if (functionImportKB.ReturnTypeColumnsRenameMapping.TryGetValue(property.Name, out var columnRenameMapping))
                {
                ...
                columnMappingLineInfo = columnMappingLineInfo is not null && columnMappingLineInfo.HasLineInfo()
                                            ? columnMappingLineInfo
                                            : navLineInfo;

                if (sTypeTvfElementType.Properties.TryGetValue(columnName, false, out var column))
                {
            ...
            out EdmProperty[] keys)
        {
            ...
            {
                if (!TryInferTVFKeysForEntityType((EntityType)typeMapping.Item1, typeMapping.Item3, out var currentKeys))
                {
                    keys = null;
                    return false;
                }
