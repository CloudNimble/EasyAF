    ...
    internal class EntityCommandDefinition : DbCommandDefinition
    {
            ...
            ColumnMapFactory columnMapFactory = null)
        {
            ...
            try
            {
                    ...
                    == commandTree.CommandTreeKind)
                {
                    ...
                    PlanCompiler.Compile(commandTree, out mappedCommandList, out var columnMap, out var columnCount, out _entitySets);
                    _columnMapGenerators = [new ConstantColumnMapGenerator(columnMap, columnCount)];
                ...
                else
                {
                    ...
                    _mappedCommandDefinitions =
                        [
                            storeCommandDefinition
                        ];

                    var firstResultEntitySet = mapping.FunctionImport.EntitySets.FirstOrDefault();
                    if (firstResultEntitySet is not null)
                    {
                        _entitySets = [mapping.FunctionImport.EntitySets.FirstOrDefault()];
            ...
            FunctionImportMappingNonComposable mapping, int resultSetIndex, out IColumnMapGenerator columnMapGenerator)
        {
            ...
            TypeUsage storeResultType;
            {
                ...
                if (MetadataHelper.TryGetFunctionImportReturnType(functionImport, resultSetIndex, out                 StructuralType baseStructuralType))
                {
                ...
                else
                {
                    ...
                    if (returnParameter is not null
                        && returnParameter.TypeUsage is not null)
                    {
        ...
        private static FunctionImportMappingNonComposable GetTargetFunctionMapping(DbFunctionCommandTree functionCommandTree)
        {
            ...
            if (
                !functionCommandTree.MetadataWorkspace.TryGetFunctionImportMapping(
                    functionCommandTree.EdmFunction, out var targetFunctionMapping))
            {
        ...
        internal static void PopulateParameterFromTypeUsage(EntityParameter parameter, TypeUsage type, bool isOutParam)
        {
            ...
            if (type is not null)
            {
                ...
                else if (Helper.IsSpatialType(type, out var primitiveTypeKind))
                {
        ...
        internal virtual DbDataReader Execute(EntityCommand entityCommand, CommandBehavior behavior)
        {
            ...
            // data reader for it.
            if (storeDataReader is not null)
            {
                try
                {
        ...
        private DbCommand PrepareEntityCommandBeforeExecution(EntityCommand entityCommand)
        {
            ...
            if (storeProviderCommand.Parameters is not null)
            {
        ...
        {
            if (_mappedCommandDefinitions is not null)
            {
