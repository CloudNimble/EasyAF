    ...
    public class EntityCommand : DbCommand
    {
        ...
        public override string CommandText
        {
            get
            {
                ...
                if (_commandTreeSetByUser is not null)
                {
        ...
        public override int CommandTimeout
        {
            get
            {
                ...
                if (_connection is not null
                    && _connection.StoreProviderFactory is not null)
                {
        ...
        private bool TryGetEntityCommandDefinitionFromQueryCache(out EntityCommandDefinition entityCommandDefinition)
        {
            ...
            if (!queryCacheManager.TryCacheLookup(queryCacheKey, out entityCommandDefinition))
            {
                ...
                // add to the cache
                if (queryCacheManager.TryLookupAndAdd(new QueryCacheEntry(queryCacheKey, entityCommandDefinition), out var outQueryCacheEntry))
                {
        ...
        private void CheckIfReadyToPrepare()
        {
            ...
            if (_connection.StoreProviderFactory is null
                || _connection.StoreConnection is null)
            {
        ...
        internal virtual Dictionary<string, TypeUsage> GetParameterTypeUsage()
        {
            ...
            foreach (EntityParameter parameter in _parameters)
            {
                ...
                if (parameter.EdmType is null
                    && parameter.DbType == DbType.Object
                    && (parameter.Value is null || parameter.Value is DBNull))
                {
