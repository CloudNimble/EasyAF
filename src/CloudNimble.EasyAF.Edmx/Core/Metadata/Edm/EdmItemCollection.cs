...
namespace System.Data.Entity.Core.Metadata.Edm
{
    ...
    public sealed class EdmItemCollection : ItemCollection
    {
        ...
        private readonly OcAssemblyCache _conventionalOcCache = new();
        ...
        internal static bool IsSystemNamespace(DbProviderManifest manifest, string namespaceName)
        {
            ...
            else
            {
                return (namespaceName == EdmConstants.TransientNamespace ||
                        namespaceName == EdmConstants.EdmNamespace ||
                        namespaceName == EdmConstants.ClrPrimitiveTypeNamespace ||
                        (manifest is not null && namespaceName == manifest.NamespaceName));
            }
        }
            ...
            bool throwOnError)
        {
            ...
            var errorCollection = SchemaManager.ParseAndValidate(
                ...
                dataModelOption, providerManifest, out var schemaCollection);
        ...
        {
            _primitiveTypeMaps.TryGetType(primitiveTypeKind, null, out var type);
            return type;
        }
        ...
        internal DbLambda GenerateFunctionDefinition(EdmFunction function)
        {
            ...
            Debug.Assert(generatedDefinition is not null, "generatedDefinition is not null");
            ...
            out IList<EdmSchemaError> errors)
        {
            ...
            return errors is not null && errors.Count > 0 ? null : edmItemCollection;
        }
    }
}
