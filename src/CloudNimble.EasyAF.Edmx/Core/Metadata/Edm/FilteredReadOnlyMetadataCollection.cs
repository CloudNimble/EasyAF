...
#pragma warning disable 1711 // compiler bug: reports TDerived and TBase as type parameters for non-existing IsReadOnly property
    ...
    */
        ...
        where TBase : MetadataItem
    {
        ...
        {
            if (_source.TryGetValue(identity, false /*ignoreCase*/, out var item))
            {
        ...
        {
            item = null;
            if (_source.TryGetValue(identity, ignoreCase, out var baseTypeItem))
            {
        ...
        {
            if (_source.TryGetValue(value.Identity, false /*ignoreCase*/, out var item))
            {
                if (_predicate(item))
                {
