...
namespace System.Data.Entity.Core.Metadata.Edm
{
    ...
    public class ObjectItemCollection : ItemCollection
    {
        ...
        private readonly CacheForPrimitiveTypes _primitiveTypeMaps = new();
        ...
        private KnownAssembliesSet _knownAssemblies = new();
        ...
        private readonly Dictionary<string, EdmType> _ocMapping = [];

        private object _loaderCookie;
        private readonly object _loadAssemblyLock = new();
        ...
        {
            if (TryGetItem(relationshipName, out AssociationType associationType))
            {
            ...
            Assembly assembly, bool loadReferencedAssemblies, EdmItemCollection edmItemCollection, Action<String> logLoadMessage)
        {
            ...
            // when using an EDMX.
            if (edmItemCollection is not null)
            {
            ...
            lock (LoadAssemblyLock)
            {
                ...
                AssemblyCache.LoadAssembly(
                    ...
                    ref _loaderCookie, out var typesInLoading, out var errors);
        ...
        private static bool TryGetClrType(EdmType objectSpaceType, out Type clrType)
        {
                ...
                || Helper.IsEnumType(objectSpaceType))
            {
                ...
                Debug.Assert(clrType is not null, "ClrType property of ClrEntityType/ClrComplexType/ClrEnumType objects must not be null");
            }
        ...
        internal override PrimitiveType GetMappedPrimitiveType(PrimitiveTypeKind modelType)
        {
            ...
            _primitiveTypeMaps.TryGetType(modelType, null, out var type);
            return type;
        }
