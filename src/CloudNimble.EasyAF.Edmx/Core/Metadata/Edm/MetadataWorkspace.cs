...
namespace System.Data.Entity.Core.Metadata.Edm
{
    ...
    public class MetadataWorkspace
    {
        ...
        private readonly object _schemaVersionLock = new();
            ...
            IEnumerable<string> paths, Func<IEnumerable<Assembly>> wildcardAssemblies, Func<AssemblyName, Assembly> resolveReference)
        {
            ...
            using (var csSpaceReaders = new DisposableCollectionWrapper<XmlReader>(composite.CreateReaders(DataSpace.CSSpace)))
            {
                if (csSpaceReaders.Any()
                    && _itemsCSpace is not null
                    && _itemsSSpace is not null)
                {
            ...
            try
            {
                switch (collection.DataSpace)
                {
                    case DataSpace.CSpace:
                        ...
                        if (_itemsOCSpace is null)
                        {
                            Debug.Assert(_itemsOSpace is not null);
                            _itemsOCSpace =
        ...
        private void CheckAndSetItemCollectionVersionInWorkSpace(ItemCollection itemCollectionToRegister)
        {
            ...
            lock (_schemaVersionLock)
            {
                ...
                {
                    Debug.Assert(itemCollectionType is not null);
        ...
        {
            if (!TryGetItemCollection(DataSpace.CSpace, out var itemCollection))
            {
                itemCollection = null;
            }
        ...
        internal virtual void ImplicitLoadAssemblyForType(Type type, Assembly callingAssembly)
        {
            ...
            if (TryGetItemCollection(DataSpace.OSpace, out var collection))
            {
                ...
                TryGetItemCollection(DataSpace.CSpace, out var itemCollection);
        ...
        internal virtual void ImplicitLoadFromEntityType(EntityType type, Assembly callingAssembly)
        {
            ...
            DebugCheck.NotNull(type);
            if (!TryGetMap(type, DataSpace.OCSpace, out var map))
            {
                ...
                if (ospaceCollection is null
                    || !ospaceCollection.TryGetOSpaceType(type, out var ospaceType))
                {
        ...
        internal virtual ItemCollection GetItemCollection(DataSpace dataSpace, bool required)
        {
            ...
            {
                case DataSpace.CSpace:
                    collection = _itemsCSpace is null ? null : _itemsCSpace.Value;
                    break;
                case DataSpace.OSpace:
                    Debug.Assert(_itemsOSpace is not null);
                ...
                case DataSpace.OCSpace:
                    collection = _itemsOCSpace is null ? null : _itemsOCSpace.Value;
                    break;
                case DataSpace.CSSpace:
                    collection = _itemsCSSpace is null ? null : _itemsCSSpace.Value;
                    break;
                case DataSpace.SSpace:
                    collection = _itemsSSpace is null ? null : _itemsSSpace.Value;
                    break;
                default:
                    if (required)
                    {
        ...
        {
            Debug.Assert(
                edmSpaceType is null || edmSpaceType is StructuralType || edmSpaceType is EnumType,
            ...
            where T : EdmType
        {
            ...
            if (TryGetMap(edmSpaceType, DataSpace.OCSpace, out var map))
            {
                var ocMap = map as ObjectTypeMapping;
                if (ocMap is not null)
                {
                    objectSpaceType = (T)ocMap.ClrType;
                }
            }

            return objectSpaceType is not null;
        }
        ...
        {
            Debug.Assert(
                objectSpaceType is null || objectSpaceType is StructuralType || objectSpaceType is EnumType,
                "Only structural or enum type expected");
            ...
            where T : EdmType
        {
            ...
            if (TryGetMap(objectSpaceType, DataSpace.OCSpace, out var map))
            {
                var ocMap = map as ObjectTypeMapping;
                if (ocMap is not null)
                {
            ...
            return edmSpaceType is not null;
        }
        ...
        {
            return (_itemsCSSpace is not null && _itemsCSSpace.Value is not null) ? _itemsCSSpace.Value.GetUpdateViewLoader() : null;
        }
        ...
        {
            return TryGetItemCollection(dataSpace, out var itemCollection);
        }
            ...
            EntitySetBase entitySet, EntityTypeBase entityType, StorageMappingItemCollection.InterestingMembersKind interestingMembersKind)
        {
            ...
            Debug.Assert(entitySet.EntityContainer is not null);
        ...
        {
            Debug.Assert(_itemsSSpace is not null && _itemsSSpace.Value is not null);
        ...
        internal virtual bool TryDetermineCSpaceModelType(Type type, out EdmType modelEdmType)
        {
            ...
            if (objectItemCollection.TryGetItem(nonNullableType.FullNameWithNesting(), out EdmType objectEdmType))
            {
                if (TryGetMap(objectEdmType, DataSpace.OCSpace, out var map))
                {
