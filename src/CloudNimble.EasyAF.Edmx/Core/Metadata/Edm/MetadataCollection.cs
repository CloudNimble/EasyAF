...
namespace System.Data.Entity.Core.Metadata.Edm
{
        ...
        where T : MetadataItem
    {
        ...
        private void HandleIdentityChange(T item, string initialIdentity, bool validate)
        {
            ...
            if (_caseSensitiveDictionary is not null)
            {
                if (!validate
                    || (_caseSensitiveDictionary.TryGetValue(initialIdentity, out var existingItem)
        ...
        public bool Contains(T item)
        {
            ...
            return
                TryGetValue(item.Identity, false, out var existingItem)
                && ReferenceEquals(existingItem, item);
        }
        ...
        {
            get { return _caseSensitiveDictionary is not null; }
        }

        internal bool HasCaseInsensitiveDictionary
        {
            get { return _caseInsensitiveDictionary is not null; }
        }
        ...
        {
            if (_caseSensitiveDictionary is null
        ...
        {
            if (_caseInsensitiveDictionary is null
                && _metadataList.Count > UseDictionaryCrossover)
            {
        ...
        private Dictionary<string, int> CreateCaseInsensitiveDictionary()
        {
            ...
            for (var i = 1; i < _metadataList.Count; i++)
            {
                ...
                if (!caseInsensitiveDictionary.TryGetValue(identity, out var index))
                {
        ...
        private bool ContainsIdentityCaseSensitive(string identity)
        {
            ...
            if (caseSensitiveDictionary is not null)
            {
        ...
        private bool FindCaseInsensitive(string identity, out T item, bool throwOnMultipleMatches)
        {
            ...
            if (caseInsensitiveDictionary is not null)
            {
                if (caseInsensitiveDictionary.TryGetValue(identity, out var index))
                {
                    if (index >= 0)
        ...
        {
            Debug.Assert(_caseSensitiveDictionary is not null);
