...
namespace System.Data.Entity.Core.Metadata.Edm
{
    public partial class StoreItemCollection
    {
        private class Loader
        {
            ...
            {
                Debug.Assert(_errors is null, "we are expecting this to be the location that sets _errors for the first time");
            ...
            private void OnProviderNotification(string provider, Action<string, ErrorCode, EdmSchemaErrorSeverity> addError)
            {
                ...
                Debug.Assert(expected is not null, "Expected provider name not initialized from _provider or _providerFactory?");
            ...
            {
                if (_providerManifest is null
                    && (_providerManifestToken is not null && _provider is not null))
                {
            ...
            {
                if (_providerManifestToken is null)
                {
                ...
                ProviderIncompatibleException provEx, Action<string, ErrorCode, EdmSchemaErrorSeverity> addError)
            {
                ...
                if (provEx.InnerException is not null
