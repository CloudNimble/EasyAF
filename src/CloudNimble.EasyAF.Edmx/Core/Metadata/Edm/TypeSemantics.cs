...
namespace System.Data.Entity.Core.Metadata.Edm
{
    ...
    internal static class TypeSemantics
    {
        ...
        internal static IEnumerable<TypeUsage> FlattenType(TypeUsage type)
        {
            ...
            Func<TypeUsage, IEnumerable<TypeUsage>> getImmediateSubNodes =
                t =>
                {
                    ...
                    {
                        return [TypeHelpers.GetElementTypeUsage(t)];
                    }
        ...
        {
            if (TryGetCommonType(type1, type2, out var commonType))
            {
                return commonType;
            }
            return null;
        ...
        {
            if (type.Facets.TryGetValue(DbProviderManifest.NullableFacetName, false, out var nullableFacet))
            {
        ...
        {
            if (!TryGetCommonType(collectionType1.TypeUsage, collectionType2.TypeUsage, out var commonTypeUsage))
            {
                commonType = null;
                return false;
            }
        ...
        private static bool TryGetCommonType(RowType rowType1, RowType rowType2, out EdmType commonRowType)
        {
            ...
            {
                if (!TryGetCommonType(rowType1.Properties[i].TypeUsage, rowType2.Properties[i].TypeUsage, out var columnCommonTypeUsage))
                {
                    commonRowType = null;
                    return false;
                }
        ...
        internal static bool TryGetCommonBaseType(EdmType type1, EdmType type2, out EdmType commonBaseType)
        {
            ...
            for (var ancestor = type2; ancestor is not null; ancestor = ancestor.BaseType)
            {
            ...
            for (var ancestor = type1; ancestor is not null; ancestor = ancestor.BaseType)
            {
