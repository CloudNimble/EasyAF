...
namespace System.Data.Entity.Core.Metadata.Edm
{
    ...
    internal abstract class Perspective
    {
            ...
            EntityContainer entityContainer, String functionImportName, bool ignoreCase, out EdmFunction functionImport)
        {
            ...
            return functionImport is not null;
        }
            ...
            string namespaceName, string functionName, bool ignoreCase, out IList<EdmFunction> functionOverloads)
        {
            ...
            if (_targetDataspace == DataSpace.CSpace)
            {
                ...
                if (overloads is null
                    || overloads.Count == 0)
                {
                    if (TryGetEntityContainer(namespaceName, /*ignoreCase:*/ false, out var entityContainer))
                    {
                        if (TryGetFunctionImport(entityContainer, functionName, /*ignoreCase:*/ false, out var functionImport))
                        {
                            overloads = [functionImport];
                        }
                    }
                }
                ...
                if (overloads is null
                    || overloads.Count == 0)
                {
                    if (_metadataWorkspace.TryGetItemCollection(DataSpace.SSpace, out var storeItemCollection))
                    {
            ...
            functionOverloads = (overloads is not null && overloads.Count > 0) ? overloads : null;
            return functionOverloads is not null;
        }
