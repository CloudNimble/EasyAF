...
namespace System.Data.Entity.Core.Metadata.Edm.Provider
{
    internal class EdmProviderManifest : DbProviderManifest
    {
        ...
        private static readonly EdmProviderManifest _instance = new();
        ...
        public override ReadOnlyCollection<FacetDescription> GetFacetDescriptions(EdmType type)
        {
            ...
            if (_facetDescriptions.TryGetValue(type as PrimitiveType, out var collection))
            {
                return collection;
            }
        ...
        {
            if (_primitiveTypes is not null)
            {
                return;
            }
            ...
            InitializePrimitiveType(
                primitiveTypes[(int)PrimitiveTypeKind.DateOnly], PrimitiveTypeKind.DateOnly, EdmConstants.DateOnly, typeof(DateOnly));
            InitializePrimitiveType(
                primitiveTypes[(int)PrimitiveTypeKind.TimeOnly], PrimitiveTypeKind.TimeOnly, EdmConstants.TimeOnly, typeof(TimeOnly));
        ...
        {
            if (_facetDescriptions is not null)
            {
                return;
            }
            ...
            // DateOnly facets - no facets needed
            list = GetInitialFacetDescriptions(PrimitiveTypeKind.DateOnly);
            applicableType = _primitiveTypes[(int)PrimitiveTypeKind.DateOnly];
            facetDescriptions.Add(applicableType, new ReadOnlyCollection<FacetDescription>(list));

            // TimeOnly facets
            list = GetInitialFacetDescriptions(PrimitiveTypeKind.TimeOnly);
            applicableType = _primitiveTypes[(int)PrimitiveTypeKind.TimeOnly];
            facetDescriptions.Add(applicableType, new ReadOnlyCollection<FacetDescription>(list));
        ...
        private void InitializePromotableTypes()
        {
            ...
            {
                promotionTypes[i] = new ReadOnlyCollection<PrimitiveType>([_primitiveTypes[i]]);
            }
            ...
            //
            // PrimitiveTypeKind.DateOnly - can be promoted to DateTime
            //
            promotionTypes[(int)PrimitiveTypeKind.DateOnly] = new ReadOnlyCollection<PrimitiveType>(
                [
                        _primitiveTypes[(int)PrimitiveTypeKind.DateOnly],
                        _primitiveTypes[(int)PrimitiveTypeKind.DateTime]
                    ]);

            //
            // PrimitiveTypeKind.TimeOnly - can be promoted to Time
            //
            promotionTypes[(int)PrimitiveTypeKind.TimeOnly] = new ReadOnlyCollection<PrimitiveType>(
                [
                        _primitiveTypes[(int)PrimitiveTypeKind.TimeOnly],
        ...
        internal TypeUsage ForgetScalarConstraints(TypeUsage type)
        {
            ...
            Debug.Assert(primitiveType is not null, "type argument must be primitive in order to use this function");
