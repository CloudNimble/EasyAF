...
namespace System.Data.Entity.Core.Metadata.Edm.Provider
{
    ...
    {
        private const int s_PrimitiveTypeCount = 19;
        private ReadOnlyCollection<PrimitiveType> _primitiveTypes;
        private static readonly ClrProviderManifest _instance = new();
        ...
        {
            primitiveType = null;
            if (TryGetPrimitiveTypeKind(clrType, out var resolvedTypeKind))
            {
                    ...
                    case TypeCode.Object:
                        {
                            ...
                            else if (typeof(DateOnly) == clrType)
                            {
                                primitiveTypeKind = PrimitiveTypeKind.DateOnly;
                            }
                            else if (typeof(TimeOnly) == clrType)
                            {
                                primitiveTypeKind = PrimitiveTypeKind.TimeOnly;
                            }
                            break;
                        }
                }
        ...
        private void InitializePrimitiveTypes()
        {
            ...
            var primitiveTypes = new PrimitiveType[33];
            ...
            primitiveTypes[(int)PrimitiveTypeKind.DateOnly] = CreatePrimitiveType(typeof(DateOnly), PrimitiveTypeKind.DateOnly);
            primitiveTypes[(int)PrimitiveTypeKind.TimeOnly] = CreatePrimitiveType(typeof(TimeOnly), PrimitiveTypeKind.TimeOnly);
