...
namespace System.Data.Entity.Core.Metadata.Edm
{
    ...
    internal sealed class ObjectItemAttributeAssemblyLoader : ObjectItemAssemblyLoader
    {
        ...
        private readonly List<Action> _unresolvedNavigationProperties = [];
        ...
        private bool TryGetLoadedType(Type clrType, out EdmType edmType)
        {
            ...
            if (clrType.IsGenericType())
            {
                ...
                if (!TryGetLoadedType(clrType.GetGenericArguments()[0], out var elementType))
                {
                    return false;
                }
        ...
        private bool TryGetCachedEdmType(Type clrType, out EdmType edmType)
        {
            ...
            if (SessionData.LockedAssemblyCache.TryGetValue(clrType.Assembly(), out var immutableCacheEntry))
            {
                Debug.Assert(
        ...
        private void LoadType(Type clrType)
        {
            ...
            if (typeAttributes.Any())
            {
                ...
                else
                {
                    ...
                    if (!ClrProviderManifest.Instance.TryGetPrimitiveType(clrType.GetEnumUnderlyingType(), out var underlyingEnumType))
                    {
                        SessionData.EdmItemErrors.Add(
                            new EdmItemError(
        ...
        private EdmType ResolveBaseType(Type type)
        {
            if (type.GetCustomAttributes<EdmEntityTypeAttribute>(inherit: false).Any()
                && TryGetLoadedType(type, out var edmType))
            {
                return edmType;
            }
            return null;
        ...
        {
            if (roleAttribute.RelationshipName is null)
            {
                SessionData.EdmItemErrors.Add(
            ...
            if (roleAttribute.RelationshipNamespaceName is null)
            {
                SessionData.EdmItemErrors.Add(
                    new EdmItemError(
            ...
            if (roleAttribute.Role1Name is null)
            {
                SessionData.EdmItemErrors.Add(
                    new EdmItemError(
                        Strings.NullParameterForEdmRelationshipAttribute(
            ...
            if (roleAttribute.Role1Type is null)
            {
                SessionData.EdmItemErrors.Add(
                    new EdmItemError(
                        Strings.NullParameterForEdmRelationshipAttribute(
            ...
            if (roleAttribute.Role2Name is null)
            {
                SessionData.EdmItemErrors.Add(
                    new EdmItemError(
                        Strings.NullParameterForEdmRelationshipAttribute(
            ...
            if (roleAttribute.Role2Type is null)
            {
                SessionData.EdmItemErrors.Add(
                    new EdmItemError(
                        Strings.NullParameterForEdmRelationshipAttribute(
        ...
        private bool TryGetRelationshipEndEntityType(Type type, out EntityType entityType)
        {
            ...
            if (!TryGetLoadedType(type, out var edmType)
        ...
        internal void ResolveNavigationProperty(StructuralType declaringType, PropertyInfo propertyInfo)
        {
            ...
            if (!TryGetLoadedType(propertyInfo.PropertyType, out var propertyType)
                ||
            ...
            if (SessionData.TypesInLoading.TryGetValue(attribute.RelationshipNamespaceName + "." + attribute.RelationshipName, out var type)
                &&
        ...
        private void ResolveEnumTypeProperty(StructuralType declaringType, PropertyInfo clrProperty)
        {
            ...
            if (!TryGetLoadedType(clrProperty.PropertyType, out var propertyType)
