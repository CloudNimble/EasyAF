...
namespace System.Data.Entity.Core.Metadata.Edm
{
    ...
    internal abstract class OSpaceTypeFactory
    {
        ...
        private bool TryCreateStructuralType(Type type, StructuralType cspaceType, out EdmType newOSpaceType)
        {
            ...
            if (cspaceType.BaseType is not null)
            {
        ...
        private bool UnderlyingEnumTypesMatch(Type enumType, EnumType cspaceEnumType)
        {
            ...
            if (!ClrProviderManifest.Instance.TryGetPrimitiveType(enumType.GetEnumUnderlyingType(), out var underlyingEnumType))
            {
                LogLoadMessage(
        ...
        {
            var clrProperties = (cspaceType.BaseType is null
        ...
        {
            var foundValue = CspaceToOspace.TryGetValue(baseCSpaceType, out var ospaceType);
            if (!foundValue)
            {
            ...
            Type type, StructuralType cspaceType, StructuralType ospaceType, IEnumerable<PropertyInfo> clrProperties)
        {
                    ...
                    cspaceType.GetDeclaredOnlyMembers<EdmProperty>().Where(p => Helper.IsPrimitiveType(p.TypeUsage.EdmType)))
            {
                ...
                if (clrProperty is not null)
                {
                    if (TryGetPrimitiveType(clrProperty.PropertyType, out var propertyType))
                    {
                        if (clrProperty.CanRead
        ...
        {
            if (CspaceToOspace.TryGetValue(cspaceProperty.TypeUsage.EdmType, out var propertyType))
            {
        ...
        {
            if (CspaceToOspace.TryGetValue(cspaceProperty.RelationshipType, out var ospaceRelationship))
            {
                ...
                if (Helper.IsCollectionType(cspaceProperty.TypeUsage.EdmType))
                {
                    foundTarget =
                        CspaceToOspace.TryGetValue(
                            ((CollectionType)cspaceProperty.TypeUsage.EdmType).TypeUsage.EdmType, out var findType);
                    if (foundTarget)
                    {
                ...
                {
                    foundTarget = CspaceToOspace.TryGetValue(cspaceProperty.TypeUsage.EdmType, out var findType);
                    if (foundTarget)
                    {
                        Debug.Assert(findType is StructuralType, "Structural type expected.");
            ...
            Type type, PropertyInfo clrProperty, StructuralType ospaceType, EdmProperty cspaceProperty, EdmType propertyType)
        {
            ...
            var nullableFacetValue = !isKeyMember
                                     &&
                                     (!clrProperty.PropertyType.IsValueType() || Nullable.GetUnderlyingType(clrProperty.PropertyType) is not null);
