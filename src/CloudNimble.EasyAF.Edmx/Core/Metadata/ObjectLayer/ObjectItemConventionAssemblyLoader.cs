...
namespace System.Data.Entity.Core.Metadata.Edm
{
    internal class ObjectItemConventionAssemblyLoader : ObjectItemAssemblyLoader
    {
        ...
        protected override void LoadTypesFromAssembly()
        {
            ...
            {
                if (TryGetCSpaceTypeMatch(type, out var cspaceType))
                {
                    if (type.IsValueType()
                        && !type.IsEnum())
                    {
        ...
        private bool TryGetCSpaceTypeMatch(Type type, out EdmType cspaceType)
        {
            ...
            if (SessionData.ConventionCSpaceTypeNames.TryGetValue(type.Name, out var pair))
            {
                if (pair.Value == 1)
                {
        ...
        {
            return sessionData.EdmItemCollection is not null;
        }
