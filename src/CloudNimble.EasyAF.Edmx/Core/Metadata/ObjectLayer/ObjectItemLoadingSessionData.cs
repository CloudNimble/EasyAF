...
namespace System.Data.Entity.Core.Metadata.Edm
{
    internal class ObjectItemLoadingSessionData
    {
        ...
        internal Dictionary<string, KeyValuePair<EdmType, int>> ConventionCSpaceTypeNames
        {
            get
            {
                if (_edmItemCollection is not null
                    && _conventionCSpaceTypeNames is null)
                {
                    ...
                    foreach (var edmType in _edmItemCollection.GetItems<EdmType>())
                    {
                        ...
                        {
                            if (_conventionCSpaceTypeNames.TryGetValue(edmType.Name, out var pair))
                            {
        ...
        internal Func<Assembly, ObjectItemLoadingSessionData, ObjectItemAssemblyLoader> ObjectItemAssemblyLoaderFactory
        {
            ...
            set
            {
                ...
                {
                    Debug.Assert(
                        _loaderFactory is null || _typesInLoading.Count == 0,
        ...
        internal object LoaderCookie
        {
            get
            {
                ...
                if (_originalLoaderCookie is not null)
                {
                    Debug.Assert(
                        _loaderFactory is null ||
            ...
            Action<String> logLoadMessage, object loaderCookie)
        {
            ...
            if (_loaderFactory == ObjectItemConventionAssemblyLoader.Create
                && _edmItemCollection is not null)
            {
