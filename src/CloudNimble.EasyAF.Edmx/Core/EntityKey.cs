...
namespace System.Data.Entity.Core
{
    ...
    public sealed class EntityKey : IEquatable<EntityKey>
    {
        ...
        private static readonly EntityKey _noEntitySetKey = new("NoEntitySetKey.NoEntitySetKey");

        // <summary>
        // Returns a singleton EntityKey identifying an entity resulted from a failed TREAT.
        // </summary>
        [SuppressMessage("Microsoft.Security", "CA2104:DoNotDeclareReadOnlyMutableReferenceTypes")]
        private static readonly EntityKey _entityNotValidKey = new("EntityNotValidKey.EntityNotValidKey");

        // <summary>
        // A dictionary of names so that singleton instances of names can be used
        // </summary>
        private static readonly ConcurrentDictionary<string, string> NameLookup = new();
        ...
        public EntityKeyMember[] EntityKeyValues
        {
            get
            {
                if (!IsTemporary)
                {
                    ...
                    if (_singletonKeyValue is not null)
                    {
                        keyValues =
                            [
        ...
        {
            get { return (SingletonKeyValue is null) && (CompositeKeyValues is null); }
        }
        ...
        public override int GetHashCode()
        {
            ...
            if (0 == hashCode)
            {
                ...
                if (_entitySetName is not null)
                {
                    hashCode = _entitySetName.GetHashCode();
                }
                if (_entityContainerName is not null)
                {
                ...
                if (_isLocked || (!String.IsNullOrEmpty(_entitySetName) &&
                                  ...
                                  (_singletonKeyValue is not null || _compositeKeyValues is not null)))
                {
                    _hashCode = hashCode;
                }
            }
        ...
        internal KeyValuePair<string, DbExpression>[] GetKeyValueExpressions(EntitySet entitySet)
        {
            ...
            if (_singletonKeyValue is not null)
            {
                ...
                Debug.Assert(singletonKeyMember is not null, "Metadata for singleton key member shouldn't be null.");
                keyColumns =
                    [
            ...
            else
            {
                ...
                for (var i = 0; i < _compositeKeyValues.Length; ++i)
                {
                    Debug.Assert(_compositeKeyValues[i] is not null, "Values within key-value pairs cannot be null.");

                    var keyMember = ((EntitySetBase)entitySet).ElementType.KeyMembers[i];
                    Debug.Assert(keyMember is not null, "Metadata for key members shouldn't be null.");
        ...
        internal object FindValueByName(string keyName)
        {
            ...
            if (SingletonKeyValue is not null)
            {
            ...
            MetadataWorkspace workspace, EdmMember keyMember, object keyValue, bool isArgumentException, string argumentName)
        {
            ...
            else
            {
                ...
                if (workspace.TryGetObjectSpaceType((EnumType)keyMemberEdmType, out var expectedEnumType))
                {
        ...
        private void AssertCorrectState(EntitySetBase entitySetBase, bool isTemporary)
        {
            ...
            if (_singletonKeyValue is not null)
            {
                ...
                Debug.Assert(_compositeKeyValues is null);
                if (entitySetBase is not null)
                {
                    Debug.Assert(entitySet.ElementType.KeyMembers.Count == 1);
                }
            }
            else if (_compositeKeyValues is not null)
            {
                Debug.Assert(!isTemporary);
                if (entitySetBase is not null)
                {
                ...
                for (var i = 0; i < _compositeKeyValues.Length; ++i)
                {
                    Debug.Assert(_compositeKeyValues[i] is not null);
                }
            }
            else if (!IsTemporary)
            {
                // one of our static keys
                Debug.Assert(EntityKeyValues is null);
                Debug.Assert(EntityContainerName is null);
                Debug.Assert(EntitySetName is not null);
            }
            else
            {
                Debug.Assert(EntityKeyValues is null);
        ...
        {
            if (_isLocked || instance is not null)
            {
        ...
        {
            get { return _deserializedMembers is not null; }
        }
