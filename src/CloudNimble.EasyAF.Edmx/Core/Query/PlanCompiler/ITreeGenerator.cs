...
namespace System.Data.Entity.Core.Query.PlanCompiler
{
    ...
    internal class ITreeGenerator : DbExpressionVisitor<Node>
    {
        ...
        private sealed class LambdaScope : CqtVariableScope
        {
            ...
            internal override Node this[string name]
            {
                get
                {
                    ...
                    if (_referencedArgs.ContainsKey(argNode))
                    {
                        ...
                        var argCopy = OpCopier.Copy(_command, argNode, out var mappedVars);
            ...
            private void MapCopiedNodeVars(IList<Node> sources, IList<Node> copies, Dictionary<Var, Var> varMappings)
            {
                ...
                for (var idx = 0; idx < sources.Count; idx++)
                {
                    ...
                    if (_treeGen.VarMap.TryGetValue(sourceNode, out var sourceVar))
                    {
        ...
        private readonly Stack<CqtVariableScope> _varScopes = new();
        private readonly Dictionary<Node, Var> _varMap = [];
        private readonly Stack<EdmFunction> _functionExpansions = new();
        ...
        private readonly HashSet<DbTreatExpression> _fakeTreats = [];
        ...
        private ITreeGenerator(DbQueryCommandTree ctree, DiscriminatorMap discriminatorMap)
        {
            ...
            if (!_iqtCommand.Root.Op.IsRelOp)
            {
                ...
                var varDefListNode = _iqtCommand.CreateVarDefListNode(scalarExpr, out var newVar);
        ...
        {
            return (paramType is not null && paramType.EdmType is not null &&
        ...
        {
            return (TypeHelpers.TryGetEdmType(typeUsage, out CollectionType collectionType) &&
                    ...
                    case DbExpressionKind.Lambda:
                        {
                            ...
                            if (_functionsIsPredicateFlag.TryGetValue(expr, out var isPredicateFunction))
                            {
        ...
        private Node EnsureRelOp(Node inputNode)
        {
            ...
            PlanCompiler.Assert(scalar is not null, "An expression in a CQT produced a non-ScalarOp and non-RelOp output Op");
            ...
            if (collect is not null)
            {
                ...
                if (inputNode.Child0.Op as PhysicalProjectOp is not null)
                {
            ...
            // varDefNode = N2
            //
            var varDefNode = _iqtCommand.CreateVarDefNode(inputNode, out var inputCollectionVar);
            ...
            var varDefListNode = _iqtCommand.CreateVarDefListNode(varRefNode, out var projectVar);
        ...
        {
            var inputNode = VisitBoundExpression(boundExpression, out var boundVar);
        ...
        private Node VisitBoundExpression(DbExpression boundExpression, out Var boundVar)
        {
            ...
            PlanCompiler.Assert(inputNode is not null, "DbExpressionBinding.Expression produced null conversion");
            ...
            PlanCompiler.Assert(boundVar is not null, "No Var found for Input Op");
        ...
        private ExpressionBindingScope ExitExpressionBinding()
        {
            ...
            PlanCompiler.Assert(retScope is not null, "ExitExpressionBinding called without ExpressionBindingScope on top of scope stack");
            return retScope;
        }
        ...
        {
            var retScope = _varScopes.Pop() as ExpressionBindingScope;
            PlanCompiler.Assert(retScope is not null, "ExitGroupExpressionBinding called without ExpressionBindingScope on top of scope stack");
        }
        ...
        private void EnterLambdaFunction(DbLambda lambda, List<Tuple<Node, bool>> argumentValues, EdmFunction expandingEdmFunction)
        {
            ...
            // 
            if (expandingEdmFunction is not null)
            {
                //
        ...
        private LambdaScope ExitLambdaFunction(EdmFunction expandingEdmFunction)
        {
            ...
            PlanCompiler.Assert(retScope is not null, "ExitLambdaFunction called without LambdaScope on top of scope stack");
        ...
        private Node ProjectNewRecord(Node inputNode, RowType recType, IEnumerable<Var> colVars)
        {
            ...
            //
            var varDefNode = _iqtCommand.CreateVarDefListNode(newRecordNode, out var newRecordVar);
        ...
        private Node BuildSoftCast(Node node, TypeUsage targetType)
        {
            ...
            if (node.Op.IsRelOp)
            {
                ...
                var varDefListNode = _iqtCommand.CreateVarDefListNode(castNode, out var projectVar);
        ...
        public override Node Visit(DbPropertyExpression e)
        {
            ...
            PlanCompiler.Assert(e.Instance is not null, "Static properties are not supported");
        ...
        private Node CreateLimitNode(Node inputNode, Node limitNode, bool withTies)
        {
            ...
            else
            {
                ...
                retNode =
                    _iqtCommand.CreateNode(
                        _iqtCommand.CreateConstrainedSortOp([], withTies),
                        inputNode,
                        _iqtCommand.CreateNode(_iqtCommand.CreateNullOp(_iqtCommand.IntegerType)),
                        limitNode
                        );
        ...
        private Node VisitSetOpExpression(DbBinaryExpression expression)
        {
            ...
            {
                { outputVar, _varMap[leftNode] }
            };

            var rightMap = new VarMap
            {
                { outputVar, _varMap[rightNode] }
            };
        ...
        public override Node Visit(DbOfTypeExpression e)
        {
            ...
            //
            var resultNode = _iqtCommand.BuildFakeTreatProject(inputNode, inputVar, e.OfType, out var resultVar);
        ...
        public override Node Visit(DbNewInstanceExpression e)
        {
            ...
            if (relPropertyExprs is not null)
            {
        ...
        private Node GenerateDiscriminatedProject(DbProjectExpression e)
        {
            ...
            {
                CreateNewInstanceArgument(_discriminatorMap.Discriminator.Property, _discriminatorMap.Discriminator)
            };
            ...
            var varDefListNode = _iqtCommand.CreateVarDefListNode(newInstNode, out var sourceVar);
        ...
        private Node GenerateStandardProject(DbProjectExpression e)
        {
            ...
            var varDefListNode = _iqtCommand.CreateVarDefListNode(projectionNode, out var projectionVar);
        ...
        private Node VisitJoin(DbExpression e, IList<DbExpressionBinding> inputs, DbExpression joinCond)
        {
            ...
            {
                var inputNode = VisitBoundExpression(inputs[idx].Expression, out var boundVar);
            ...
            //
            PlanCompiler.Assert(joinOp is not null, "Unrecognized JoinOp specified in DbJoinExpression, no JoinOp was produced");
            ...
            {
                PlanCompiler.Assert(joinCondNode is not null, "Non CrossJoinOps must specify a join condition");
        ...
        public override Node Visit(DbGroupByExpression e)
        {
            ...
            ExtractKeys(e, keyVarSet, outputVarSet, out var inputNode, out var keyVarDefNodes, out var scope);
            ...
            {
                //not needed
                ExtractKeys(e, copyKeyVarSet, copyOutputVarSet, out copyOfInput, out copyOfKeyVarDefNodes, out var copyOfScope);
            }
            ...
            for (var idx = 0; idx < e.Aggregates.Count; idx++)
            {
                ...
                if (idx != groupAggregateIndex)
                {
                    ...
                    PlanCompiler.Assert(funcAgg is not null, "Unrecognized DbAggregate used in DbGroupByExpression");
            ...
            {
                inputNode, // The Node produced from the Input set
                _iqtCommand.CreateNode(
            ...
            out ExpressionBindingScope scope)
        {
            ...
            for (var idx = 0; idx < e.Keys.Count; idx++)
            {
                ...
                //
                PlanCompiler.Assert(keyOp is not null, "GroupBy Key is not a ScalarOp");
                ...
                //
                keyVarDefNodes.Add(_iqtCommand.CreateVarDefNode(keyNode, out var keyVar));
            ...
            out Var groupAggVar)
        {
            ...
            if (keyVarDefNodes.Count > 0)
            {
                ...
                for (var j = 0; j < flattentedKeys.Count; j++)
                {
                    ...
                    if (filterPredicateNode is null)
                    {
        ...
        private Node VisitSortArguments(DbExpressionBinding input, IList<DbSortClause> sortOrder, List<SortKey> sortKeys, out Var inputVar)
        {
            ...
            for (var idx = 0; idx < sortOrder.Count; idx++)
            {
                ...
                PlanCompiler.Assert(specOp is not null, "DbSortClause Expression converted to non-ScalarOp");
                ...
                //
                sortVarDefs.Add(_iqtCommand.CreateVarDefNode(exprNode, out var specVar));
        ...
        public override Node Visit(DbSkipExpression expression)
        {
            ...
            var inputNode = VisitSortArguments(expression.Input, expression.SortOrder, sortKeys, out var inputVar);
        ...
        public override Node Visit(DbSortExpression e)
        {
            ...
            //
            var sortKeys = new List<SortKey>();
            var inputNode = VisitSortArguments(e.Input, e.SortOrder, sortKeys, out var inputVar);
