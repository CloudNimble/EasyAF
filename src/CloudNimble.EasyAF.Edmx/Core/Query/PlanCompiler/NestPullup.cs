...
namespace System.Data.Entity.Core.Query.PlanCompiler
{
    ...
    internal class NestPullup : BasicOpVisitorOfNode
    {
        ...
        // </summary>
        private readonly Dictionary<Var, Node> m_definingNodeMap = [];
        ...
        // </summary>
        private readonly Dictionary<Var, Var> m_varRefMap = [];
        ...
        private static void RemapSortKeys(List<SortKey> sortKeys, Dictionary<Var, Var> varMap)
        {
            if (sortKeys is not null)
            {
                ...
                {
                    if (varMap.TryGetValue(sortKey.Var, out var replacementVar))
                    {
        ...
        private static IEnumerable<Var> RemapVars(IEnumerable<Var> vars, Dictionary<Var, Var> varMap)
        {
            ...
            {
                if (varMap.TryGetValue(v, out var mappedVar))
                {
        ...
        // Also, FILTER(Zi, v is not null) may be transformed to push the filter below any NestOps.
        ...
        private Node ApplyOpJoinOp(Op op, Node n)
        {
            ...
            foreach (var chi in n.Children)
            {
                    ...
                    == OpType.MultiStreamNest)
                {
                    ...
                    //
                    //  Note: we transform FILTER(Zi, v is not null) to push the filter below any MSNs. 
                    ...
                    {
                        newJoinApplyChildren.Add(AugmentNodeWithConstant(chi.Child0, () => Command.CreateNullSentinelOp(), out var sentinelVar));
            ...
            NestBaseOp newNestOp = Command.CreateMultiStreamNestOp([], newOutputVars, newCollectionInfoList);
        ...
        private Node ApplyIsNotNullFilter(Node node, Var sentinelVar)
        {
            ...
            if (newFilterParent is not null)
            {
        ...
        private Node ProjectOpCase1(Node projectNode)
        {
            ...
            foreach (var chi in projectNode.Child1.Children)
            {
                         ...
                         == definingExprNode.Op.OpType)
                {
                    ...
                    if (m_definingNodeMap.TryGetValue(refVar, out var physicalProjectNode))
                    {
            ...
            var nestOp = Command.CreateMultiStreamNestOp([], outputVars, collectionInfoList);
        ...
        private Node ProjectOpCase2(Node projectNode)
        {
            ...
            else
            {
                ...
                newNestNodeChildren = [nestNode.Child0];
                var i = 1;
                foreach (var ci in nestOp.CollectionInfo)
                {
            ...
            if (0 != newCollectionInfoList.Count)
            {
                ...
                if (newProjectOpOutputs.IsEmpty)
                {
                    ...
                    var varDefNode = Command.CreateVarDefNode(tempNode, out var tempVar);
        ...
        public override Node Visit(UnnestOp op, Node n)
        {
                     ...
                     == newNode.Op.OpType)
            {
                ...
                var found = m_definingNodeMap.TryGetValue(refVar, out var refVarDefiningNode);
        ...
        {
            var newNode = OpCopierTrackingCollectionVars.Copy(Command, refVarDefiningNode, out var varMap, out var collectionVarDefinitions);
            ...
            if (collectionVarDefinitions.Count != 0)
            {
                ...
                foreach (var collectionVarDefinitionPair in collectionVarDefinitions)
                {
                    ...
                    if (m_definingNodeMap.TryGetValue(keyDefiningVar, out var keyDefiningNode))
                    {
        ...
        public override Node Visit(PhysicalProjectOp op, Node n)
        {
            ...
            nestNode = ConvertToSingleStreamNest(nestNode, varRefReplacementMap, outputVars, out var keyColumnMaps);
            ...
            out SimpleColumnMap[] parentKeyColumnMaps)
        {
            ...
            for (var i = 1; i < nestNode.Children.Count; i++)
            {
                    ...
                    == OpType.MultiStreamNest)
                {
                    ...
                    nestNode.Children[i] = ConvertToSingleStreamNest(
                        chi, varRefReplacementMap, childFlattenedOutputVars, out var childKeyColumnMaps);
            ...
            NormalizeNestOpInputs(nestOp, nestNode, out var discriminatorVarList, out var postfixSortKeyList);

            // Now build up the union-all subquery
            var unionAllNode = BuildUnionAllSubqueryForNestOp(
                nestOp, nestNode, drivingNodeVars, discriminatorVarList, out var outputDiscriminatorVar, out var varMapList);
            ...
            NestBaseOp nestOp, Node nestNode, out VarList discriminatorVarList, out List<List<SortKey>> sortKeys)
        {
            ...
            sortKeys = [nestOp.PrefixSortKeys];
            ...
            for (var i = 1; i < nestNode.Children.Count; i++)
            {
                ...
                var augmentedInput = AugmentNodeWithInternalIntegerConstant(inputNode, i, out var discriminatorVar);
                ...
                for (var j = 1; j < nestNode.Children.Count; j++)
                {
                    ...
                    else
                    {
                        foreach (var v in otherCollectionInfo.FlattenedElementVars)
                        {
                            ...
                            var nullOpVarDefNode = Command.CreateVarDefNode(nullOpNode, out var nullOpVar);
