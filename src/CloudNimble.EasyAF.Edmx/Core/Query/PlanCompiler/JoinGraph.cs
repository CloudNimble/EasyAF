...
namespace System.Data.Entity.Core.Query.PlanCompiler
{
    ...
    internal class JoinGraph
    {
            ...
            out Node otherPredicateNode)
        {
            ...
            predicate.GetEquiJoinPredicates(leftDefinitions, rightDefinitions, out var tempLeftVars, out var tempRightVars, out var otherPredicate);
        ...
        private bool AddJoinEdge(AugmentedJoinNode joinNode, ColumnVar leftVar, ColumnVar rightVar)
        {
            ...
            if (!m_tableVertexMap.TryGetValue(leftVar.Table, out var leftTableNode))
            {
                return false;
            }
            if (!m_tableVertexMap.TryGetValue(rightVar.Table, out var rightTableNode))
            {
                return false;
            }
        ...
        private bool IsConstraintPresentForTurningIntoInnerJoin(JoinEdge joinEdge)
        {
            ...
            if (m_constraintManager.IsParentChildRelationship(
                joinEdge.Right.Table.TableMetadata.Extent, joinEdge.Left.Table.TableMetadata.Extent, out var fkConstraints))
            {
                PlanCompiler.Assert(fkConstraints is not null && fkConstraints.Count > 0, "Invalid foreign key constraints");
                foreach (var fkConstraint in fkConstraints)
                {
                    if (IsJoinOnFkConstraint(fkConstraint, joinEdge.RightVars, joinEdge.LeftVars, out var columnVars))
                    {
        ...
        {
            if (tableJoinEdge.JoinNode is null
                || replacingTableJoinEdge.JoinNode is null)
            {
                return false;
            }
        ...
        {
            PlanCompiler.Assert(tableVars is not null && replacementVars is not null, "null vars");
        ...
        {
            if (m_reverseVarMap.TryGetValue(replacedVar, out var oldReplacedVars))
            {
                m_reverseVarMap.Remove(replacedVar);
            }

            if (!m_reverseVarMap.TryGetValue(replacingVar, out var replacedVars))
            {
                // Try to reuse oldReplacedVars
                if (oldReplacedVars is not null)
                {
            ...
            else if (oldReplacedVars is not null)
            {
        ...
        {
            if (edge1.JoinNode is null)
            {
                return edge2.JoinNode is null;
            }

            if (edge2.JoinNode is null)
            {
                return false;
            }

            if (edge1.JoinNode.OtherPredicate is null)
            {
                return edge2.JoinNode.OtherPredicate is null;
            }

            if (edge2.JoinNode.OtherPredicate is null)
            {
                return false;
            }
        ...
        private bool QualifiesForStarSelfJoinGroup(JoinEdge joinEdge)
        {
            ...
            if (joinEdge.JoinNode is not null 
                && joinEdge.JoinNode.OtherPredicate is not null)
            {
        ...
        private void EliminateStarSelfJoins(AugmentedTableNode tableNode)
        {
            ...
            foreach (var joinEdge in tableNode.JoinEdges)
            {
                ...
                if (!groupedEdges.TryGetValue(joinEdge.Right.Table.TableMetadata.Extent, out var edges))
                {
                    edges = [];
        ...
        private void EliminateLeftTable(JoinEdge joinEdge)
        {
            ...
            //
            if (joinEdge.Right.NullableColumns is null)
            {
        ...
        private bool RightTableHasKeyReferences(JoinEdge joinEdge)
        {
            ...
            {
                if (m_reverseVarMap.TryGetValue(key, out var replacedVars))
                {
        ...
        private bool TryEliminateParentChildJoin(JoinEdge joinEdge, ForeignKeyConstraint fkConstraint)
        {
            ...
            if (!IsJoinOnFkConstraint(fkConstraint, joinEdge.LeftVars, joinEdge.RightVars, out var childColumnVars))
            {
                return false;
            }
            ...
            foreach (var keyProp in fkConstraint.ChildKeys)
            {
                ...
                for (var pos = 0; pos < parentVars.Count; pos++)
                {
                    ...
                    if (rightVar.ColumnMetadata.Name.Equals(keyProp))
                    {
                        ...
                        if (!fkConstraint.GetParentProperty(rightVar.ColumnMetadata.Name, out var parentPropertyName)
                            ||
        ...
        {
            if (!IsJoinOnFkConstraint(fkConstraint, joinEdge.RightVars, joinEdge.LeftVars, out var childColumnVars))
            {
                return false;
            }
        ...
        private Node BuildNodeTree()
        {
            ...
            foreach (var kv in m_varMap)
            {
                ...
                while (m_varMap.TryGetValue(newVar1, out var newVar2))
                {
                    PlanCompiler.Assert(newVar2 is not null, "null var mapping?");
                    newVar1 = newVar2;
                }
            ...
            var newNode = RebuildNodeTree(m_root, out var predicates);
            PlanCompiler.Assert(newNode is not null, "Resulting node tree is null");
            PlanCompiler.Assert(predicates is null || predicates.Count == 0, "Leaking predicates?");
            return newNode;
        }
        ...
        private Node BuildFilterForNullableColumns(Node inputNode, VarVec nonNullableColumns)
        {
            ...
            PlanCompiler.Assert(predNode is not null, "Null predicate?");
        ...
        private Node RebuildPredicate(AugmentedJoinNode joinNode, out int minLocationId)
        {
            ...
            for (var i = 0; i < joinNode.LeftVars.Count; i++)
            {
                if (!m_varMap.TryGetValue(joinNode.LeftVars[i], out var newLeftVar))
                {
                    newLeftVar = joinNode.LeftVars[i];
                }
                if (!m_varMap.TryGetValue(joinNode.RightVars[i], out var newRightVar))
                {
        ...
        private Node RebuildNodeTreeForCrossJoins(AugmentedJoinNode joinNode)
        {
            ...
            foreach (var chi in joinNode.Children)
            {
                newChildren.Add(RebuildNodeTree(chi, out var predicates));
                PlanCompiler.Assert(predicates is null || predicates.Count == 0, "Leaking predicates");
            }
        ...
        private Node RebuildNodeTree(AugmentedJoinNode joinNode, out Dictionary<Node, int> predicates)
        {
            ...
            var leftNode = RebuildNodeTree(joinNode.Children[0], out var leftPredicates);
            var rightNode = RebuildNodeTree(joinNode.Children[1], out var rightPredicates);
            ...
            Dictionary<Node, int> rightPredicates, out Dictionary<Node, int> outPredicates)
        {
            ...
            if (rightPredicates is not null)
            {
        ...
        {
            if (m_varToDefiningNodeMap.TryGetValue(var, out var node))
            {
                if (node.IsMoved)
                {
