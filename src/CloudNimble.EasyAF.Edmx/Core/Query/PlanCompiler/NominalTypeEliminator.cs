...
namespace System.Data.Entity.Core.Query.PlanCompiler
{
    ...
    internal class NominalTypeEliminator : BasicOpVisitorOfNode
    {
            ...
            Dictionary<md.EdmFunction, md.EdmProperty[]> tvfResultKeys)
        {
            ...
            PropertyPushdownHelper.Process(compilerState.Command, out var varPropertyMap, out var nodePropertyMap);
        ...
        private md.TypeUsage GetNewType(md.TypeUsage type)
        {
            ...
            if (m_typeToNewTypeMap.TryGetValue(type, out var newType))
            {
                return newType;
            }

            if (TypeHelpers.TryGetEdmType(type, out
        ...
        private Node BuildAccessor(Node input, md.EdmProperty property)
        {
            ...
            if (null != newRecordOp)
            {
                ...
                if (newRecordOp.GetFieldPosition(property, out var fieldPos))
                {
        ...
        private Node CreateTypeIdConstant(TypeInfo typeInfo)
        {
            ...
            if (typeInfo.RootType.DiscriminatorMap is not null)
            {
        ...
        private IEnumerable<PropertyRef> GetPropertyRefsForComparisonAndIsNull(TypeInfo typeInfo, OperationKind opKind)
        {
            ...
            if (TypeHelpers.TryGetEdmType(currentType, out
            md.RowType recordType))
            {
        ...
        private List<SortKey> HandleSortKeys(List<SortKey> keys)
        {
            ...
            {
                if (!m_varInfoMap.TryGetVarInfo(k.Var, out var varInfo))
                {
                    newSortKeys.Add(k);
                }
                else
                {
                    ...
                    if (structuredVarInfo is not null
            ...
            Node unnestNode, List<Var> unnestOpTableColumns, TypeInfo unnestOpTableTypeInfo, out List<Var> newVars)
        {
            ...
            PlanCompiler.Assert(originalRowType is not null, "Unexpected TVF return type (must be row): " + unnestOpTableTypeInfo.Type);
            ...
            foreach (var flattenedTypeProperty in unnestOpTableTypeInfo.FlattenedType.Properties)
            {
                ...
                else
                {
                    ...
                    if (nullSentinelPropRef is not null)
                    {
                ...
                PlanCompiler.Assert(var is not null, "TVFs returning a collection of rows with non-primitive properties are not supported");
        ...
        public override Node Visit(VarDefListOp op, Node n)
        {
            ...
            foreach (var chi in n.Children)
            {
                    ...
                    || TypeUtils.IsCollectionType(varDefOp.Var.Type))
                {
                    ...
                    FlattenComputedVar((ComputedVar)varDefOp.Var, chi, out var newChiList, out var x);
        ...
        private void FlattenComputedVar(ComputedVar v, Node node, out List<Node> newNodes, out md.TypeUsage newType)
        {
            ...
            if (TypeUtils.IsCollectionType(v.Type))
            {
                ...
                var newVarDefNode = m_command.CreateVarDefNode(definingExprNode, out var newVar);
            ...
            foreach (var p in typeInfo.PropertyRefList)
            {
                ...
                var newVarDefNode = m_command.CreateVarDefNode(propAccessor, out var newVar);
                newNodes.Add(newVarDefNode);
                newVars.Add(newVar);
        ...
        private Node FlattenEnumOrStrongSpatialVar(VarDefOp varDefOp, Node node)
        {
            ...
            var newVarDefNode = m_command.CreateVarDefNode(node, out var newVar);
        ...
        private IEnumerable<Var> FlattenVars(IEnumerable<Var> vars)
        {
            foreach (var v in vars)
            {
                ...
                if (!m_varInfoMap.TryGetVarInfo(v, out var varInfo))
                {
                    yield return v;
                }
                else
        ...
        public override Node Visit(ScanViewOp op, Node n)
        {
            ...
            PlanCompiler.Assert(inputVar is not null, "cannot identify Var for the input node to the ScanViewOp");
            ...
            if (!m_varInfoMap.TryGetVarInfo(inputVar, out var varInfo))
            {
        ...
        public override Node Visit(UnnestOp op, Node n)
        {
                ...
                if (null != varDefOp)
                {
                    if (TypeUtils.IsCollectionType(varDefOp.Var.Type))
                    {
                        ...
                        else
                        {
                            ...
                            var newChildren = new List<Node>();
                            FlattenComputedVar(computedVar, chi, out newChildren, out var newType);
            ...
            if (processingTVF is not null)
            {
                PlanCompiler.Assert(newUnnestVar is not null, "newUnnestVar must be initialized in the TVF case.");
            }
            else
            {
                ...
                if (m_varInfoMap.TryGetVarInfo(op.Var, out var unnestVarInfo)
            ...
            {
                PlanCompiler.Assert(
                    processingTVF is null, "TVFs returning a collection of values of a non-structured type are not supported");
        ...
        {
            if (m_tvfResultKeys.TryGetValue(tvf, out var keys))
            {
                return keys;
            }
        ...
        protected override Node VisitSetOp(SetOp op, Node n)
        {
            ...
            for (var i = 0; i < op.VarMap.Length; i++)
            {
                ...
                if (newComputedVars is not null)
                {
        ...
        private VarMap FlattenVarMap(VarMap varMap, out List<ComputedVar> newComputedVars)
        {
            ...
            foreach (var kv in varMap)
            {
                ...
                if (!m_varInfoMap.TryGetVarInfo(kv.Value, out var innerVarInfo))
                {
                ...
                {
                    // get my own var info
                    if (!m_varInfoMap.TryGetVarInfo(kv.Key, out var outerVarInfo))
                    {
                    ...
                    else
                    {
                        ...
                        {
                            var ret = outerSvarInfo.TryGetVar(prop, out var outerVar);
                            PlanCompiler.Assert(ret, "Could not find VarInfo for prop " + prop.Name);

                            if (!innerSvarInfo.TryGetVar(prop, out var innerVar))
                            {
        ...
        public override Node Visit(ConstantOp op, Node n)
        {
            ...
            PlanCompiler.Assert(op.Value is not null, "Value must not be null");
        ...
        public override Node Visit(CaseOp op, Node n)
        {
            ...
            var canSimplifyPrecheck = PlanCompilerUtil.IsRowTypeCaseOpWithNullability(op, n, out var thenClauseIsNull);

            VisitChildren(n);

            if (canSimplifyPrecheck)
            {
                if (TryRewriteCaseOp(n, thenClauseIsNull, out var rewrittenNode))
                {
        ...
        public override Node Visit(ComparisonOp op, Node n)
        {
            ...
            GetPropertyValues(child0TypeInfo, OperationKind.Equality, n.Child0, false, out var properties1, out var values1);
            GetPropertyValues(child1TypeInfo, OperationKind.Equality, n.Child1, false, out var properties2, out var values2);
        ...
        public override Node Visit(ConditionalOp op, Node n)
        {
            ...
            GetPropertyValues(typeInfo, OperationKind.IsNull, n.Child0, false, out var properties, out var values);
            ...
            if (TypeUtils.IsStructuredType(outputType))
            {
                ...
                foreach (var npr in outputTypeInfo.PropertyRefList)
                {
                    ...
                    if (expectedProperties.Contains(npr))
                    {
                        ...
                        if (inputTypeInfo.TryGetNewProperty(newPropRef, throwIfMissing, out var newNestedProp))
                        {
        ...
        public override Node Visit(RefOp op, Node n)
        {
            ...
            GetPropertyValues(inputTypeInfo, OperationKind.All, n.Child0, false, out var inputFields, out var inputFieldValues);
        ...
        private Node FlattenConstructor(ScalarOp op, Node n)
        {
            ...
            if (typeInfo.HasEntitySetIdProperty)
            {
                ...
                PlanCompiler.Assert(newEntityOp is not null, "unexpected optype:" + op.OpType);
        ...
        {
            PlanCompiler.Assert(typeInfo.RootType.DiscriminatorMap is not null, "should be used only for DiscriminatorMap type checks");
