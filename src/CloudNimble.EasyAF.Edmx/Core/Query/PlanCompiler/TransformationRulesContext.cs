...
namespace System.Data.Entity.Core.Query.PlanCompiler
{
    internal class TransformationRulesContext : RuleProcessingContext
    {
        ...
        {
            PlanCompiler.Assert(varRefMap is not null, "Null varRef map");
        ...
        internal Dictionary<Var, Node> GetVarMap(Node varDefListNode, Dictionary<Var, int> varRefMap)
        {
            ...
            foreach (var chi in varDefListNode.Children)
            {
                ...
                if ((nonLeafNodeCount > 100)
                    &&
                    (varRefMap is not null)
                    &&
                    varRefMap.TryGetValue(varDefOp.Var, out var refCount)
                    &&
                    (refCount > 2))
                {
                    return null;
                ...
                if (varMap.TryGetValue(varDefOp.Var, out var n))
                {
        ...
        internal static bool TryGetInt32Var(IEnumerable<Var> varList, out Var int32Var)
        {
            foreach (var v in varList)
            {
                ...
                if (TypeHelpers.TryGetPrimitiveTypeKind(v.Type, out var typeKind)
        ...
        private readonly Stack<Node> m_relOpAncestors = new();
#if DEBUG
        /// <summary>
        ...
        /// </summary>
        internal readonly StringBuilder appliedRules = new();
#endif
        ...
        internal override void PostProcess(Node n, QueryRule rule)
        {
            if (rule is not null)
            {
#if DEBUG
        ...
        private bool IsScalarOpTree(Node node, Dictionary<Var, int> varRefMap, ref int nonLeafNodeCount)
        {
            ...
            if (varRefMap is not null
                && node.Op.OpType == OpType.VarRef)
            {
                var varRefOp = (VarRefOp)node.Op;
                if (!varRefMap.TryGetValue(varRefOp.Var, out var refCount))
                {
                    refCount = 1;
                }
                else
