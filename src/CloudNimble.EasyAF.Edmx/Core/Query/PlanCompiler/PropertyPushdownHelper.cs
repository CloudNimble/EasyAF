...
namespace System.Data.Entity.Core.Query.PlanCompiler
{
    ...
    internal class PropertyPushdownHelper : BasicOpVisitor
    {
        ...
        {
            if (!m_nodePropertyRefMap.TryGetValue(node, out var propRefs))
            {
        ...
        {
            if (!m_varPropertyRefMap.TryGetValue(v, out var propRefs))
            {
                propRefs = new PropertyRefList();
        ...
        private static PropertyRefList GetKeyProperties(md.EntityType entityType)
        {
            ...
            foreach (var p in entityType.KeyMembers)
            {
                ...
                PlanCompiler.Assert(edmP is not null, "EntityType had non-EdmProperty key member?");
        ...
        public override void Visit(GetEntityRefOp op, Node n)
        {
            ...
            PlanCompiler.Assert(childOp is not null, "input to GetEntityRefOp is not a ScalarOp?");
        ...
        public override void Visit(ScanViewOp op, Node n)
        {
            ...
            PlanCompiler.Assert(inputVar is not null, "cannot determine single Var from ScanViewOp's input");
