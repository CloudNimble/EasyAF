...
namespace System.Data.Entity.Core.Query.PlanCompiler
{
    ...
    internal class PreProcessor : SubqueryTrackingVisitor
    {
        ...
        // </summary>
        private readonly Stack<EntitySet> m_entityTypeScopes = new();
        ...
        private readonly HashSet<EntitySet> m_referencedEntitySets = [];
        private readonly HashSet<TypeUsage> m_referencedTypes = [];
        ...
        private Node ExpandView(ScanTableOp scanTableOp, ref IsOfOp typeFilter)
        {
            ...
            PlanCompiler.Assert(entitySet is not null, "The target of a ScanTableOp must reference an EntitySet to be used with ExpandView");
            PlanCompiler.Assert(
            ...
            //
            PlanCompiler.Assert(definingQuery is not null, Strings.ADP_NoQueryMappingView(entitySet.EntityContainer.Name, entitySet.Name));
            ...
            Node viewNode, EntitySetBase entitySet, EntityTypeBase rootEntityType, bool includeSubtypes)
        {
            ...
            if (!m_discriminatorMaps.TryGetValue(entitySet, out var discriminatorMapInfo))
            {
        ...
        private Node RewriteNavigateOp(Node navigateOpNode, NavigateOp navigateOp, out Var outputVar)
        {
            ...
            m_command.BuildUnionAllLadder(scanTableNodes, scanTableVars, out var unionAllNode, out
            Var unionAllVar);
            ...
            var projectNode = m_command.BuildProject(filterNode, targetEnd, out var projectVar);
        ...
        private Node RewriteDerefOp(Node derefOpNode, DerefOp derefOp, out Var outputVar)
        {
            ...
            {
                var tableNode = BuildOfTypeTable(entitySet, entityType, out var tableVar);

                scanTableNodes.Add(tableNode);
                scanTableVars.Add(tableVar);
            }
            m_command.BuildUnionAllLadder(scanTableNodes, scanTableVars, out var unionAllNode, out Var unionAllVar);
        ...
        private static EntitySetBase FindTargetEntitySet(RelationshipSet relationshipSet, RelationshipEndMember targetEnd)
        {
            ...
            PlanCompiler.Assert(
                entitySet is not null, "Could not find entity set for relationship set " + relationshipSet + ";association end " + targetEnd);
            return entitySet;
        }
        ...
        private Node RewriteOneToManyNavigationProperty(
            ...
            Node sourceRefNode)
        {
            var ret = RewriteFromOneNavigationProperty(relProperty, relationshipSets, sourceRefNode, out var outputVar);
            ...
            RelProperty relProperty, List<RelationshipSet> relationshipSets, Node sourceRefNode, out Var outputVar)
        {
            ...
            m_command.BuildUnionAllLadder(scanTableNodes, scanTableVars, out var unionAllNode, out outputVar);
            ...
            Node sourceRefNode)
        {
            ...
            foreach (var r in relationshipSets)
            {
                var joinNode = BuildJoinForNavProperty(r, relProperty.ToEnd, out var rsVar, out var esVar);
            ...
            // Build the union-all node
            //
            m_command.BuildUnionAllLadder(joinNodes, outputVars, out var unionAllNode, out
            // 
            // Build the union-all node
            //
            ...
            //
            var projectNode = m_command.BuildProject(filterNode, [unionAllVars[1]], []);
        ...
        public override Node Visit(DerefOp op, Node n)
        {
            ...
            var ret = RewriteDerefOp(n, op, out var outputVar);
            ret = VisitNode(ret);
        ...
        public override Node Visit(FunctionOp op, Node n)
        {
            if (op.Function.IsFunctionImport)
            {
                ...
                if (!m_command.MetadataWorkspace.TryGetFunctionImportMapping(op.Function, out var functionImportMapping))
                {
                ...
                if (op.Function.EntitySet is not null)
                {
                    m_entityTypeScopes.Push(op.Function.EntitySet);
                    AddEntitySetReference(op.Function.EntitySet);
                    PlanCompiler.Assert(
                        functionImportMappingComposable.TvfKeys is not null && functionImportMappingComposable.TvfKeys.Length > 0,
            ...
            {
                PlanCompiler.Assert(op.Function.EntitySet is null, "Entity type scope is not supported on functions that aren't mapped.");
        ...
        public override Node Visit(CaseOp op, Node n)
        {
            ...
            if (PlanCompilerUtil.IsRowTypeCaseOpWithNullability(op, n, out var thenClauseIsNull))
            {
        ...
        private Node VisitNavPropertyOp(PropertyOp op, Node n)
        {
            ...
            if (_navigationPropertyOpRewrites.TryGetValue(nodeInfo, out var rewrite))
            {
        ...
        {
            VisitScalarOpDefault(op, n);
            var ret = RewriteNavigateOp(n, op, out var outputVar);
            ret = VisitNode(ret);
            ...
            Node keyExpr)
        {
            ...
            {
                if (!prebuiltExpressions.TryGetValue(r, out var relPropNode))
                {
        ...
        public override Node Visit(NewEntityOp op, Node n)
        {
            ...
            if (scope is null)
            {
                ...
                PlanCompiler.Assert(
                    op.RelationshipProperties is null ||
        ...
        public override Node Visit(NewMultisetOp op, Node n)
        {
            ...
            if (!n.HasChild0)
            {
                ...
                var projectNode = m_command.BuildProject(filterNode, fakeChild, out var newVar);
                     ...
                     || AreAllConstantsOrNulls(n.Children))
            {
                ...
                {
                    var singleRowTableNode = m_command.CreateNode(m_command.CreateSingleRowTableOp());
                    var projectNode = m_command.BuildProject(singleRowTableNode, chi, out var newVar);
            ...
            else
            {
                ...
                for (var i = 0; i < n.Children.Count; i++)
                {
                    ...
                    var projectNode = m_command.BuildProject(singleRowTableNode, discriminatorNode, out var newVar);
        ...
        private Node ProcessScanTable(Node scanTableNode, ScanTableOp scanTableOp, ref IsOfOp typeFilter)
        {
            ...
            PlanCompiler.Assert(scanTableOp.Table.TableMetadata.Extent is not null, "ScanTableOp must reference a table with an extent");
        ...
        private bool IsSortUnnecessary()
        {
            ...
            PlanCompiler.Assert(ancestor is not null, "unexpected SortOp as root node?");
        ...
        private class NavigationPropertyOpInfo
        {
            ...
            public NavigationPropertyOpInfo(Node node, Node root, Command command)
            {
                ...
                {
                    _hashCode
                        = ((_root is not null ? RuntimeHelpers.GetHashCode(_root) : 0) * 397
