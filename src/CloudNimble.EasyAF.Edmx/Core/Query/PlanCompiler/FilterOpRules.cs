...
namespace System.Data.Entity.Core.Query.PlanCompiler
{
    ...
    internal static class FilterOpRules
    {
        ...
        private static Node GetPushdownPredicate(Command command, Node filterNode, VarVec columns, out Node nonPushdownPredicateNode)
        {
            ...
            predicate = predicate.GetSingleTablePredicates(columns, out var nonPushdownPredicate);
        ...
        private static bool ProcessFilterOverSetOp(RuleProcessingContext context, Node filterNode, out Node newNode)
        {
            ...
            // 
            var pushdownPredicate = GetPushdownPredicate(trc.Command, filterNode, null, out var nonPushdownPredicate);
            if (pushdownPredicate is null)
            {
                return false;
            }
            ...
            // to account for that
            // 
            if (nonPushdownPredicate is not null)
            {
        ...
        private static bool ProcessFilterOverDistinct(RuleProcessingContext context, Node filterNode, out Node newNode)
        {
            ...
            // 
            var pushdownPredicate = GetPushdownPredicate(context.Command, filterNode, null, out var nonPushdownPredicate);
            if (pushdownPredicate is null)
            {
                return false;
            }
        ...
        private static bool ProcessFilterOverGroupBy(RuleProcessingContext context, Node filterNode, out Node newNode)
        {
            ...
            // 
            var pushdownPredicate = GetPushdownPredicate(context.Command, filterNode, groupByOp.Keys, out var nonPushdownPredicate);
            if (pushdownPredicate is null)
            {
                return false;
            }
            ...
            // predicate
            //
            if (nonPushdownPredicate is null)
            {
        ...
        private static bool ProcessFilterOverJoin(RuleProcessingContext context, Node filterNode, out Node newNode)
        {
            ...
            if (newJoinPredicateNode is not null)
            {
                needsTransformation = true;
                if (joinOp.OpType
                    == OpType.CrossJoin)
            ...
            if (newFilterPredicateNode is null)
            {
                newNode = newJoinNode;
            }
            else
        ...
        private static bool ProcessFilterWithConstantPredicate(RuleProcessingContext context, Node n, out Node newNode)
        {
            ...
            foreach (var v in childNodeInfo.Definitions)
            {
                ...
                var varDefNode = trc.Command.CreateVarDefNode(constNode, out var computedVar);
