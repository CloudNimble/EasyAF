...
namespace System.Data.Entity.Core.Query.PlanCompiler
{
    ...
    internal class CTreeGenerator : BasicOpVisitorOfT<DbExpression>
    {
        ...
        private class VarInfo
        {
            ...
            private readonly List<string> _propertyChain = [];
        ...
        private abstract class BindingScope : IqtVarScope
        {
            ...
            {
                resultExpr = null;
                if (_definedVars.TryGetInfo(targetVar, out var foundInfo))
                {
        ...
        private class VarDefScope : IqtVarScope
        {
            ...
            {
                resultExpr = null;
                if (_definedVars.TryGetValue(targetVar, out var foundExpr))
                {
        ...
        private readonly Stack<IqtVarScope> _bindingScopes = new();
        private readonly Stack<VarDefScope> _varScopes = new();
        private readonly Dictionary<DbExpression, RelOpInfo> _relOpState = [];

        private readonly AliasGenerator _applyAliases = new("Apply");
        private readonly AliasGenerator _distinctAliases = new("Distinct");
        private readonly AliasGenerator _exceptAliases = new("Except");
        private readonly AliasGenerator _extentAliases = new("Extent");
        private readonly AliasGenerator _filterAliases = new("Filter");
        private readonly AliasGenerator _groupByAliases = new("GroupBy");
        private readonly AliasGenerator _intersectAliases = new("Intersect");
        private readonly AliasGenerator _joinAliases = new("Join");
        private readonly AliasGenerator _projectAliases = new("Project");
        private readonly AliasGenerator _sortAliases = new("Sort");
        private readonly AliasGenerator _unionAllAliases = new("UnionAll");
        private readonly AliasGenerator _elementAliases = new("Element");
        private readonly AliasGenerator _singleRowTableAliases = new("SingleRowTable");
        private readonly AliasGenerator _limitAliases = new("Limit");
        private readonly AliasGenerator _skipAliases = new("Skip");
        ...
        {
            PlanCompiler.Assert(
                inputState is not null && inputState.PublisherName is not null && inputState.PublishedVars is not null,
        ...
        private void EnterVarDefScope(List<Node> varDefNodes)
        {
            ...
            foreach (var childNode in varDefNodes)
            {
                ...
                PlanCompiler.Assert(defOp is not null, "VarDefListOp contained non-VarDefOp child node");
            ...
            AliasGenerator defaultAliasGenerator, Dictionary<string, string> alreadyUsedNames)
        {
            ...
            if (projectedVar.TryGetName(out var columnName))
            {
        ...
        public override DbExpression Visit(ScanTableOp op, Node n)
        {
            ...
            //
            PlanCompiler.Assert(op.Table.TableMetadata.Extent is not null, "Invalid TableMetadata used in ScanTableOp - no Extent specified");
        ...
        private RelOpInfo BuildEmptyProjection(Node relOpNode)
        {
            ...
            var projectedCols = new List<KeyValuePair<string, DbExpression>>
            {
                new KeyValuePair<string, DbExpression>("C0", constExpr)
            };
        ...
        public override DbExpression Visit(GroupByOp op, Node n)
        {
            ...
            foreach (var aggVarDefNode in aggRootNode.Children)
            {
                ...
                PlanCompiler.Assert(aggVarDef is not null, "Non-VarDefOp Node encountered as child of Aggregates VarDefListOp Node");
                ...
                PlanCompiler.Assert(funcAggOp is not null, "Non-Aggregate Node encountered as child of Aggregate VarDefOp Node");
            ...
            SetOp op, Node n, AliasGenerator alias, Func<DbExpression, DbExpression, DbExpression> setOpExpressionBuilder)
        {
            ...
            if (TypeHelpers.TryGetEdmType(outputType.TypeUsage, out RowType outputElementType))
            {
            ...
            foreach (var outputVar in op.Outputs)
            {
                ...
                if (outputElementType is not null)
                {
        ...
        {
            var collectionExpr = DbExpressionBuilder.NewCollection([DbExpressionBuilder.Constant(1)]);
