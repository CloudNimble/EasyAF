...
namespace System.Data.Entity.Core.Query.PlanCompiler
{
    ...
    internal class Predicate
    {
        ...
        internal Predicate(Command command, Node andTree)
            : this(command)
        {
            PlanCompiler.Assert(andTree is not null, "null node passed to Predicate() constructor");
            ...
            out Predicate otherPredicates)
        {
            ...
            GetSingleTablePredicates(tableDefinitionList, out var singleTablePredicateList, out otherPredicates);
            ...
            out Predicate otherPredicates)
        {
            ...
            leftTableEquiJoinColumns = [];
            rightTableEquiJoinColumns = [];
            foreach (var part in m_parts)
            {

                if (IsEquiJoinPredicate(part, leftTableDefinitions, rightTableDefinitions, out var leftTableVar, out var rightTableVar))
                {
            ...
            out Var leftVar, out Var rightVar)
        {
            ...
            leftVar = null;
            rightVar = null;
            if (!IsEquiJoinPredicate(simplePredicateNode, out var tempLeftVar, out var tempRightVar))
            {
                return false;
            }
        ...
        private static bool PreservesNulls(Node simplePredNode, VarVec tableColumns)
        {
            ...
            switch (simplePredNode.Op.OpType)
            {
                ...
                case OpType.Not:
                    ...
                    return (varRefOp is null || !tableColumns.IsSet(varRefOp.Var));
