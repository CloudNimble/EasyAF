...
namespace System.Data.Entity.Core.Query.PlanCompiler
{
    ...
    internal class PlanCompiler
    {
        ...
        {
            Assert(ctree is not null, "Expected a valid, non-null Command Tree input");
...
#if DEBUG
            if (s_traceCallback is not null)
            {
        ...
        private void Initialize()
        {
            ...
            Assert(cqtree is not null, "Unexpected command tree kind. Only query command tree is supported.");

            // Generate the ITree
            m_command = ITreeGenerator.Generate(cqtree);
            Assert(m_command is not null, "Unable to generate internal tree from Command Tree");
        }
