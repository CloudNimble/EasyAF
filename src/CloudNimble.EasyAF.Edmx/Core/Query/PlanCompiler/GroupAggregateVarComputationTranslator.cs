...
namespace System.Data.Entity.Core.Query.PlanCompiler
{
    ...
    internal class GroupAggregateVarComputationTranslator : BasicOpVisitorOfNode
    {
            ...
            out bool isUnnested)
        {
            ...
            if (handler._targetGroupAggregateVarInfo is null
                || templateNode is null)
            {
                return false;
            }
            if (softCastOp is not null)
            {
        ...
        private Node VisitCollect(Node n)
        {
                   ...
                   == OpType.Project)
            {
                ...
                if (VisitDefault(currentNode.Child1) is null)
                {
                    return null;
                }
            ...
            {
                if (_targetGroupAggregateVarInfo is null)
                {
            ...
            if (computationTemplate is not null)
            {
                _isUnnested = true;
                return computationTemplate;
            }

            if (constantDefinitions.TryGetValue(outputVar, out var constantDefinitionNode))
            {
                _isUnnested = true;
