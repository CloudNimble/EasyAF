...
namespace System.Data.Entity.Core.Query.PlanCompiler
{
    ...
    internal class GroupAggregateRefComputingVisitor : BasicOpVisitor
    {
        ...
        private readonly GroupAggregateVarInfoManager _groupAggregateVarInfoManager = new();
        private readonly Dictionary<Node, Node> _childToParent = [];
        ...
        public override void Visit(VarDefOp op, Node n)
        {
            ...
            if (GroupAggregateVarComputationTranslator.TryTranslateOverGroupAggregateVar(
                definingNode, true, _command, _groupAggregateVarInfoManager, out var referencedVarInfo, out var templateNode, out var isUnnested))
            {
        ...
        public override void Visit(GroupByIntoOp op, Node n)
        {
            ...
            foreach (var child in n.Child3.Children)
            {
                ...
                if (!_groupAggregateVarInfoManager.TryGetReferencedGroupAggregateVarInfo(groupAggregateVar, out var groupAggregateVarRefInfo))
                {
        ...
        public override void Visit(FunctionOp op, Node n)
        {
            ...
            if (GroupAggregateVarComputationTranslator.TryTranslateOverGroupAggregateVar(
                n.Child0, false, _command, _groupAggregateVarInfoManager, out var referencedGroupAggregateVarInfo, out var templateNode,
                out var isUnnested)
                &&
