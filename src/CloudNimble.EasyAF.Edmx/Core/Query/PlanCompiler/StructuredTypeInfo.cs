...
namespace System.Data.Entity.Core.Query.PlanCompiler
{
    ...
    internal class StructuredTypeInfo
    {
        ...
        internal md.EntitySet GetEntitySet(md.EntityTypeBase type)
        {
            ...
            if (!m_entityTypeToEntitySetMap.TryGetValue(rootType, out var set))
            {
                return null;
            }
            return set;
        ...
        internal int GetEntitySetId(md.EntitySet e)
        {
            ...
            if (!m_entitySetToEntitySetIdMap.TryGetValue(e, out var result))
            {
        ...
        internal TypeInfo GetTypeInfo(md.TypeUsage type)
        {
            ...
            if (!m_typeInfoMap.TryGetValue(type, out var typeInfo))
            {
                PlanCompiler.Assert(
        ...
        private void AddEntityTypeToSetEntry(md.EntityType entityType, md.EntitySet entitySet)
        {
            ...
            else if (m_entityTypeToEntitySetMap.TryGetValue(rootType, out var other))
            {
                if (other != entitySet)
                {
                    hasSingleEntitySet = false;
            ...
            if (null != discriminatorMaps)
            {
                ...
                foreach (var setMapPair in discriminatorMaps)
                {
                    ...
                    if (null != map)
                    {
                        ...
                        var hasOneSet = GetEntitySet(rootType) is not null;
                        if (hasOneSet)
                        {
        ...
        private bool TryGetDiscriminatorMap(md.EdmType type, out ExplicitDiscriminatorMap discriminatorMap)
        {
            ...
            // find entity set
            if (!m_entityTypeToEntitySetMap.TryGetValue(rootEntityType, out var entitySet))
            {
                return false;
            }
        ...
        private TypeInfo CreateTypeInfoForStructuredType(md.TypeUsage type, ExplicitDiscriminatorMap discriminatorMap)
        {
            ...
            if (type.EdmType.BaseType is not null)
            {
            ...
            //
            else if (TypeHelpers.TryGetEdmType(type, out md.RefType refType))
            {
            ...
            //
            {
                if (TypeHelpers.TryGetEdmType(type, out md.EntityTypeBase entityType))
                {
        ...
        private void AssignTypeIds()
        {
            ...
            foreach (var kv in m_typeInfoMap)
            {
                ...
                if (kv.Value.RootType.DiscriminatorMap is not null)
                {
        ...
        private bool NeedsEntitySetIdProperty(TypeInfo typeInfo)
        {
            ...
            var result = ((entityType is not null) && (GetEntitySet(entityType) is null));
            return result;
        }
        ...
        {
            // Already done??
            if (rootType.FlattenedType is not null)
            {
                return;
            }
        ...
        private void ExplodeRootStructuredTypeHelper(TypeInfo typeInfo)
        {
            ...
            if (TypeHelpers.TryGetEdmType(typeInfo.Type, out md.RefType refType))
            {
                //
        ...
        private md.TypeUsage GetPropertyType(RootTypeInfo typeInfo, PropertyRef p)
        {
            ...
            if (p is TypeIdPropertyRef)
            {
                ...
                if (simplePropertyRef is not null)
                {
