...
namespace System.Data.Entity.Core.Query.InternalTrees
{
    ...
    internal class OpCopier : BasicOpVisitorOfNode
    {
        ...
        {
            return Copy(cmd, n, out var varMap);
        }
        ...
        {
            var newNode = Copy(cmd, node, out var varMap);
        ...
        private Var GetMappedVar(Var v)
        {
            ...
            //
            if (m_varMap.TryGetValue(v, out var mappedVar))
            {
                return mappedVar;
            }
        ...
        public override Node Visit(NewEntityOp op, Node n)
        {
            ...
            {
                Debug.Assert(op.EntitySet is null, "op.EntitySet must be null for the constructor that hasn't been scoped yet.");
        ...
        public override Node Visit(VarRefOp op, Node n)
        {
            ...
            if (!m_varMap.TryGetValue(op.Var, out var newVar))
            {
                newVar = op.Var;
            }
        ...
        public override Node Visit(PhysicalProjectOp op, Node n)
        {
            ...
            Debug.Assert(newColumnMap is not null, "Coping of a physical project's columnMap did not return a SimpleCollectionColumnMap");
            // Create a new ProjectOp based on the copied VarSet
