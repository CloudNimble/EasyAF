...
namespace System.Data.Entity.Core.Query.InternalTrees
{
    ...
    internal class ColumnMapFactory
    {
            ...
            StructuralType baseStructuralType)
        {
            ...
            Debug.Assert(resultMapping is not null);
            ...
            {
                baseEntityType // make sure the base type is represented
            };
            ...
            DbDataReader reader, Type type, MetadataWorkspace workspace)
        {
                                     ...
                                     .Select(p => p.GetPropertyInfoForSet()))
            {
                ...
                if (TryGetColumnOrdinalFromReader(reader, prop.Name, out var ordinal)
                    && workspace.TryDetermineCSpaceModelType(propType, out var modelType)
            ...
            Dictionary<string, FunctionImportReturnTypeStructuralTypeColumnRenameMapping> renameList)
        {
            ...
            if (!TryGetColumnOrdinalFromReader(storeDataReader, memberName, out var result))
            {
            ...
            EdmMember member, EdmType currentType, Dictionary<string, FunctionImportReturnTypeStructuralTypeColumnRenameMapping> renameList)
        {
            ...
            return renameList is null || renameList.Count == 0 || !renameList.Any(m => m.Key == member.Name)
                       ? member.Name
        ...
        {
            if (!TryGetColumnOrdinalFromReader(storeDataReader, columnName, out var result))
            {
                throw new EntityCommandExecutionException(
