...
namespace System.Data.Entity.Internal
{
    ...
    internal static class DbHelpers
    {
            ...
            Expression<Func<TEntity, TProperty>> property, string methodName, string paramName)
        {
            ...
            if (!TryParsePath(property.Body, out var path)
                || path is null)
            {
        ...
        public static IDictionary<string, Type> GetPropertyTypes(Type type)
        {
            ...
            if (!_propertyTypes.TryGetValue(type, out var types))
            {
        ...
        public static IDictionary<string, Func<object, object>> GetPropertyGetters(Type type)
        {
            ...
            if (!_propertyGetters.TryGetValue(type, out var getters))
            {
                var properties = type.GetInstanceProperties().Where(p => p.GetIndexParameters().Length == 0);
            ...
            string propertyName, IEnumerable<ValidationResult> validationResults)
        {
            ...
            foreach (var validationResult in validationResults)
            {
                ...
                var memberNames = validationResult.MemberNames is null || !validationResult.MemberNames.Any()
                                      ? [null]
