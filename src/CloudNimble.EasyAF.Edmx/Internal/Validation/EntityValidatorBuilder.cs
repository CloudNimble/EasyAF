    ...
    internal class EntityValidatorBuilder
    {
            ...
            foreach (var property in clrProperties)
            {
                ...
                if (edmProperty is not null)
                {
                    var referencingAssociations = from navigationProperty in navigationProperties
                                                  ...
                                                  where associationType is not null
                ...
                if (propertyValidator is not null)
                {
            ...
            PropertyInfo clrProperty, EdmProperty edmProperty, bool buildFacetValidators)
        {
                ...
                == BuiltInTypeKind.ComplexType)
            {
                ...
                return propertyAttributeValidators.Any() || complexTypeValidator is not null
            ...
            PropertyInfo clrProperty, EdmMember edmProperty, IEnumerable<Attribute> existingAttributes)
        {
            ...
            edmProperty.MetadataProperties.TryGetValue(
                ...
                false,
                out var storeGeneratedItem);

            var propertyIsStoreGenerated = storeGeneratedItem is not null && storeGeneratedItem.Value is not null;

            edmProperty.TypeUsage.Facets.TryGetValue(EdmConstants.Nullable, false, out var nullable);

            var nullableFacetIsFalse = nullable is not null && nullable.Value is not null && !(bool)nullable.Value;
            ...
            edmProperty.TypeUsage.Facets.TryGetValue(XmlConstants.MaxLengthElement, false, out var MaxLength);
            if (MaxLength is not null
                && MaxLength.Value is not null
