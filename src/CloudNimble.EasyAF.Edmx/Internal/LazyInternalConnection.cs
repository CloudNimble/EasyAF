...
namespace System.Data.Entity.Internal
{
    ...
    internal class LazyInternalConnection : InternalConnection
    {
        ...
        public override bool ConnectionHasModel
        {
            get
            {
                if (!_hasModel.HasValue)
                {
                    ...
                    if (UnderlyingConnection is null)
                    {
                        ...
                        else if (DbHelpers.TryGetConnectionName(_nameOrConnectionString, out var name))
                        {
        ...
        {
            if (UnderlyingConnection is not null)
            {
        ...
        {
            get { return UnderlyingConnection is not null; }
        }
        ...
        private void Initialize()
        {
            if (UnderlyingConnection is null)
            {
                Debug.Assert(AppConfig is not null);
                         ...
                         || !TryInitializeFromAppConfig(name, AppConfig))
                {
                    ...
                    {
                        if (base.ProviderName is not null)
                        {
            ...
            Debug.Assert(UnderlyingConnection is not null, "Connection should have been initialized by some mechanism.");
        }
        ...
        private bool TryInitializeFromAppConfig(string name, AppConfig config)
        {
            ...
            if (appConfigConnection is not null)
            {
        ...
        private static ConnectionStringSettings FindConnectionInConfig(string name, AppConfig config)
        {
            ...
            var appConfigConnection = (from c in candidates
                where config.GetConnectionString(c) is not null
        ...
        private void CreateConnectionFromProviderName(string providerInvariantName)
        {
            ...
            Debug.Assert(factory is not null, "Expected DbProviderFactories.GetFactory to throw if provider not found.");
