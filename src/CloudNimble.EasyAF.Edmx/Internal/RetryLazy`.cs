        ...
        where TResult : class
    {
        ...
        public TResult GetValue(TInput input)
        {
            ...
            lock (_lock)
            {
                if (_value is null)
                {
                    Debug.Assert(_valueFactory is not null, "Same thread called Value while already calculating Value.");
                    ...
                    {
                        Debug.Assert(_value is null, "_value should only be set if no exception is thrown.");
                ...
                Debug.Assert(_value is not null, "This class needs modification if it should ever return null.");
                return _value;
            }
        }
