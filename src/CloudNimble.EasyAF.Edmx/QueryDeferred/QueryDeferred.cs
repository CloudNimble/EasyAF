...
namespace EasyAF.Edmx
{
    ...
    public class QueryDeferred<TResult>
    {
        ...
        public QueryDeferred(IQueryable query, Expression expression)
        {
            ...
            var createQueryMethod = provider.GetType().GetMethod("CreateQuery", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance, null, [typeof(Expression), typeof(Type)], null);
            Query = (IQueryable<TResult>)createQueryMethod.Invoke(provider, [expression, typeof(TResult)]);
        }
        ...
        public Task<TResult> ExecuteAsync(CancellationToken cancellationToken)
        {
            ...
            return asyncQueryProvider is not null ? asyncQueryProvider.ExecuteAsync<TResult>(Expression, cancellationToken) : Task.Run(() => Execute(), cancellationToken);
        }
#endif
    }
}