...
namespace System.Data.Entity.ModelConfiguration.Configuration.Mapping
{
    internal class EntityMappingService
    {
        ...
        private void Transform()
        {
            foreach (var entitySet in _entityTypes.GetEntitySets())
            {
                ...
                foreach (var entityType in _entityTypes.GetEntityTypes(entitySet))
                {
                    ...
                    {
                        if (!setRootMappings.TryGetValue(tableMapping, out var rootMappings))
                        {
                            rootMappings = [];
                        ...
                        if (
                            !FindPropertyEntityTypeMapping(
                                ...
                                entityType,
                                requiresIsTypeOf,
                                out var propertiesTypeMapping,
                                out var propertiesTypeMappingFragment))
                        {
                            continue;
                        }
                        ...
                        if (requiresIsTypeOf)
                        {
                            if (propertiesTypeMapping.IsHierarchyMapping == false)
                            {
                                ...
                                if (isTypeOfEntityTypeMapping is null)
                                {
                        ...
                        foreach (var fkConstraint in tableMapping.Table.ForeignKeyBuilders)
                        {
                            ...
                            if (associationType is not null
        ...
        private void ConfigureAssociationSetMappingForeignKeys(EntitySet entitySet)
        {
            ...
            {
                AssociationEndMember _;
                asm.AssociationSet.ElementType.TryGuessPrincipalAndDependentEnds(out _, out var dependentEnd);
            ...
            MappingFragment conditionTypeMappingFragment)
        {
            ...
            {
                if (columnMapping.Property.PropertyPath is not null
                    &&
                    !IsRootTypeMapping(
                        rootMappings, columnMapping.Property.EntityType, columnMapping.Property.PropertyPath))
                {
                    ...
                    if (existingPropertyMapping is not null)
                    {
                ...
                if (columnMapping.Property.Conditions is not null)
                {
            ...
            EntityTypeMapping conditionTypeMapping)
        {
            ...
            if (conditionTypeMappingFragment is null)
            {
                conditionTypeMappingFragment
                ...
                if (propertiesTypeMappingFragment.GetDefaultDiscriminator() is not null)
                {
            ...
            EntitySet entitySet, EntityTypeMapping entityTypeMapping, MappingFragment fragment)
        {
            ...
            if (defaultDiscriminator is not null
                && entityTypeMapping.EntityType.BaseType is not null
                         ...
                         x =>
                             {
                                 ...
                                 if (x.PropertyMapping.PropertyPath is null)
                                 {
            ...
            IList<EdmProperty> propertyPath)
        {
            ...
            {
                if (rootMappings.TryGetValue(baseType, out var rootMapping))
                {
                    return
        ...
        {
            if (!_tableMappings.TryGetValue(table, out var tableMapping))
            {
