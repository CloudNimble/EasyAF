    ...
    public class ValueConditionConfiguration
    {
        ...
        private static void ValidateValueType(object value)
        {
            ...
            if (value is not null
                && !value.GetType().IsPrimitiveType(out var edmType))
            {
            ...
            if (existingConfiguration is not null)
            {
                if ((existingConfiguration.OverridableConfigurationParts &
                    ...
                    && !existingConfiguration.IsCompatible(
                        _configuration, inCSpace: true, errorMessage: out var errorMessage))
                {
                    throw Error.ConflictingColumnConfiguration(discriminatorColumn, fragment.Table, errorMessage);
                }
            }

            if (_configuration.IsNullable is not null)
            {
        ...
        {
            if (((existingConfiguration is not null)
                 && existingConfiguration.ColumnType is not null)
                || ((_configuration is not null)
                    && (_configuration.ColumnType is not null)))
            {
                return;
            }
