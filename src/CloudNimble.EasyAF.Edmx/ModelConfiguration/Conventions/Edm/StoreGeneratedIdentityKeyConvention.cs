...
namespace System.Data.Entity.ModelConfiguration.Conventions
{
    ...
    public class StoreGeneratedIdentityKeyConvention : IConceptualModelConvention<EntityType>
    {
        private static readonly IEnumerable<PrimitiveTypeKind> _applicableTypes
            = [PrimitiveTypeKind.Int16, PrimitiveTypeKind.Int32, PrimitiveTypeKind.Int64];
        ...
        public virtual void Apply(EntityType item, DbModel model)
        {
            ...
            Debug.Assert(item.KeyProperties is not null);

            if ((item.BaseType is null && item.KeyProperties.Count == 1)
                && !(from p in item.DeclaredProperties
                     let sgp = p.GetStoreGeneratedPattern()
                     where sgp is not null && sgp == StoreGeneratedPattern.Identity
                     select sgp).Any()) // Entity already has an Identity property.
            {
                ...
                if ((property.GetStoreGeneratedPattern() is null)
                    && property.PrimitiveType is not null
        ...
        {
            if (association.Constraint is not null
                && association.Constraint.ToProperties.Contains(property))
            {
                ...
                return sourceConfig is null
                       || targetConfig is null
                       || sourceConfig.GetTableName() is null
                       || targetConfig.GetTableName() is null
        ...
        private static bool ParentOfTpc(EntityType entityType, EdmModel model)
        {
            return (from e in model.EntityTypes.Where(et => et.GetRootType() == entityType)
                    ...
                    where configuration is not null && configuration.IsMappingAnyInheritedProperty(e)
                    select e).Any();
        }
    }
}
