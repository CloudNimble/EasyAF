        ...
        public virtual void Apply(EdmModel item, DbModel model)
        {
            ...
            var candidates
                = from entityType in item.EntityTypes
                  ...
                  where ((entityTypeConfiguration is null) // (2)
                  ...
                  where matchingAssociations.All(
                      a => a.AssociationType.Constraint is null // (4)
                           && a.AssociationType.GetConfiguration() is null // (5)
                           && !a.AssociationType.IsSelfReferencing() // (6)
                           && a.DeclaringEnd.IsOptional() // (7)
                           && a.NavigationProperties.All(n => n.GetConfiguration() is null))
                  // (8)
                  select new
                      {
