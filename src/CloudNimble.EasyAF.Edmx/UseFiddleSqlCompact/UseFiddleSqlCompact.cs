...
namespace EasyAF.Edmx
{
    internal static class UseFiddleSqlCompact
    {
        ...
        internal class MyDependencyResolver : IDbDependencyResolver
        {
            ...
            {
                if (type == typeof(DbProviderFactory) && key is not null && (string) key == "System.Data.SqlClient")
                    return SqlCeProviderFactoryInstance;
                if (type == typeof(DbProviderServices) && key is not null && (string) key == "System.Data.SqlServerCe.4.0")
            ...
            {
                return new[] {GetService(type, key)}.ToList().Where(o => o is not null);
            }
        }
