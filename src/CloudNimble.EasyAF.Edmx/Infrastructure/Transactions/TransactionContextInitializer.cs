        ...
        where TContext : TransactionContext
    {
        public void InitializeDatabase(TContext context)
        {
            ...
            if (entityConnection.State == ConnectionState.Open
                && entityConnection.CurrentTransaction is not null)
            {
                try
                {
        ...
        {
            if (DbConfiguration.DependencyResolver.GetService<Func<MigrationSqlGenerator>>(context.InternalContext.ProviderName) is not null)
            {
                ...
                return migrationSqlGenerator.Generate([createTableOperation], providerManifestToken);
            }
            else
            {
                return
