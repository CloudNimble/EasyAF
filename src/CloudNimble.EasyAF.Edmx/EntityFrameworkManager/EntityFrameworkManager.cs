...
namespace EasyAF.Edmx
{
    ...
    public static class EntityFrameworkManager
    {
        ...
        public static ConcurrentDictionary<Type, ConcurrentDictionary<Type, bool>> IsAssignableFromDict = new();
        ...
        {
            EasyAF.Edmx.UseFiddleSqlCompact.Hook(sqlCeProviderServicesInstance, sqlCeProviderFactoryInstance);
        }
        ...
        public static bool IsAssignableFrom(Type parentClass, Type baseClass)
        {
            ...
            if (!IsAssignableFromDict.TryGetValue(parentClass, out var assignableFromDict))
            {

                assignableFromDict = IsAssignableFromDict[parentClass] = new ConcurrentDictionary<Type, bool>();
            }

            if (!assignableFromDict.TryGetValue(baseClass, out var isAssignable))
            {
